...
function detectLocale(): Locale {
  ...
  for (const language of languages) {
    ...
    if (language.toLowerCase().startsWith("cs")) return "cs"
    if (language.toLowerCase().startsWith("hu")) return "hu"
    if (language.toLowerCase().startsWith("ro")) return "ro"
    if (language.toLowerCase().startsWith("el")) return "el"
...
function build(locale: Locale): Dictionary {
  ...
  // The renderer startup dictionary is intentionally conservative. A locale
  // without a complete desktop dictionary must remain readable in English;
  // never silently display a different language (the old default was Korean).
  return base
}
