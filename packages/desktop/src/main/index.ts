...
import { assertDesktopWindowHandle, clickScreen, focusDesktopWindow, getWindowRect, hotkeyScreen, listDesktopWindows, listWindowElements, typeScreenText } from "./computer-use-windows"
...
const main = Effect.gen(function* () {
  ...
  // Do not wait for the sidecar's module graph or health probe before opening
  // the native surface. The renderer already has an initialization screen and
  // waits for `await-initialization` before mounting server-backed providers.
  // Keeping the window behind `spawnLocalServer()` made a slow first start look
  // like a double-click had done nothing on Windows.
      ...
      spawnLocalServer(hostname, port, password, {
        ...
        onHostRequest: async (action, payload) => {
          ...
          if (action === "desktop.windows") {
            const input = payload as { title?: unknown; pid?: unknown }
            const filter = {
              ...(typeof input?.title === "string" && input.title.trim() ? { title: input.title } : {}),
              ...(typeof input?.pid === "number" && Number.isFinite(input.pid) ? { pid: input.pid } : {}),
            }
            return await listDesktopWindows(Object.keys(filter).length > 0 ? filter : undefined)
          }
          if (action === "desktop.elements") {
            return await listWindowElements(assertDesktopWindowHandle((payload as { handle?: unknown })?.handle))
          }
          if (action === "desktop.window-rect") {
            return await getWindowRect(assertDesktopWindowHandle((payload as { handle?: unknown })?.handle))
          }
          if (action === "desktop.click-screen") {
            const input = payload as { x?: unknown; y?: unknown }
            const x = typeof input?.x === "number" ? input.x : NaN
            const y = typeof input?.y === "number" ? input.y : NaN
            return await clickScreen(x, y)
          }
          if (action === "desktop.type-text") {
            const text = typeof (payload as { text?: unknown })?.text === "string" ? (payload as { text: string }).text : ""
            return await typeScreenText(text)
          }
          if (action === "desktop.hotkey") {
            const keys = typeof (payload as { keys?: unknown })?.keys === "string" ? (payload as { keys: string }).keys : ""
            return await hotkeyScreen(keys)
          }
          if (action === "desktop.focus-window") {
            return await focusDesktopWindow(assertDesktopWindowHandle((payload as { handle?: unknown })?.handle))
          }
  ...
  // The renderer still waits for the real server credentials before mounting
  // the workbench; the visible startup surface is deliberately independent.
