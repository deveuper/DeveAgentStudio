...
// ponytail: the sidecar imports a ~23MB single server bundle on every launch.
// V8's compile cache (Node 22.1+) persists parsed/compiled JS across runs, so
// the 2nd+ startup skips most of the parse/eval cost. Best-effort: silently
// ignored where unavailable.
try {
  const enableCompileCache = (module as unknown as { enableCompileCache?: (dir?: string) => unknown })
    .enableCompileCache
  if (typeof enableCompileCache === "function") enableCompileCache()
} catch {
  /* compile cache unsupported — continue normally */
}
...
function prepareSidecarEnv(password: string, userDataPath: string, databasePath?: string) {
  Object.assign(process.env, {
    ...
    // DeveAgent team/moa dispatch drives member subagents through the native
    // TaskTool background seam; without this the members fall back to
    // foreground execution where child failures were swallowed.
