...
const rendererProtocol = "deveagent"
...
export function createMainWindow() {
  const defaults = defaultWindowSize(screen.getPrimaryDisplay().workAreaSize)
  ...
  const win = new BrowserWindow({
    ...
    width: state.width,
    height: state.height,
    // Show the native window immediately. Waiting for the renderer's
    // `ready-to-show` event made a slow sidecar or renderer look like a
    // double-click did nothing on Windows.
    show: true,
...
function wireWindowRecovery(win: BrowserWindow, name: string) {
  ...
  win.webContents.on("render-process-gone", (_event, details) => {
    ...
    void show(
      "DeveAgent window terminated unexpectedly",
  ...
  win.on("unresponsive", () => {
    ...
    sampler.start()
    void show("DeveAgent is not responding", "You can relaunch the app, open the logs, or keep waiting.", true)
  })
  win.on("responsive", () => {
