...
const CONTEXT_GROUP_TOOLS = new Set(["read", "glob", "grep", "list", "pty", "pty_write", "computer-use"])
...
export function UserMessageDisplay(props: { message: UserMessage; parts: PartType[]; actions?: UserActions }) {
  ...
  // Independent-vision injected notes (OCR/API image analysis) are synthetic
  // so they never masquerade as typed text; render them as a muted note so the
  // user always sees what happened to an image-only attachment.
  const visionNotes = createMemo(
    () =>
      (props.parts?.filter(
        (p) =>
          p.type === "text" &&
          (p as TextPart).synthetic &&
          ((p as TextPart).text.startsWith("[Image text extracted via OCR]") ||
            (p as TextPart).text.startsWith("[Image analyzed via independent vision]")),
      ) as TextPart[]) ?? [],
  )
  ...
  return (
    <div data-component="user-message" data-timeline-part-id={textPart()?.id}>
      ...
      <Show when={visionNotes().length > 0}>
        <div data-slot="user-message-vision-note" class="text-12-regular text-text-weak">
          <For each={visionNotes()}>
            {(note) => (
              <div style={{ "font-style": "italic", "white-space": "pre-wrap", overflow: "hidden" }}>
                {note.text}
              </div>
            )}
          </For>
...
PART_MAPPING["reasoning"] = function ReasoningPartDisplay(props) {
  ...
  const text = () => readPartText(data.store.part_text_accum_delta, part())
  // Collapsed reasoning, Codex-style: the reasoning stays open while the model
  // is still thinking and collapses to a label once completed. A manual toggle
  // overrides the streaming default.
  const [override, setOverride] = createSignal<boolean | undefined>(undefined)
  const expanded = createMemo(() => override() ?? streaming())
  const toggle = () => setOverride((value) => !(value ?? streaming()))
    ...
    <Show when={text()}>
      ...
      >
        <button
          ...
          onClick={toggle}
        >
          <span data-slot="reasoning-part-label">{i18n.t("ui.sessionTurn.reasoning.label")}</span>
...
// ponytail: computer-use (CU) tool card — click/type/key/scroll/wait shown
// as a readable action line instead of raw GenericTool JSON.
ToolRegistry.register({
  name: "computer-use",
  render(props) {
    const i18n = useI18n()
    ...
    const args: string[] = []
    if (typeof props.input.text === "string" && props.input.text) args.push(props.input.text)
    if (props.input.deltaY !== undefined) args.push(`deltaY=${props.input.deltaY}`)
    return (
      <BasicTool
        {...props}
        icon="eye"
        trigger={{
          title: i18n.t("ui.tool.computer"),
          subtitle: action,
          args,
        }}
      >
...
// ponytail: interactive PTY sessions — one-line collapsed trigger with the
// command (or target session for pty_write) and the raw session output as the
// expanded body, matching the glob/grep card shape.
ToolRegistry.register({
  name: "pty",
  render(props) {
    const i18n = useI18n()
    const args: string[] = []
    if (props.input.workdir) args.push("dir=" + getDirectory(String(props.input.workdir)))
    if (props.input.yield_ms !== undefined) args.push("yield=" + props.input.yield_ms)
    return (
      <BasicTool
        {...props}
        icon="console"
        trigger={{
          title: i18n.t("ui.tool.pty"),
          subtitle: typeof props.input.command === "string" ? props.input.command : "pty",
          args,
        }}
      >
        <Show when={props.output}>
...
ToolRegistry.register({
  name: "pty_write",
  render(props) {
    const chars = typeof props.input.chars === "string" ? props.input.chars : ""
    const shown = chars ? (chars === "\u0003" ? "Ctrl+C" : JSON.stringify(chars)) : ""
    return (
      <BasicTool
        {...props}
        icon="console"
        trigger={{
          title: "pty_write",
          subtitle: typeof props.input.session_id === "string" ? props.input.session_id : undefined,
          args: shown ? [shown] : [],
        }}
      >
        <Show when={props.output}>
