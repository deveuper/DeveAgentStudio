...
export interface SessionReviewProps {
  ...
  empty?: JSX.Element
  split?: boolean
  /** Optional: apply the first hunk of this file's patch (R158). */
  onApplyHunk?: (file: string, hunkPatch: string) => void
...
function firstHunkOf(patch: string | undefined): string | undefined {
  if (!patch) return undefined
  const start = patch.indexOf("@@ ")
  if (start === -1) return undefined
  const next = patch.indexOf("@@ ", start + 3)
                            ...
                            <div data-slot="session-review-trigger-content">
                              ...
                              <div data-slot="session-review-trigger-actions">
                                <Switch>
                                  ...
                                  <Show when={props.onApplyHunk && !!firstHunkOf(diff().patch)}>
                                    <IconButton
                                      data-action="review-hunk-apply"
                                      type="button"
                                      icon="check"
                                      variant="secondary"
                                      size="normal"
                                      onClick={() => {
                                        const hunk = firstHunkOf(diff().patch)
                                        if (hunk && props.onApplyHunk) props.onApplyHunk(diff().file, hunk)
                                      }}
                                    />
                                  </Show>
