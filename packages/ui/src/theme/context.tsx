...
const STORAGE_KEYS = {
  ...
  COLOR_SCHEME_EXPLICIT: "opencode-color-scheme-explicit",
...
const names: Record<string, string> = {
  "oc-2": "OC-2",
  "deveagent-dark": "DeveAgent Dark",
  "deveagent-light": "DeveAgent Light",
  "deveagent-forest": "DeveAgent Forest",
  "deveagent-retro": "DeveAgent Retro",
  amoled: "AMOLED",
  aura: "Aura",
  ayu: "Ayu",
  carbonfox: "Carbonfox",
...
function applyThemeCss(theme: DesktopTheme, themeId: string, mode: "light" | "dark") {
  ...
  // Inline style beats every stylesheet rule: the resolved mode is explicit
  // (never "system" at this point), so pin it so static `@media
  // (prefers-color-scheme)` rules cannot flip the chrome on dark-OS machines.
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: { defaultTheme?: string; onThemeApplied?: (theme: DesktopTheme, mode: "light" | "dark") => void }) => {
    ...
    // The product default is LIGHT on every OS: "system" would follow the OS
    // and show the dark variant on dark-OS machines, contradicting the light
    // default requirement. Users can still pick system/dark in Settings.
    ...
    const ids = () => {
      const extra = Object.keys(store.themes)
        ...
        .sort()
      // The picker only surfaces DeveAgent-branded themes; the other bundled
      // themes stay loadable (setTheme still accepts them) but are not listed.
      const listed = themeIDs().filter((id) => id.startsWith("deveagent-"))
    ...
    onMount(() => {
      ...
      void load(savedTheme).then((theme) => {
        ...
        // Same promise as setTheme: a persisted dark-named theme must boot
        // into its declared scheme, not the light default.
        // An explicit scheme choice made in Settings outranks the theme's
        // designed scheme; without this guard the boot-time reconciliation
        // rewrote that choice on every restart.
        if (
          theme.preferredScheme &&
          store.colorScheme !== theme.preferredScheme &&
          read(STORAGE_KEYS.COLOR_SCHEME_EXPLICIT) !== "1"
        ) {
          setStore("colorScheme", theme.preferredScheme)
          write(STORAGE_KEYS.COLOR_SCHEME, theme.preferredScheme)
          setStore("mode", theme.preferredScheme)
        }
      })
    })
    ...
    const setTheme = (id: string) => {
      ...
      void load(next).then((theme) => {
        ...
        cacheThemeVariants(theme, next)
        write(STORAGE_KEYS.THEME_ID, next)
        // A theme declares the color scheme it is designed for (e.g.
        // DeveAgent Dark is dark in every mode); selecting the theme applies
        // its scheme so the selection actually renders as its name promises.
        if (theme.preferredScheme && store.colorScheme !== theme.preferredScheme) {
          setStore("colorScheme", theme.preferredScheme)
          write(STORAGE_KEYS.COLOR_SCHEME, theme.preferredScheme)
          setStore("mode", theme.preferredScheme)
        }
    ...
    const setColorScheme = (scheme: ColorScheme) => {
      ...
      write(STORAGE_KEYS.COLOR_SCHEME_EXPLICIT, "1")
