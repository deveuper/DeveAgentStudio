        ...
        filepath: yield* Effect.cached(
          Effect.gen(function* () {
            ...
            const bytes = yield* HttpClientRequest.get(url).pipe(
              ...
              // The result is cached; without a bound a stalled GitHub
              // connection would hang every ripgrep consumer forever.
              Effect.timeout("60 seconds"),
              Effect.mapError((cause) =>
                cause instanceof Error ? cause : new Error(`ripgrep download failed: ${url} (${String(cause)})`),
              ),
            )
