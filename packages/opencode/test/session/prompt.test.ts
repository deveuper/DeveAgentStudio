...
import { setSessionAuxiliary } from "../../src/plugin/deveagent"
...
it.instance("loop switches to a configured provider fallback before any output", () =>
  Effect.gen(function* () {
    const { llm } = yield* useServerConfig(providerCfg)
    const prompt = yield* SessionPrompt.Service
    const sessions = yield* Session.Service
    const chat = yield* sessions.create({ title: "Pinned" })
    setSessionAuxiliary(chat.id, {
      fallbackChain: [{ providerID: "test", modelID: "fallback-model" }],
    })
    for (let attempt = 0; attempt < 2; attempt++) {
      yield* llm.errorMatch(
        (hit) => hit.body.model === "test-model",
        503,
        { error: { message: "provider unavailable" } },
      )
    }
    yield* llm.textMatch((hit) => hit.body.model === "fallback-model", "fallback answer")
    yield* prompt.prompt({
      sessionID: chat.id,
      agent: "build",
      noReply: true,
    ...
    if (result.info.role === "assistant") {
      expect(String(result.info.providerID)).toBe("test")
      expect(String(result.info.modelID)).toBe("fallback-model")
    }
...
it.instance("prompt tools replace previous prompt tool rules", () =>
  Effect.gen(function* () {
    // The test provider is needed for model resolution; noReply admits without
    // a provider run, so the LLM server queue stays empty.
