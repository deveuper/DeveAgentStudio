...
const preOutputFailureLLM = Layer.succeed(
  LLM.Service,
  LLM.Service.of({
    stream: () => Stream.make(LLMEvent.providerError({ message: "provider unavailable" })),
  }),
)
const preOutputFailureEnv = LayerNode.buildLayer(root, {
  replacements: [...replacements, LayerNode.replace(LLM.node, preOutputFailureLLM)],
})
const itPreOutputFailure = testEffect(preOutputFailureEnv)
...
itPreOutputFailure.live("session.processor effect marks an empty failed attempt safe for fallback", () =>
  provideTmpdirInstance(
    (dir) =>
      Effect.gen(function* () {
        const { processors, session, provider } = yield* boot()
        ...
        expect(handle.canFailover).toBe(true)
      }),
    { config: cfg },
  ),
)
