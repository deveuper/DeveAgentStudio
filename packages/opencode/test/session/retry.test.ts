...
describe("session.retry.delay", () => {
  ...
  test("jitter stays within 25% of the base delay", () => {
    const error = apiError()
  ...
  test("prefers retry-after-ms when shorter than exponential", () => {
    ...
    expect(SessionRetry.delay(4, error, 0)).toBe(1500)
  })

  test("uses retry-after seconds when reasonable", () => {
    const error = apiError({ "retry-after": "30" })
    expect(SessionRetry.delay(3, error, 0)).toBe(30000)
  })
  ...
  test("uses retry-after values even when exceeding 10 minutes with headers", () => {
    ...
    expect(SessionRetry.delay(1, error, 0)).toBe(50000)

    const longError = apiError({ "retry-after-ms": "700000" })
    expect(SessionRetry.delay(1, longError, 0)).toBe(700000)
  })

  test("caps oversized header delays to the runtime timer limit", () => {
    const error = apiError({ "retry-after-ms": "999999999999" })
    expect(SessionRetry.delay(1, error, 0)).toBe(SessionRetry.RETRY_MAX_DELAY)
  })
  ...
  test("matches retryable API response bodies", () => {
    const error = Schema.decodeUnknownSync(SessionV1.APIError.Schema)(
      new SessionV1.APIError({
        ...
        isRetryable: false,
        statusCode: 400,
        responseBody: JSON.stringify({ error: "upstream returned status 524" }),
      }).toObject(),
    )
