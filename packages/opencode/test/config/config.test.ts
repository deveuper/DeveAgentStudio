...
test("config parser can preserve unknown top-level keys for write-backs", () => {
  // Read paths ignore unknown keys; the update/updateGlobal write-back parse
  // preserves them so a rewrite never erases forward-compatible keys.
  const preserved = ConfigParse.schema(
    ConfigV1.Info,
    { permission: { "*": "deny" }, custom_key: true },
    "test",
    { onExcessProperty: "preserve" },
  )
  expect(preserved).toHaveProperty("custom_key")
  const ignored = ConfigParse.schema(ConfigV1.Info, { permission: { "*": "deny" }, custom_key: true }, "test")
  expect(ignored).not.toHaveProperty("custom_key")
})
