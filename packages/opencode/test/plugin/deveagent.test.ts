...
import {
  ...
  getExpertPrompt,
  createReviewScope,
  createDeveAgentCodeGraphIndex,
  createDeveAgentContextPack,
  saveLocalSkill,
  loadLocalSkills,
  removeLocalSkill,
  startGrilling,
...
describe("Goal state machine", () => {
  ...
  test("getGoalQueue returns ready goals", () => {
    ...
    // New goals carry a 30s worker grace window (nextAttemptAt): the creating
    // turn drives first; the worker poll is the recovery net, not an instant
    // double-prompt. ready flips once the window elapses.
    expect(item!.ready).toBe(false)
    expect(item!.nextAttemptAt).toBeGreaterThan(Date.now() - 1000)
    expect(item!.nextAttemptAt!).toBeLessThanOrEqual(Date.now() + 31_000)
  })
...
// --- CodeGraph transitive blast radius ---

describe("CodeGraph transitive blast radius", () => {
  test("changed file surfaces direct AND transitive import dependents via context-pack", async () => {
    const { mkdtempSync, writeFileSync, rmSync } = await import("node:fs")
    const { tmpdir } = await import("node:os")
    const { join } = await import("node:path")
    const dir = mkdtempSync(join(tmpdir(), "da-cg-blast-"))
    try {
      // a.ts imports b.ts; b.ts imports c.ts. A change to c.ts must surface
      // BOTH b.ts (direct) and a.ts (transitive) — one-hop used to miss a.ts.
      writeFileSync(join(dir, "a.ts"), 'import { b } from "./b"\nexport function a() { return b() }\n')
      writeFileSync(join(dir, "b.ts"), 'import { c } from "./c"\nexport function b() { return c() }\n')
      writeFileSync(join(dir, "c.ts"), "export function c() { return 1 }\n")
      const index = await createDeveAgentCodeGraphIndex({ directory: dir })
      expect(index.edgeCount).toBeGreaterThan(0)
      const pack = await createDeveAgentContextPack({ directory: dir, files: [{ path: "c.ts" }], maxFiles: 10 })
