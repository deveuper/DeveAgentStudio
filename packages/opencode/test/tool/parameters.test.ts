...
describe("tool parameters", () => {
  ...
  describe("task", () => {
    ...
    test("accepts an optional hard output-token cap", () => {
      const parsed = parse(Task, { description: "d", prompt: "p", subagent_type: "general", max_output_tokens: 4_000 })
      expect(parsed.max_output_tokens).toBe(4_000)
    })
    test("accepts a read-only model override for advisor subagents", () => {
      const parsed = parse(Task, {
        description: "d",
        prompt: "p",
        subagent_type: "general",
        read_only: true,
        model: { providerID: "example", modelID: "advisor" },
      })
      expect(parsed.read_only).toBe(true)
      expect(parsed.model).toEqual({ providerID: "example", modelID: "advisor" })
    })
