...
describe("ProviderTransform.message - DeepSeek reasoning content", () => {
  ...
  test("DeepSeek without tool calls omits reasoning_content from providerOptions", () => {
    const msgs = [
      {
        role: "assistant",
        content: [
  ...
  test("api.id without deepseek keeps reasoning_content even when the provider id says deepseek", () => {
    // The omit branch is keyed on model.api.id, not the provider/model id:
    // a third-party model served through a deepseek-named provider must keep
    // the field so the always-set behavior is not silently narrowed.
    const msgs = [
      {
        role: "assistant",
        content: [
    ...
    const result = ProviderTransform.message(
      msgs,
      {
        id: ModelV2.ID.make("deepseek/qwen-max"),
        providerID: ProviderV2.ID.make("deepseek"),
        api: {
          id: "qwen-max",
          url: "https://api.deepseek.com",
        ...
        name: "Qwen Max via DeepSeek-shaped provider",
        capabilities: {
          temperature: true,
          reasoning: true,
          attachment: false,
    ...
    expect(result[0].content).toEqual([{ type: "text", text: "Answer" }])
    expect(result[0].providerOptions?.openaiCompatible?.reasoning_content).toBe("Internal thought")
  })
...
describe("ProviderTransform sampling defaults - DeepSeek V4 Flash", () => {
  const model = (providerID: string, id: string) =>
    ({ id: `${providerID}/${id}`, providerID, api: { id } }) as unknown as Parameters<typeof ProviderTransform.topP>[0]
