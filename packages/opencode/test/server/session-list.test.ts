...
describe("session.list", () => {
  ...
  it.instance(
    "hides archived sessions by default and exposes them when requested",
    () =>
      Effect.gen(function* () {
        ...
        const activeIDs = (yield* SessionNs.use.list()).map((session) => session.id)
        const allIDs = (yield* SessionNs.use.list({ archived: true })).map((session) => session.id)
