...
describe("instance HttpApi", () => {
  ...
  it.live("rejects remote Skill install outside or through a symlinked workspace", () =>
    Effect.gen(function* () {
      const dir = yield* tmpdirScoped({ git: true })
      const outside = yield* tmpdirScoped({ git: true })
      const escaped = yield* HttpClientRequest.post("/api/deveagent/skill/install").pipe(
        directoryHeader(dir),
      ...
      const linked = yield* Effect.promise(() => symlink(dir, link, "junction").then(() => true).catch(() => false))
      if (!linked) return
      yield* Effect.addFinalizer(() => Effect.promise(() => unlink(link).catch(() => undefined)))

      const symlinked = yield* HttpClientRequest.post("/api/deveagent/skill/install").pipe(
        directoryHeader(dir),
        HttpClientRequest.bodyJson({
          url: "https://raw.githubusercontent.com/acme/skills/main/SKILL.md",
          id: "boundary-symlink",
  ...
  it.live("keeps remote Skill listing and removal inside the current instance workspace", () =>
    Effect.gen(function* () {
      const dir = yield* tmpdirScoped({ git: true })
      const outside = yield* tmpdirScoped({ git: true })
      const listedOutside = yield* HttpClientRequest.get("/api/deveagent/skill/list-remote").pipe(
        HttpClientRequest.setUrlParam("directory", outside),
        directoryHeader(dir),
        HttpClient.execute,
      )
      expect(listedOutside.status).toBe(400)

      const removedOutside = yield* HttpClientRequest.post("/api/deveagent/skill/remove").pipe(
        directoryHeader(dir),
        HttpClientRequest.bodyJson({ id: "outside", directory: outside }),
        Effect.flatMap(HttpClient.execute),
      )
      expect(removedOutside.status).toBe(400)
    }),
  )

  it.live("verifies vision-chain session ownership at the instance boundary", () =>
    Effect.gen(function* () {
      const currentDirectory = yield* tmpdirScoped({ git: true })
      const otherDirectory = yield* tmpdirScoped({ git: true })
      const createSession = (directory: string, title: string) =>
        HttpClientRequest.post(SessionPaths.create).pipe(
          directoryHeader(directory),
          HttpClientRequest.bodyJson({ title }),
          Effect.flatMap(HttpClient.execute),
        )

      const current = (yield* (yield* createSession(currentDirectory, "current")).json) as { id: string }
      const other = (yield* (yield* createSession(otherDirectory, "other")).json) as { id: string }
      const denied = yield* HttpClientRequest.post("/api/deveagent/auxiliary").pipe(
        directoryHeader(currentDirectory),
        HttpClientRequest.bodyJson({ sessionID: other.id, visionChain: [{ providerID: "openai", modelID: "gpt-4o" }] }),
        Effect.flatMap(HttpClient.execute),
      )
      expect(denied.status).toBe(403)

      const allowed = yield* HttpClientRequest.post("/api/deveagent/auxiliary").pipe(
        directoryHeader(currentDirectory),
        HttpClientRequest.bodyJson({ sessionID: current.id, visionChain: [{ providerID: "openai", modelID: "gpt-4o" }] }),
        Effect.flatMap(HttpClient.execute),
      )
      expect(allowed.status).toBe(200)

      const reset = yield* HttpClientRequest.post("/api/deveagent/auxiliary").pipe(
        directoryHeader(currentDirectory),
        HttpClientRequest.bodyJson({ sessionID: current.id, reset: true }),
        Effect.flatMap(HttpClient.execute),
      )
