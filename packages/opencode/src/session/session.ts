...
export const Event = {
  ...
  // prompt loop publishes this so the frontend can surface a visible notice
  // that names the failed model, the model it switched to, and whether that
  // model is billable.
  ProviderFallback: EventV2.define({
    type: "session.provider-fallback",
    schema: {
      sessionID: SessionID,
      failedProviderID: Schema.String,
      failedModelID: Schema.String,
      fallbackProviderID: Schema.String,
      fallbackModelID: Schema.String,
      fallbackPaid: Schema.Boolean,
