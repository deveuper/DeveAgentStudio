...
export interface Handle {
  ...
  /** True only when this attempt failed before text, reasoning, or tool activity. */
  ...
  Effect.gen(function* () {
    ...
    const create = Effect.fn("SessionProcessor.create")(function* (input: Input) {
      ...
      const handleEvent = Effect.fnUntraced(function* (value: StreamEvent) {
        switch (value.type) {
          ...
          case "text-delta":
            ...
            if (value.text) ctx.hasObservableOutput = true
          ...
          yield* Effect.gen(function* () {
            ...
            Effect.retry(
              SessionRetry.policy({
                ...
                maxAttempts: streamInput.maxProviderRetries,
                parse,
                set: (info) => {
                  // TODO(v2): Temporary dual-write while migrating session messages to v2 events.
      ...
      return {
        ...
        get canFailover() {
          return Boolean(ctx.assistantMessage.error) && !ctx.hasObservableOutput
        },
        updateToolCall,
        completeToolCall,
        process,
