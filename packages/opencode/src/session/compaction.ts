...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const processCompaction = Effect.fn("SessionCompaction.process")(function* (input: {
      ...
      const nextPrompt = compacting.prompt ?? buildPrompt({ previousSummary, context: agent.prompt ? [agent.prompt, ...compacting.context] : compacting.context })
      ...
      // Cache-reusing compaction: prepare skips the compaction persona prompt
      // (request.ts) and the system.transform hook prepends the conversation's
      // byte-stable prefix, so the summarizer request starts with the same
      // leading bytes as conversation turns (position-0 prefix-cache reuse).
      // The summarizer's own instructions ride the user prompt via nextPrompt.
