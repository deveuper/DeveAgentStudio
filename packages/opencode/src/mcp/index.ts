...
import { isDeveAgentMcpConfigVisible } from "./deveagent-gate"
import { isProjectOriginBlocked } from "@/plugin/deveagent-trust"
  ...
  Effect.gen(function* () {
        ...
        yield* Effect.forEach(
          ...
          ([key, mcp]) =>
            Effect.gen(function* () {
              ...
              // DeveAgent trust gate: a server declared by the project's own config
              // spawns a child process, so it stays down until the workspace is trusted.
              const origin = cfg.mcp_origins?.[key]
              if (origin && isProjectOriginBlocked({ directory: ctx.directory, source: origin.source, scope: origin.scope })) {
                s.status[key] = {
                  status: "failed",
                  error: `DeveAgent trust gate: project-declared MCP server "${key}" was not started (untrusted workspace). Review it, then trust the project to enable it.`,
                }
                yield* Effect.logWarning("deveagent trust gate blocked project MCP server", { key, source: origin.source })
                return
              }
    ...
    const tools = Effect.fn("MCP.tools")(function* () {
      ...
      for (const [clientName, client] of Object.entries(s.clients)) {
        ...
        if (!isDeveAgentMcpConfigVisible(mcpConfig)) continue
