...
function normalizeMessages(
  ...
  if (
    ...
    // DeepSeek's official reasoning passback rule: reasoning_content must be
    // returned only on tool-call turns. Omitting it on plain turns keeps the
    // replay envelope smaller without breaking the protocol.
    const deepseekReasoning = field === "reasoning_content" && model.api.id.toLowerCase().includes("deepseek")
    return msgs.map((msg) => {
      if (msg.role === "assistant" && Array.isArray(msg.content)) {
        ...
        if (deepseekReasoning && !filteredContent.some((part: any) => part.type === "tool-call")) {
          return { ...msg, content: filteredContent }
        }

        // Include reasoning_content | reasoning_details directly on the message
        // for all other assistant messages. On tool turns the field is always
        // set — even when empty — because some providers (e.g. DeepSeek)
        // require the empty reasoning_content back on tool-call turns.
        return {
          ...msg,
...
export function topP(model: Provider.Model) {
  ...
  // Upstream fix #41620 (hand-ported): scope DeepSeek V4 Flash sampling to
  // first-party providers; legacy third-party IDs keep their defaults.
  if (
