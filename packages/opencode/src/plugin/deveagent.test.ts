...
describe("CodeGraph ranked context", () => {
  ...
  test("MCP local allow-list env admits loopback http fixtures only", async () => {
    const prev = process.env.DEVEAGENT_MCP_ALLOW_LOCAL
    process.env.DEVEAGENT_MCP_ALLOW_LOCAL = "1"
    try {
      const local = await validateDeveAgentMcpRemoteUrl("http://127.0.0.1:3927/mcp")
      expect(local).toContain("127.0.0.1")
      // Non-loopback hosts stay blocked even with the seam on.
      await expect(validateDeveAgentMcpRemoteUrl("http://example.com/mcp")).rejects.toThrow()
      // The seam never opens non-http schemes.
      await expect(validateDeveAgentMcpRemoteUrl("file:///etc/passwd")).rejects.toThrow()
    } finally {
      if (prev === undefined) delete process.env.DEVEAGENT_MCP_ALLOW_LOCAL
      else process.env.DEVEAGENT_MCP_ALLOW_LOCAL = prev
    }
    // Seam off: loopback http is blocked again.
    await expect(validateDeveAgentMcpRemoteUrl("http://127.0.0.1:3927/mcp")).rejects.toThrow()
  })
  ...
  test("independent verifier blocks a false completion claim and records per-criterion verdicts", async () => {
    const directory = await mkdtemp(path.join(tmpdir(), "deveagent-goal-verifier-"))
    const sessionID = "goal-verifier-reject"
    const originalFetch = globalThis.fetch
    try {
      ...
      const hooks = await deveagentPlugin.server({ client: {}, directory } as unknown as Parameters<typeof deveagentPlugin.server>[0])
      setGoal({ sessionID, description: "Ship the focused fix", criteria: ["tests pass", "changelog updated"] })
            ...
            choices: [
              {
                message: {
                  content: JSON.stringify({
                    criteria: [
                      { index: 1, met: true, reason: "suite output present" },
                      { index: 2, met: false, reason: "no changelog entry in the transcript" },
                    ],
                    allMet: true,
                  }),
                },
      ...
      const output = await hooks.tool!["goal-verify"].execute(
        { met: true, reason: "I believe it is done." },
        { sessionID } as unknown as ToolContext,
      )
      const result = JSON.parse(output as string) as {
        status: string
        verifier?: { verdict: string; allMet: boolean; perCriterion: { index: number; met: boolean }[] }
        independentVerifier?: { verdict: string }
      }
      expect(result.status).toBe("in_progress")
      expect(result.verifier?.verdict).toBe("rejected")
      expect(result.verifier?.allMet).toBe(false)
      expect(result.verifier?.perCriterion.map((item) => item.met)).toEqual([true, false])
      expect(result.independentVerifier?.verdict).toBe("rejected")
  ...
  test("independent verifier confirms a backed claim and fails closed when the verifier cannot answer", async () => {
    const directory = await mkdtemp(path.join(tmpdir(), "deveagent-goal-verifier-ok-"))
    const sessionID = "goal-verifier-confirm"
    const originalFetch = globalThis.fetch
    try {
      await mkdir(path.join(directory, ".deveagent"), { recursive: true })
      await writeFile(path.join(directory, ".deveagent", "verifier.json"), JSON.stringify({ baseUrl: "http://verifier.test/v1", model: "verifier-m" }))
      ...
      globalThis.fetch = (async () =>
        new Response(JSON.stringify({ choices: [{ message: { content: JSON.stringify({ criteria: [{ index: 1, met: true, reason: "evidence present" }], allMet: true }) } }] }), {
          status: 200,
        })) as unknown as typeof fetch
      setGoal({ sessionID, description: "Ship the focused fix", criteria: ["tests pass"] })
      ...
      // retries, then fails honestly instead of trusting an unverified claim.
      clearGoal(sessionID)
      setGoal({ sessionID, description: "Second goal", criteria: ["tests pass"] })
      globalThis.fetch = (async () => {
      ...
      for (let attempt = 1; attempt <= 3; attempt += 1) {
        const failClosed = JSON.parse(
          (await hooks.tool!["goal-verify"].execute({ met: true }, { sessionID } as unknown as ToolContext)) as string,
        ) as { status: string; independentVerifier?: { verdict: string; note?: string } }
        expect(failClosed.status).toBe("in_progress")
        expect(failClosed.independentVerifier?.verdict).toBe("unavailable")
        expect(failClosed.independentVerifier?.note).toContain("独立验收不可用")
        expect(getGoal(sessionID).verifyUnavailableCount).toBe(attempt)
      }
      const exhausted = JSON.parse(
        (await hooks.tool!["goal-verify"].execute({ met: true }, { sessionID } as unknown as ToolContext)) as string,
      ) as { status: string; stopReason?: string; independentVerifier?: { verdict: string } }
      expect(exhausted.status).toBe("failed")
      expect(exhausted.stopReason).toContain("独立验收")
      expect(exhausted.independentVerifier?.verdict).toBe("unavailable")
    } finally {
      globalThis.fetch = originalFetch
      clearGoal(sessionID)
      await rm(directory, { recursive: true, force: true, maxRetries: 10, retryDelay: 200 })
...
describe("browser + vision + debate + superpowers runtime", () => {
  ...
  test("goal queue exposes only persisted in-progress goals", () => {
    ...
    // setGoal seeds a 30s worker grace window; fast-forward past it so the
    // queue reports the goal as ready for scheduling.
    goalNextAttemptForTest("queue_active", Date.now() - 1)
...
describe("persistent remote skill install", () => {
  ...
  test("remote Skill update flow: manifest records source, check detects drift, update refreshes", async () => {
    const workspace = await mkdtemp(path.join(tmpdir(), "deveagent-skill-update-"))
    const originalFetch = globalThis.fetch
    let served = "name: Update Flow Skill\n\nVersion one."
    globalThis.fetch = (async () => new Response(served, { status: 200 })) as unknown as typeof fetch
    try {
      ...
      let checks = await checkRemoteSkillUpdates({ directory: workspace })
      expect(checks.length).toBe(1)
      expect(checks[0].upToDate).toBe(true)
      expect(checks[0].error).toBeUndefined()

      served = "name: Update Flow Skill\n\nVersion two."
      checks = await checkRemoteSkillUpdates({ directory: workspace })
      expect(checks[0].upToDate).toBe(false)
      expect(checks[0].remoteHash).not.toBe(checks[0].currentHash)

      const updated = await updateRemoteSkill({ id: "update-flow-test", directory: workspace })
      expect(updated.updated).toBe(true)
      expect(await readFile(installed.savedPath, "utf8")).toContain("Version two.")
  ...
  test("updateRemoteSkill refuses skills with no recorded source", async () => {
    const workspace = await mkdtemp(path.join(tmpdir(), "deveagent-skill-nomanifest-"))
    try {
      const result = await updateRemoteSkill({ id: "never-installed", directory: workspace })
      expect(result.updated).toBe(false)
      expect(result.error).toContain("No install source recorded")
    } finally {
      await rm(workspace, { recursive: true, force: true })
    }
  })
  ...
  test("remove then re-install the same id loads it again (P1-5 unload/reload verification)", async () => {
    const workspace = await mkdtemp(path.join(tmpdir(), "deveagent-skill-loadverify-"))
    const originalFetch = globalThis.fetch
    let served = "name: Load Verify Skill\n\nDistinctive load verification marker."
    globalThis.fetch = (async () => new Response(served, { status: 200 })) as unknown as typeof fetch
    try {
      // install -> load: the installed content is actually served into the prompt pipeline
      const installed = await installRemoteSkill({
        url: "https://raw.githubusercontent.com/anthropics/anthropic-cookbook/main/README.md",
        id: "load-verify-test",
        directory: workspace,
      ...
      expect(installed.error).toBeUndefined()
      expect((await loadRemoteSkills(workspace)).find((skill) => skill.id === "load-verify-test")?.prompt).toContain("Distinctive load verification marker.")

      // unload: the same loader must no longer serve it (no residue left behind)
      expect((await removeRemoteSkill("load-verify-test", workspace)).removed).toBe(true)
      expect((await loadRemoteSkills(workspace)).find((skill) => skill.id === "load-verify-test")).toBeUndefined()

      // reload: re-installing the same id must not be blocked by leftover state,
      // and the loader must serve the fresh content rather than a stale copy.
      served = "name: Load Verify Skill\n\nReloaded marker after unload."
      const reinstalled = await installRemoteSkill({
        url: "https://raw.githubusercontent.com/anthropics/anthropic-cookbook/main/README.md",
        id: "load-verify-test",
        directory: workspace,
      })
      expect(reinstalled.error).toBeUndefined()
      expect(reinstalled.changed).toBe(true)
      const reloaded = (await loadRemoteSkills(workspace)).find((skill) => skill.id === "load-verify-test")
      expect(reloaded).toBeDefined()
      expect(reloaded?.prompt).toContain("Reloaded marker after unload.")
    } finally {
      await removeRemoteSkill("load-verify-test", workspace)
      await rm(workspace, { recursive: true, force: true })
      globalThis.fetch = originalFetch
    }
  })
...
describe("goal worker recovery and scheduling evidence", () => {
  ...
  /**
   * clearGoal & friends persist fire-and-forget; an immediate rm races the
   * write chain and EBUSYs on Windows. Flush, then best-effort remove.
   */
  async function flushAndRmTemp(dir: string) {
    await waitForGoalStoreFlush()
    await rm(dir, { recursive: true, force: true, maxRetries: 10, retryDelay: 200 }).catch(() => undefined)
  }
  ...
  // ponytail: setGoal seeds a 30s worker grace window (nextAttemptAt in the
  // future). Scheduling tests arm the goal ready immediately instead of
  // waiting out the window.
  const armGoal = (input: Parameters<typeof setGoal>[0]) => {
    const goal = setGoal(input)
    goalNextAttemptForTest(input.sessionID ?? "", Date.now() - 1)
    return goal
  }
  ...
  test("goal budget breach ends the goal as failed with the real numbers", async () => {
    ...
    await withGoalStore(async (dir) => {
      setGoal({ sessionID: "goal-budget", description: "Budgeted goal", criteria: ["A"], budgetTokens: 50 })
      goalNextAttemptForTest("goal-budget", Date.now() - 1)
      const client = {
        session: {
          promptAsync: async () => { throw new Error("must not run a pass after a budget breach") },
          get: async () => { console.log("[dbg] session.get called"); return { data: { tokens: { input: 40, output: 20, reasoning: 0, cache: { read: 0, write: 0 } }, cost: 0.01 } } },
        },
      }
      await scanGoalQueueOnce(client, dir)
      const goal = getGoal("goal-budget")
      expect(goal.status).toBe("failed")
      expect(goal.stopReason).toContain("预算已用尽")
      expect(goal.stopReason).toContain("60/50")
  ...
  test("goal cancel is not resurrected by a concurrent recovery scan", async () => {
    drainGoalQueue()
    await withGoalStore(async (dir) => {
      const calls: string[] = []
      const client = {
      ...
      setGoal({ sessionID: "goal-cancel-race", description: "Long runner", criteria: ["A"] })
      await waitForGoalStoreFlush()
      expect(getGoal("goal-cancel-race").active).toBe(true)
      // clearGoal deletes memory synchronously and persists fire-and-forget.
      clearGoal("goal-cancel-race")
      // A worker scan that starts immediately after the cancel must flush the
      // pending save before reloading, so the pre-cancel snapshot on disk
      // cannot resurrect the goal.
      await scanGoalQueueOnce(client, dir)
      expect(getGoal("goal-cancel-race").active).toBe(false)
  ...
  test("expireGoal writes a failed run record", async () => {
    drainGoalQueue()
    await withGoalStore(async (dir) => {
      const workspace = await mkdtemp(path.join(tmpdir(), "deveagent-runs-expire-"))
      try {
        const store = path.join(dir, "opencode", "deveagent-goals.json")
        await mkdir(path.dirname(store), { recursive: true })
        await writeFile(store, JSON.stringify({ version: 3, goals: [
          ["goal-expire-run", {
            active: true,
            status: "in_progress",
            description: "Will blow its deadline",
            directory: workspace,
            criteria: ["A"],
            startedAt: Date.now(),
            reentries: 0,
        ...
        const client = {
          session: {
            async prompt() {
              throw new Error("an expired goal must not be resumed")
            },
            async promptAsync() {
              throw new Error("an expired goal must not be resumed")
            },
        ...
        await scanGoalQueueOnce(client, workspace)
        expect(getGoal("goal-expire-run").status).toBe("failed")
        // The run append is async; poll briefly for the terminal record.
        let goalRun: { kind?: string; status?: string; stopReason?: string } | undefined
        for (let attempt = 0; attempt < 20 && !goalRun; attempt++) {
          try {
            const log = await readFile(path.join(workspace, ".deveagent", "deveagent-runs.log"), "utf8")
            const entries = log.trim().split("\n").filter(Boolean).map((line) => JSON.parse(line) as { id: string; kind: string; status: string; stopReason?: string })
            goalRun = entries.filter((entry) => entry.id.startsWith("goal-goal-expire-run-")).at(-1)
          } catch {
            await new Promise((resolve) => setTimeout(resolve, 100))
          }
        }
        expect(goalRun).toMatchObject({ kind: "goal", status: "failed", stopReason: "Goal wall-clock budget exhausted." })
        clearGoal("goal-expire-run")
      } finally {
  ...
  test("does not let a replaced lease commit stale Goal recovery state", async () => {
    ...
    await withGoalStore(async (dir) => {
      ...
      armGoal({ sessionID, directory: dir, description: "Keep the newer worker state", criteria: ["A"] })
      await settle()
  ...
  test("goal recovery only schedules the current workspace", async () => {
    ...
    await withGoalStore(async (dir) => {
      ...
      armGoal({ sessionID: "goal-a", directory: workspaceA, description: "Workspace A", criteria: ["A"] })
      armGoal({ sessionID: "goal-b", directory: workspaceB, description: "Workspace B", criteria: ["B"] })
      await settle()
  ...
  test("worker records lastError and capped backoff after a failed resume, then skips until nextAttemptAt", async () => {
    ...
    goalNextAttemptForTest("goal_retry", Date.now() - 1)
    await withGoalStore(async (dir) => {
      let calls = 0
  ...
  test("self-improvement review fires after enough work and the nudge reaches the model", async () => {
    const directory = await mkdtemp(path.join(tmpdir(), "deveagent-skill-review-"))
    const sessionID = "ses_skill_review"
    try {
      const hooks = await deveagentPlugin.server({ client: {}, directory } as unknown as Parameters<typeof deveagentPlugin.server>[0])
      expect(autoSkillReviewPendingForTest()).not.toContain(sessionID)

      // The trigger only fires on a settled turn once the counter crosses the
      // threshold; a mid-task idle must not queue a review.
      const idle = (id: string) => hooks.event!({ event: { type: "session.idle", properties: { sessionID: id } } } as never)
      await idle(sessionID)
      expect(autoSkillReviewPendingForTest()).not.toContain(sessionID)

      // Simulate the work: ten successful tool iterations.
      for (let index = 0; index < 10; index += 1) {
        await hooks["tool.execute.after"]!(
          { tool: "read", sessionID, callID: `call_${index}`, args: {} } as never,
          { title: "", output: "", metadata: {} } as never,
        )
      }
      await idle(sessionID)
      expect(autoSkillReviewPendingForTest()).toContain(sessionID)

      // And the next turn must actually carry the instruction to the model.
      const tail = await buildDeveAgentTurnTail({ sessionID, text: "next task", workspaceDirectory: directory })
      expect(tail.turn).toContain("Self-Improvement Review")
      expect(tail.turn).toContain("skill-save")
      // Delivered once: a second turn must not repeat it.
      const again = await buildDeveAgentTurnTail({ sessionID, text: "another task", workspaceDirectory: directory })
      expect(again.turn).not.toContain("Self-Improvement Review")
    } finally {
      await rm(directory, { recursive: true, force: true, maxRetries: 10, retryDelay: 200 })
    }
  })

  test("message mode snapshots are bounded, persisted, and survive a reload", async () => {
    await withGoalStore(async (dir) => {
      const sessionID = "ses_mode_snap"
      stampMessageMode(sessionID, "msg_a", "plan")
      stampMessageMode(sessionID, "msg_b", "build")
      stampMessageMode("other-session", "msg_c", "craft")
      await new Promise((resolve) => setTimeout(resolve, 50))
      // Bounded per session: a long session keeps only the newest stamps.
      for (let index = 0; index < 70; index += 1) stampMessageMode(sessionID, `msg_x${index}`, "build")
      const modes = getMessageModes(sessionID)
      expect(Object.keys(modes).length).toBeLessThanOrEqual(60)
      expect(modes["msg_x69"]).toBe("build")
      expect(modes["msg_a"]).toBeUndefined()
      // Restart: a fresh load from disk must restore the persisted stamps.
      const disk = JSON.parse(await readFile(path.join(dir, "opencode", "deveagent-message-modes.json"), "utf8"))
      expect(disk.version).toBe(1)
      const restored = disk.modes.find(([sid]: [string, unknown]) => sid === sessionID)
      expect(restored).toBeDefined()
      await loadMessageModesFromDisk()
      expect(getMessageModes(sessionID)["msg_x69"]).toBe("build")
      expect(getMessageModes("other-session")["msg_c"]).toBe("craft")
      // Garbage input is ignored.
      stampMessageMode("", "msg", "plan")
      stampMessageMode(sessionID, undefined, "plan")
      stampMessageMode(sessionID, "msg_y", undefined)
      expect(getMessageModes(sessionID)["msg_y"]).toBeUndefined()
    })
  })

  test("goal stops after repeated unreadable budget usage instead of running uncapped", async () => {
    drainGoalQueue()
    await withGoalStore(async (dir) => {
      const sessionID = "goal_budget_unreadable"
      const store = path.join(dir, "opencode", "deveagent-goals.json")
      await mkdir(path.dirname(store), { recursive: true })
      await writeFile(store, JSON.stringify({ version: 3, goals: [
        [sessionID, {
          active: true,
          status: "in_progress",
          description: "Run with an unreadable budget meter",
          criteria: ["A"],
          startedAt: Date.now(),
          reentries: 0,
          maxReentries: 8,
      ...
      let prompts = 0
      const promptAsync = async () => { prompts += 1 }
      const failing = { session: { get: async () => { throw new Error("usage endpoint down") }, promptAsync } }
      ...
      const goal = getGoal(sessionID)
      expect(goal.status).toBe("failed")
      expect(goal.stopReason).toContain("不可读")
      // The budget gate stops the goal before any resume prompt fires.
  ...
  test("clearing an in-flight goal aborts its active provider session", async () => {
    drainGoalQueue()
    await withGoalStore(async (dir) => {
      armGoal({ sessionID: "goal_cancel_inflight", directory: dir, description: "Cancel active provider work", criteria: ["A"] })
      let releasePrompt!: () => void
      const promptDone = new Promise<void>((resolve) => { releasePrompt = resolve })
      const aborted: string[] = []
      const client = {
  ...
  test("worker stops scheduling once the re-entry budget is exhausted", async () => {
    ...
    goalNextAttemptForTest("goal_budget_worker", Date.now() - 1)
    await withGoalStore(async (dir) => {
      let calls = 0
      const client = { session: { promptAsync: async () => { calls += 1 } } }
      await scanGoalQueueOnce(client, dir)
  ...
  test("provider goal-verify completion is not overwritten by the worker", async () => {
    ...
    await withGoalStore(async (dir) => {
      ...
      armGoal({ sessionID, directory: dir, description: "Stop after provider verification", criteria: ["done"] })
      let calls = 0
      const client = {
        session: {
          async prompt() {
  ...
  test("provider goal-verify rejection keeps the goal eligible for a later re-entry", async () => {
    ...
    await withGoalStore(async (dir) => {
      ...
      armGoal({ sessionID, directory: dir, description: "Continue after provider rejection", criteria: ["done"] })
      const client = {
        session: {
          async prompt() {
...
describe("loop worker persistence and scheduling evidence", () => {
  ...
  test("whitespace-only cron counts as no cron and keeps the interval deadline", () => {
    drainLoopQueue()
    const loop = setLoop({ sessionID: "loop-ws-cron", task: "Guarded task", intervalSeconds: 300, cron: "   " })
    expect(loop.cron).toBeUndefined()
    expect(loop.deadlineAt).toBeGreaterThan(Date.now())
    clearLoop("loop-ws-cron")
  })

  test("persisted loop revision never goes backwards when the loop object is replaced", async () => {
    drainLoopQueue()
    await withLoopStore(async (dir) => {
      const persistedRev = async (sessionID: string) => {
        const data = JSON.parse(await readFile(path.join(dir, "opencode", "deveagent-loops.json"), "utf8"))
        const entry = (data.loops as [string, { rev?: number }][]).find(([id]) => id === sessionID)
        return entry?.[1]?.rev ?? 0
      }
      // Prompt-time auto-loop (defaults) then loop-set replacing the object
      // with tighter limits — exactly the packaged sequence that reverted.
      setLoop({ sessionID: "loop-rev", task: "Auto", intervalSeconds: 60, maxRuns: 8 })
      await waitForLoopStoreFlush()
      const first = await persistedRev("loop-rev")
      setLoop({ sessionID: "loop-rev", task: "Tightened", intervalSeconds: 3600, maxRuns: 1 })
      await waitForLoopStoreFlush()
      const second = await persistedRev("loop-rev")
      // The revision must be carried across replacement, otherwise a snapshot
      // of the replaced object outranks the live one and loadLoopsFromDisk
      // resurrects the old limits.
      expect(second).toBeGreaterThan(first)
      // And the tightened limits must survive a worker tick (which reloads).
      await scanLoopQueueOnce({ session: { promptAsync: async () => {}, get: async () => ({ data: {} }) } }, dir)
      const entry = getLoop("loop-rev")
      expect(entry.maxRuns).toBe(1)
      expect(entry.intervalSeconds).toBe(3600)
      clearLoop("loop-rev")
    })
  })
  for (const rev of [0, 7]) {
    test(`v2 loop recovery preserves revision ${rev} and executes a ready pass`, async () => {
      drainLoopQueue()
      await withLoopStore(async (dir) => {
        const sessionID = `loop-v2-cold-${rev}`
        const store = path.join(dir, "opencode", "deveagent-loops.json")
        await mkdir(path.dirname(store), { recursive: true })
        await writeFile(store, JSON.stringify({ version: 2, loops: [[sessionID, {
          active: true, status: "running", task: "Recovered pass", directory: dir, rev,
          runCount: 0, maxRuns: 1, intervalSeconds: 5, retryCount: 0, maxRetries: 1,
          startedAt: Date.now(), nextRunAt: Date.now() - 1, deadlineAt: Date.now() + 60_000,
        }]] }), "utf8")
        let calls = 0
        try {
          await scanLoopQueueOnce({ session: { prompt: async () => {
            calls += 1
            expect(getLoop(sessionID).rev).toBe(rev)
          } } }, dir)
          expect(calls).toBe(1)
          expect(getLoop(sessionID)).toMatchObject({ active: false, status: "completed", runCount: 1, rev })
        } finally {
          clearLoop(sessionID)
          await waitForLoopStoreFlush()
        }
  ...
  test("v2 loop recovery replaces live state only with a strictly newer revision", async () => {
    drainLoopQueue()
    await withLoopStore(async (dir) => {
      const sessionID = "loop-v2-order"
      setLoop({ sessionID, directory: dir, task: "Live task", intervalSeconds: 3600 })
      await waitForLoopStoreFlush()
      const live = getLoop(sessionID)
      const rev = live.rev!
      const store = path.join(dir, "opencode", "deveagent-loops.json")
      const client = { session: { prompt: async () => { throw new Error("no pass is due") } } }
      try {
        for (const diskRev of [rev - 1, rev, rev + 1]) {
          await writeFile(store, JSON.stringify({ version: 2, loops: [[sessionID, {
            ...live, rev: diskRev, task: "Disk task", status: "paused", nextRunAt: undefined,
          }]] }), "utf8")
          await scanLoopQueueOnce(client, dir)
          expect(getLoop(sessionID)).toMatchObject(diskRev > rev
            ? { task: "Disk task", status: "paused", rev: diskRev }
            : { task: "Live task", status: "running", rev })
        }
        resumeLoop(sessionID)
        await waitForLoopStoreFlush()
        const persisted = JSON.parse(await readFile(store, "utf8"))
        expect(persisted.loops.find(([id]: [string, unknown]) => id === sessionID)[1].rev).toBeGreaterThan(rev + 1)
      } finally {
        clearLoop(sessionID)
        await waitForLoopStoreFlush()
      }
  ...
  test("v2 loop recovery rejects invalid explicit revisions without dropping valid entries", async () => {
    drainLoopQueue()
    await withLoopStore(async (dir) => {
      const invalid = [-1, 1.5, "7", null, Number.MAX_SAFE_INTEGER + 1]
      const loops = [...invalid, 3].map((rev, index) => [`loop-v2-invalid-${index}`, {
        active: true, status: "paused", task: "Validate revision", directory: dir, rev,
      }])
      const store = path.join(dir, "opencode", "deveagent-loops.json")
      await mkdir(path.dirname(store), { recursive: true })
      await writeFile(store, JSON.stringify({ version: 2, loops }), "utf8")
      try {
        await scanLoopQueueOnce({ session: { prompt: async () => { throw new Error("paused loop must not run") } } }, dir)
        for (let index = 0; index < invalid.length; index++) expect(getLoop(`loop-v2-invalid-${index}`).active).toBe(false)
        expect(getLoop(`loop-v2-invalid-${invalid.length}`)).toMatchObject({ active: true, status: "paused", rev: 3 })
      } finally {
        clearLoop(`loop-v2-invalid-${invalid.length}`)
        await waitForLoopStoreFlush()
      }
    })
  })
  ...
  test("v2 loop recovery never replaces an in-flight pass with a newer disk snapshot", async () => {
    drainLoopQueue()
    await withLoopStore(async (dir) => {
      const sessionID = "loop-v2-inflight"
      setLoop({ sessionID, directory: dir, task: "Active pass", maxRuns: 2 })
      runLoopNow(sessionID)
      try {
        await scanLoopQueueOnce({ session: { prompt: async () => {
          const live = getLoop(sessionID)
          await writeFile(path.join(dir, "opencode", "deveagent-loops.json"), JSON.stringify({ version: 2, loops: [[sessionID, {
            ...live, rev: live.rev! + 10, task: "Stale replacement", status: "paused",
          }]] }), "utf8")
          await scanLoopQueueOnce({ session: { prompt: async () => { throw new Error("duplicate pass") } } }, dir)
          expect(getLoop(sessionID)).toMatchObject({ task: "Active pass", status: "running", rev: live.rev })
        } } }, dir)
        expect(getLoop(sessionID)).toMatchObject({ task: "Active pass", status: "running", runCount: 1 })
      } finally {
        clearLoop(sessionID)
        await waitForLoopStoreFlush()
      }
  ...
  test("budget breach pauses a loop with the real numbers in the reason", async () => {
    drainLoopQueue()
    await withLoopStore(async (dir) => {
      const loop = setLoop({ sessionID: "loop-budget", task: "Budgeted", intervalSeconds: 3600, maxRuns: 8, budgetTokens: 100 })
      expect(loop.budgetTokens).toBe(100)
      // The session already burned 600 tokens (500 + 100 write): the worker
      // must pause the automation instead of running another pass.
      const client = {
        session: {
          promptAsync: async () => { throw new Error("must not run a pass after a budget breach") },
          get: async () => ({ data: { tokens: { input: 500, output: 100, reasoning: 0, cache: { read: 0, write: 0 } }, cost: 0.02 } }),
        },
      }
      await scanLoopQueueOnce(client, dir)
      const entry = getLoop("loop-budget")
      expect(entry.status).toBe("paused")
      expect(entry.stopReason).toContain("预算已用尽")
      expect(entry.stopReason).toContain("600/100")
      clearLoop("loop-budget")
    })
  })
  ...
  test("stale disk snapshot does not overwrite the live replacement loop", async () => {
    drainLoopQueue()
    await withLoopStore(async (dir) => {
      // Session runs automation loop #1 (one-hour interval), then replaces it
      // with loop #2 on the same sessionID. A worker tick that reloads the
      // store right after the replacement must keep loop #2 in memory
      // instead of reverting to loop #1's stale disk snapshot.
      const first = setLoop({ sessionID: "loop-stale", task: "Old automation", intervalSeconds: 3600, maxRuns: 8 })
      await waitForLoopStoreFlush()
      expect(first.task).toBe("Old automation")
      const second = setLoop({ sessionID: "loop-stale", task: "New automation", intervalSeconds: 3600, maxRuns: 8 })
      expect(second.task).toBe("New automation")
      // Simulate the worker scan racing the replacement: the disk still
      // holds loop #1 until the replacement's save drains, and the reload
      // must not resurrect it over the live key.
      const store = path.join(dir, "opencode", "deveagent-loops.json")
      await writeFile(store, JSON.stringify({ version: 1, loops: [
        ["loop-stale", { active: true, status: "running", task: "Old automation", intervalSeconds: 3600, runCount: 0, maxRuns: 8, retryCount: 0, maxRetries: 1, startedAt: Date.now() - 1000, nextRunAt: Date.now() - 1 }],
      ] }), "utf8")
      const client = { session: { promptAsync: async () => {} } }
      await scanLoopQueueOnce(client, dir)
      const entry = getLoop("loop-stale")
      expect(entry.task).toBe("New automation")
      clearLoop("loop-stale")
    })
  })
  test("a paused loop is not resurrected by a concurrent recovery scan", async () => {
    drainLoopQueue()
    await withLoopStore(async (dir) => {
      setLoop({ sessionID: "loop-pause-race", task: "Long runner", intervalSeconds: 3600 })
      await waitForLoopStoreFlush()
      expect(getLoop("loop-pause-race").status).toBe("running")
      pauseLoop("loop-pause-race")
      const client = { session: { promptAsync: async () => { throw new Error("a paused loop must not be resumed") } } }
      await scanLoopQueueOnce(client, dir)
      const after = getLoop("loop-pause-race")
      expect(after.status).toBe("paused")
      expect(after.nextRunAt).toBeUndefined()
      clearLoop("loop-pause-race")
    })
  })
  ...
  test("setLoop accepts a 5-field cron with a timezone and schedules by cron", () => {
    drainLoopQueue()
    const now = Date.now()
    const loop = setLoop({
      sessionID: "loop-cron",
    ...
    expect(loop.cron).toBe("*/5 * * * *")
    expect(loop.timezone).toBe("Asia/Shanghai")
    // Cron automations are long-lived: no wall-clock deadline unless asked for.
    expect(loop.deadlineAt).toBeUndefined()
    expect(loop.nextRunAt).toBeGreaterThan(now)
    // The scheduled instant is the next 5-minute wall boundary in Shanghai.
    const parts = new Intl.DateTimeFormat("en-US", {
      timeZone: "Asia/Shanghai",
      hour: "2-digit",
      minute: "2-digit",
      hourCycle: "h23",
    }).formatToParts(new Date(loop.nextRunAt!))
    const minute = Number(parts.find((part) => part.type === "minute")?.value)
    expect(minute % 5).toBe(0)
    expect(loop.nextRunAt! - now).toBeLessThan(6 * 60_000)
    clearLoop("loop-cron")
  })

  test("setLoop rejects invalid cron expressions and timezones instead of guessing", () => {
    expect(() => setLoop({ sessionID: "loop-bad-cron", task: "x", cron: "not a cron" })).toThrow(/Invalid 5-field cron/)
    expect(() => setLoop({ sessionID: "loop-bad-never", task: "x", cron: "0 2 31 2 *" })).toThrow(/never matches/)
    expect(() => setLoop({ sessionID: "loop-bad-tz", task: "x", cron: "*/5 * * * *", timezone: "Mars/Olympus" })).toThrow(/Unknown IANA timezone/)
    drainLoopQueue()
  })

  test("runLoopNow reschedules the next run to now and the queue reports it ready", () => {
    drainLoopQueue()
    setLoop({ sessionID: "loop-run-now", task: "Report", intervalSeconds: 3600, maxRuns: 3 })
    expect(getLoopQueue().find((item) => item.sessionID === "loop-run-now")?.ready).toBe(false)
    expect(runLoopNow("loop-run-now").ok).toBe(true)
    expect(getLoopQueue().find((item) => item.sessionID === "loop-run-now")?.ready).toBe(true)
    expect(runLoopNow("loop-missing").ok).toBe(false)
    pauseLoop("loop-run-now")
    expect(runLoopNow("loop-run-now").ok).toBe(false)
    clearLoop("loop-run-now")
  })
  ...
  for (const abortMode of ["resolve", "reject", "hang"] as const) {
    test(`loop in-flight deadline aborts a hung prompt (${abortMode}) and persists failure`, async () => {
      drainLoopQueue()
      await withLoopStore(async (dir) => {
        const sessionID = `loop-timeout-${abortMode}`
        const now = Date.now.bind(Date)
        setLoop({ sessionID, directory: dir, task: "Bound the active pass", maxRuns: 1, maxRetries: 3, maxDurationMinutes: 1 })
        runLoopNow(sessionID)
        await waitForLoopStoreFlush()
        const clock = spyOn(Date, "now").mockImplementation(() => now() + 59_000)
        let releasePrompt!: () => void
        const promptDone = new Promise<void>((resolve) => { releasePrompt = resolve })
        // Bound the regression itself even when the worker has no timeout.
        const watchdog = setTimeout(() => releasePrompt(), 2_500)
        const aborted: unknown[] = []
        let calls = 0
        const client = {
          session: {
            prompt() { calls += 1; return promptDone },
            async abort(input: unknown) {
              aborted.push(input)
              if (abortMode === "resolve") releasePrompt()
              if (abortMode === "reject") throw new Error("abort unavailable")
              if (abortMode === "hang") await new Promise<void>(() => {})
            },
          },
        }
        try {
          await scanLoopQueueOnce(client, dir)
          expect(calls).toBe(1)
          expect(aborted).toEqual([{ path: { id: sessionID }, query: { directory: path.resolve(dir) } }])
          expect(getLoop(sessionID)).toMatchObject({
            active: false, status: "failed", runCount: 0, retryCount: 0,
            stopReason: "Loop wall-clock budget exhausted.", lastError: "Loop wall-clock budget exhausted.",
          })
          expect(getLoop(sessionID).nextRunAt).toBeUndefined()
          expect(getLoop(sessionID).completedAt).toBeUndefined()
          expect(getLoopQueue(dir).some((item) => item.sessionID === sessionID)).toBe(false)
          const persisted = JSON.parse(await readFile(path.join(dir, "opencode", "deveagent-loops.json"), "utf8"))
          expect(persisted.loops.some(([id]: [string, unknown]) => id === sessionID)).toBe(false)
          const ledger = (await readFile(path.join(dir, ".deveagent", "deveagent-runs.log"), "utf8")).trim().split("\n").map((line) => JSON.parse(line))
          expect(ledger.at(-1)).toMatchObject({ sessionID, status: "failed", stopReason: "Loop wall-clock budget exhausted." })
          expect(ledger.at(-1).finishedAt).toBeGreaterThanOrEqual(ledger.at(-1).startedAt)
          expect(ledger.some((run) => run.status === "completed")).toBe(false)
          expect((await readdir(path.join(dir, "opencode"))).some((name) => name.endsWith(".lock"))).toBe(false)
          releasePrompt()
          await promptDone
          await scanLoopQueueOnce(client, dir)
          expect(calls).toBe(1)
          expect(getLoop(sessionID).status).toBe("failed")
          clearLoop(sessionID)
          expect(aborted).toHaveLength(1)
          // Reusing the session must not be blocked by stale in-flight state or a lease.
          setLoop({ sessionID, directory: dir, task: "Fresh pass", maxRuns: 1 })
          runLoopNow(sessionID)
          await scanLoopQueueOnce({ session: { prompt: async () => { calls += 1 } } }, dir)
          expect(calls).toBe(2)
          expect(getLoop(sessionID)).toMatchObject({ status: "completed", runCount: 1 })
        } finally {
  ...
  for (const bounded of [true, false]) {
    test(`loop in-flight deadline preserves normal completion (bounded=${bounded})`, async () => {
      drainLoopQueue()
      await withLoopStore(async (dir) => {
        const sessionID = `loop-normal-${bounded}`
        const now = Date.now.bind(Date)
        const loop = setLoop({ sessionID, directory: dir, task: "Finish normally", maxRuns: 1, ...(bounded ? { maxDurationMinutes: 1 } : { cron: "* * * * *" }) })
        expect(loop.deadlineAt !== undefined).toBe(bounded)
        runLoopNow(sessionID)
        await waitForLoopStoreFlush()
        const clock = spyOn(Date, "now").mockImplementation(() => now() + 59_500)
        let calls = 0
        const aborted: unknown[] = []
        try {
          await scanLoopQueueOnce({ session: {
            prompt: async () => { calls += 1 },
            abort: async (input: unknown) => { aborted.push(input) },
          } }, dir)
          await new Promise((resolve) => setTimeout(resolve, 700))
          expect(calls).toBe(1)
          expect(aborted).toEqual([])
          expect(getLoop(sessionID)).toMatchObject({ active: false, status: "completed", runCount: 1, retryCount: 0 })
          expect(getLoop(sessionID).lastError).toBeUndefined()
          const ledger = (await readFile(path.join(dir, ".deveagent", "deveagent-runs.log"), "utf8")).trim().split("\n").map((line) => JSON.parse(line))
          expect(ledger.at(-1)).toMatchObject({ sessionID, status: "completed" })
          expect(ledger.some((run) => run.status === "failed")).toBe(false)
          expect((await readdir(path.join(dir, "opencode"))).some((name) => name.endsWith(".lock"))).toBe(false)
        } finally {
          clearLoop(sessionID)
          await waitForLoopStoreFlush()
  ...
  for (const action of ["clear", "replace"] as const) {
    test(`loop in-flight deadline preserves ${action} when a prompt settles late`, async () => {
      drainLoopQueue()
      await withLoopStore(async (dir) => {
        const sessionID = `loop-late-${action}`
        setLoop({ sessionID, directory: dir, task: "Original pass", maxRuns: 1 })
        runLoopNow(sessionID)
        try {
          await scanLoopQueueOnce({ session: {
            prompt: async () => {
              if (action === "clear") clearLoop(sessionID)
              else setLoop({ sessionID, directory: dir, task: "Replacement pass", maxRuns: 2 })
            },
            abort: async () => {},
          } }, dir)
          if (action === "clear") expect(getLoop(sessionID)).toMatchObject({ active: false, status: "idle" })
          else expect(getLoop(sessionID)).toMatchObject({ active: true, status: "running", task: "Replacement pass", runCount: 0 })
          await new Promise((resolve) => setTimeout(resolve, 50))
          const ledger = (await readFile(path.join(dir, ".deveagent", "deveagent-runs.log"), "utf8")).trim().split("\n").map((line) => JSON.parse(line))
          expect(ledger.some((run) => run.status === "completed")).toBe(false)
          expect((await readdir(path.join(dir, "opencode"))).some((name) => name.endsWith(".lock"))).toBe(false)
  ...
  test("loop in-flight deadline preserves ordinary failure retries", async () => {
    drainLoopQueue()
    await withLoopStore(async (dir) => {
      const sessionID = "loop-ordinary-retry"
      setLoop({ sessionID, directory: dir, task: "Retry within budget", maxRuns: 1, maxRetries: 1 })
      runLoopNow(sessionID)
      let calls = 0
      const client = { session: { prompt: async () => { calls += 1; throw new Error("provider offline") } } }
      try {
        await scanLoopQueueOnce(client, dir)
        expect(getLoop(sessionID)).toMatchObject({ active: true, status: "running", retryCount: 1, runCount: 0, lastError: "provider offline" })
        expect(getLoop(sessionID).nextRunAt).toBeGreaterThan(Date.now())
        await scanLoopQueueOnce(client, dir)
        expect(calls).toBe(1)
        runLoopNow(sessionID)
        await scanLoopQueueOnce(client, dir)
        expect(calls).toBe(2)
        expect(getLoop(sessionID)).toMatchObject({ active: false, status: "failed", retryCount: 2, runCount: 0, stopReason: "Loop retry budget exhausted." })
        expect((await readdir(path.join(dir, "opencode"))).some((name) => name.endsWith(".lock"))).toBe(false)
      } finally {
        clearLoop(sessionID)
        await waitForLoopStoreFlush()
...
describe("team orchestration evidence", () => {
  ...
  test("team dispatch error isolation: one member's crash waits for siblings, then surfaces", async () => {
    const done: number[] = []
    const slow = (ms: number) => new Promise((resolve) => setTimeout(resolve, ms))
    const items = [0, 1, 2, 3]
    const promise = mapTeamWithConcurrency(items, 3, async (item) => {
      if (item === 0) {
        await slow(20)
        done.push(item)
    ...
    await expect(promise).rejects.toThrow("member exploded")
    // the rejection must surface only after every sibling finished its work
    expect([...done].sort((a, b) => a - b)).toEqual([0, 1, 2, 3])
  })
...
describe("computer-use tool permission gates", () => {
  ...
  test("CU permission level auto skips the ask step for computer-use tools", async () => {
    const directory = await mkdtemp(path.join(tmpdir(), "deveagent-cu-level-auto-"))
    try {
      const { setCuPermissionLevel } = await import("./deveagent-cu-level")
      await setCuPermissionLevel(directory, "auto")
      const tools = await cuTools(directory)
      const { context, asks } = cuContext()
      await withHostRequest(async () => ({ ok: true }), async () => {
        const output = await run(() => tools["computer-use-click"].execute({ x: 5, y: 6 }, context))
  ...
  test("CU permission level default keeps the ask step", async () => {
    const directory = await mkdtemp(path.join(tmpdir(), "deveagent-cu-level-def-"))
    try {
      const tools = await cuTools(directory)
      const { context, asks } = cuContext()
      await withHostRequest(async () => ({ ok: true }), async () => {
        await run(() => tools["computer-use-click"].execute({ x: 5, y: 6 }, context))
      })
...
describe("cache metrics from step-finish parts", () => {
  test("message.part.updated step-finish feeds the accumulator (regression: phantom usage event)", async () => {
    resetDeveAgentMetrics()
    const directory = await mkdtemp(path.join(tmpdir(), "deveagent-cache-metrics-"))
    try {
      const hooks = await deveagentPlugin.server({ client: {}, directory } as unknown as Parameters<typeof deveagentPlugin.server>[0])
      const event = (hooks as any).event
      // The old listener required event types "usage"/"llm.usage" that nothing
      // publishes; this is the shape session/processor.ts actually emits.
      await event({
        event: {
          ...
          properties: {
            part: {
              type: "step-finish",
              sessionID: "cache-metrics-session",
              cost: 0.002,
              tokens: { input: 1000, output: 200, reasoning: 50, cache: { read: 8000, write: 500 } },
            },
          },
        },
      })
      // Non-step-finish parts and other event types must not count.
      await event({ event: { type: "message.part.updated", properties: { part: { type: "text", text: "hi" } } } })
      await event({ event: { type: "session.idle", properties: {} } })
      const m = getDeveAgentMetrics()
      expect(m.totalCacheHitTokens).toBe(8000)
      expect(m.totalCacheMissTokens).toBe(1000)
      expect(m.totalPromptTokens).toBe(9500) // input + cache.read + cache.write
      expect(m.totalCompletionTokens).toBe(250) // output + reasoning
      expect(m.overallCacheHitRate).toBe(84) // 8000/9500
      expect(m.totalCost).toBeCloseTo(0.002, 6)
      expect(m.roundsCount).toBe(1)
      expect(m.rounds[0].cacheHitRate).toBe(84)
    } finally {
      resetDeveAgentMetrics()
      await rm(directory, { recursive: true, force: true, maxRetries: 10, retryDelay: 200 })
    }
...
describe("goal self-iteration (auto-iterate)", () => {
  const settle = () => new Promise((resolve) => setTimeout(resolve, 120))

  function mockAux(responseBody: string | null) {
    const original = globalThis.fetch
    globalThis.fetch = (async () => {
      if (responseBody === null) return new Response("boom", { status: 500 })
      return new Response(JSON.stringify({ choices: [{ message: { content: responseBody } }] }), { status: 200 })
    }) as unknown as typeof fetch
    return () => { globalThis.fetch = original }
  }

  const drainGoalQueue = () => {
    for (const item of getGoalQueue()) clearGoal(item.sessionID)
  }
  function withAuxWorkspace(fn: (dir: string) => void | Promise<void>) {
    const dir = mkdtempSync(path.join(tmpdir(), "deveagent-autoiter-"))
    mkdirSync(path.join(dir, ".deveagent"), { recursive: true })
    writeFileSync(path.join(dir, ".deveagent", "recap.json"), JSON.stringify({ baseUrl: "http://aux.test/v1", model: "m" }))
    return Promise.resolve(fn(dir)).finally(() => rmSync(dir, { recursive: true, force: true }))
  }


  test("getActiveGoalByDirectory returns the live goal anchored to a worktree", () => {
    const dir = path.join(tmpdir(), "deveagent-wt-goal-") + Math.random().toString(36).slice(2)
    mkdirSync(dir, { recursive: true })
    try {
      setGoal({ sessionID: "wt_goal_link", description: "Worktree goal", criteria: ["done"], directory: dir, autoIterate: true, maxIterations: 2 })
      expect(getActiveGoalByDirectory(dir)).toMatchObject({ status: "in_progress", description: "Worktree goal" })
      expect(getActiveGoalByDirectory(path.join(dir, "other"))).toBeUndefined()
      clearGoal("wt_goal_link")
      expect(getActiveGoalByDirectory(dir)).toBeUndefined()
    } finally {
      clearGoal("wt_goal_link")
    }
  })

  test("verified auto-iterate goal re-arms itself with the proposed next goal", async () => {
    await withAuxWorkspace(async (dir) => {
      const restoreFetch = mockAux('{"description":"Round 2: harden the auth flow","criteria":["tests added","no regressions"]}')
      try {
        drainGoalQueue()
        setGoal({ sessionID: "autoiter_1", description: "Round 1: build auth module", criteria: ["auth works"], directory: dir, autoIterate: true, maxIterations: 3 })
        const verified = verifyGoal({ sessionID: "autoiter_1", met: true })
        expect(verified.status).toBe("verified")
        await settle()
        const next = getGoal("autoiter_1")
        expect(next.active).toBe(true)
        expect(next.description).toBe("Round 2: harden the auth flow")
        expect(next.criteria).toEqual(["tests added", "no regressions"])
        expect(next.iterations).toBe(1)
        expect(next.autoIterate).toBe(true)
        expect(next.status).toBe("in_progress")
        clearGoal("autoiter_1")
      } finally {
        restoreFetch()
      }
    })
  ...
  test("toggling all criteria done verifies the goal", async () => {
    drainGoalQueue()
    setGoal({ sessionID: "crit_toggle", description: "Two-step goal", criteria: ["step one", "step two"], directory: undefined, autoIterate: true })
    const toggle = (index: number, done: boolean) => toggleGoalCriterion("crit_toggle", index, done)
    toggle(0, true)
    expect(getGoal("crit_toggle").status).toBe("in_progress")
    toggle(1, true)
    expect(getGoal("crit_toggle").status).toBe("verified")
    clearGoal("crit_toggle")
  })

  test("respects the iteration budget and never clobbers a busy slot", async () => {
    await withAuxWorkspace(async (dir) => {
      const restoreFetch = mockAux('{"description":"should never appear","criteria":["x"]}')
      try {
        drainGoalQueue()
        setGoal({ sessionID: "autoiter_2", description: "maxed out", criteria: ["done"], directory: dir, autoIterate: true, maxIterations: 1, iterations: 1 })
        verifyGoal({ sessionID: "autoiter_2", met: true })
        await settle()
        const g = getGoal("autoiter_2")
        expect(g.active).toBe(true)
        expect(g.description).toBe("maxed out")
        clearGoal("autoiter_2")
      } finally {
        restoreFetch()
      }
    })
  ...
  test("aux failure leaves the goal verified (fail-soft)", async () => {
    await withAuxWorkspace(async (dir) => {
      const restoreFetch = mockAux(null)
      try {
        drainGoalQueue()
        setGoal({ sessionID: "autoiter_3", description: "one shot", criteria: ["done"], directory: dir, autoIterate: true })
        verifyGoal({ sessionID: "autoiter_3", met: true })
        await settle()
        const g = getGoal("autoiter_3")
        // Fail-soft: the proposal failed after the verified goal was cleared,
        // so the slot is simply free - no crash, no zombie re-arm.
        expect(g.active).toBe(false)
        expect(getGoalQueue().some((item) => item.sessionID === "autoiter_3")).toBe(false)
        clearGoal("autoiter_3")
      } finally {
        restoreFetch()
      }
    })
