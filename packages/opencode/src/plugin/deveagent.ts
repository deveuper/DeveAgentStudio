...
import { copyFile, lstat, mkdir, open, readFile, readdir, realpath, rename, stat, unlink, writeFile } from "node:fs/promises"
...
// Guardian: independent LLM risk review for unattended (goal/loop) dangerous asks.
...
import { cuAskSkipped, getCuPermissionLevel, setCuLevelWriter } from "./deveagent-cu-level"
import { loadHookRules, matchHookRule } from "./deveagent-hooks"
import { recordDeveAgentRun } from "./deveagent-run"
import { shouldRunAutoSkillReview, writeAutoSkill, markAutoSkillUsed } from "./deveagent-auto-skill"
import { inspectSkills } from "./deveagent-skill-doctor"

// ponytail: window-relative -> screen point. Duplicated from the desktop main
// process (packages cannot import each other across the server/Electron
// boundary); both copies are unit-tested.
function windowRelativeToScreen(rect: { x: number; y: number; width: number; height: number }, x: number, y: number) {
  return { x: Math.round(rect.x) + Math.round(x), y: Math.round(rect.y) + Math.round(y) }
}
export { guardianReview, guardianTelemetry, loadGuardianConfig, resetGuardianCircuit } from "./deveagent-guardian"
export { loadVerifierConfig, verifyGoalIndependently } from "./deveagent-verifier"
import {
  clearSttConfig,
  loadSttConfig,
  saveSttConfig,
...
export { installLocalStt, localSttStatus, transcribeLocalAudio } from "./deveagent-local-stt"
...
export async function validateDeveAgentMcpRemoteUrl(raw: string) {
  // E-track test seam: the probe fixture runs a local MCP server over HTTP.
  // Gated behind an explicit env opt-in that only test drivers set; production
  // users keep the HTTPS-only guarantee.
  if (process.env.DEVEAGENT_MCP_ALLOW_LOCAL === "1") {
    // Loopback-only seam: no credentials, http to 127.0.0.1/localhost/[::1].
    // Every other host still goes through the full public-address checks.
    const local = new URL(raw)
    const host = local.hostname.replace(/^\[|\]$/g, "").toLowerCase()
    const loopback = host === "127.0.0.1" || host === "localhost" || host === "::1"
    if (local.protocol === "http:" && loopback && !local.username && !local.password) {
...
type DeveAgentRuntimeState = {
  ...
  auxiliary: {
    ...
    // Red line (AGENTS.md): paid models are never auto-selected for failover
    // unless this is explicitly enabled. Absent/false keeps the safe default.
...
/**
 * ponytail: long-run leak guard for per-session maps. The desktop app
 * accumulates sessions for days; payload-heavy maps (context packs, turn
 * tails, prefix shapes, team snapshots) would grow with them. Insertion-
 * ordered Maps make FIFO eviction cheap: once past the cap, drop the oldest
 * entries. Sessions are keyed by id, so an evicted entry is simply rebuilt
 * on demand.
 */
const SESSION_MAP_CAP = 128
function boundSessionMap<T>(map: Map<string, T>, cap = SESSION_MAP_CAP) {
  while (map.size > cap) {
...
export function setDeveAgentSessionContextPack(sessionID: string | undefined, pack: DeveAgentContextPack | undefined) {
  ...
  boundSessionMap(contextPackBySession)
...
// P0-4: per-message composer-mode snapshots. The transcript's plan badge must
// reflect the mode the turn RAN in, not whatever the composer shows when the
// history is rendered later. Bounded per session and globally; persisted to
// the config dir so badges survive a restart.
const MESSAGE_MODE_MAX_PER_SESSION = 60
const MESSAGE_MODE_SESSION_MAX = 40
const messageModeBySession = new Map<string, Map<string, string>>()
let messageModeStoreWrite = Promise.resolve()

function messageModeStorePath() {
  const base = process.env.XDG_CONFIG_HOME || path.join(os.homedir(), ".config")
  return path.join(base, "opencode", "deveagent-message-modes.json")
}

export function stampMessageMode(sessionID: string, messageID: string | undefined, mode: string | undefined) {
  if (!sessionID || !messageID || typeof mode !== "string" || mode.length > 24) return
  let modes = messageModeBySession.get(sessionID)
  if (!modes) {
    modes = new Map()
    if (messageModeBySession.size >= MESSAGE_MODE_SESSION_MAX) {
      const oldest = messageModeBySession.keys().next().value
      if (oldest) messageModeBySession.delete(oldest)
    }
    messageModeBySession.set(sessionID.slice(0, 160), modes)
  }
  modes.set(messageID.slice(0, 80), mode.slice(0, 24))
  while (modes.size > MESSAGE_MODE_MAX_PER_SESSION) {
    const oldest = modes.keys().next().value
    if (!oldest) break
    modes.delete(oldest)
  }
  messageModeStoreWrite = messageModeStoreWrite
    .then(async () => {
      const payload = {
        version: 1,
        modes: [...messageModeBySession.entries()].map(([sid, entries]) => [sid, [...entries.entries()]]),
      }
      await mkdir(path.dirname(messageModeStorePath()), { recursive: true })
      await writeFile(messageModeStorePath(), JSON.stringify(payload), "utf8")
    })
    .catch(() => undefined)
}

export function getMessageModes(sessionID: string): Record<string, string> {
  const modes = messageModeBySession.get(sessionID)
  if (!modes) return {}
  return Object.fromEntries(modes)
}

export async function loadMessageModesFromDisk() {
  try {
    const data = JSON.parse(await readFile(messageModeStorePath(), "utf8"))
    if (!Array.isArray(data?.modes)) return
    for (const [sessionID, entries] of data.modes) {
      if (typeof sessionID !== "string" || !Array.isArray(entries)) continue
      const modes = new Map<string, string>()
      for (const [messageID, mode] of entries) {
        if (typeof messageID === "string" && typeof mode === "string") modes.set(messageID.slice(0, 80), mode.slice(0, 24))
      }
      if (modes.size > 0) messageModeBySession.set(sessionID.slice(0, 160), modes)
    }
  } catch {
    // An absent or corrupt store must never block startup.
  }
}
...
const MODE_RULES: Record<DeveAgentMode, string[]> = {
  ...
  plan: [
    ...
    // ponytail: Codex-style plan discipline — decision-complete plans, strict
    // mode boundaries, non-mutating exploration only.
    "Explore freely with non-mutating actions (reads, search, CodeGraph) to improve the plan; never perform mutating ones.",
    "Produce a decision-complete plan: architecture, affected files, risks, task tree, and verification plan, detailed enough that an implementer needs to make zero further decisions.",
    "Plan mode ends only when the session mode explicitly changes; requests to implement now while in plan mode are answered by planning the implementation, not performing it.",
  ],
  build: [
...
function normalizeAuxiliary(input: unknown, previous: DeveAgentRuntimeState["auxiliary"]): DeveAgentRuntimeState["auxiliary"] {
  ...
  return {
    ...
    visionChain,
    fallbackChain,
    fallbackChainAllowPaid:
      typeof value.fallbackChainAllowPaid === "boolean" ? value.fallbackChainAllowPaid : previous.fallbackChainAllowPaid,
...
export function createDeveAgentRuntimePrompt(state: DeveAgentRuntimeState, sessionID?: string) {
  ...
  const lines = [
    ...
    ...(selected.some((s) => s.id === "computer-use")
      ? ["- computer-use skill: permission-gated browser/desktop host tools are available (including OS window inventory and focus on Windows); raw shell execution is disabled"]
      : []),
...
// parentID chains are immutable, so the resolved root is cached per session
// (bounded FIFO). Team/task child sessions inherit the root's automation state.
const rootSessionBySession = new Map<string, string>()

/**
 * Walk parentID links up to the automation root. Team members and TaskTool
 * children have their own session ids and no goal of their own — without this
 * walk a dangerous write from an unattended child would look "attended" and
 * skip the Guardian review entirely.
 */
async function resolveRootSessionID(sessionID: string, client: unknown, directory: string): Promise<string> {
  const cached = rootSessionBySession.get(sessionID)
  if (cached) return cached
  let current = sessionID
  for (let depth = 0; depth < 8; depth++) {
    try {
      const info = await (client as { session?: { get?: (args: unknown) => Promise<{ data?: { parentID?: unknown } }> } }).session?.get?.({
        path: { id: current },
        query: { directory },
      })
      const parentID = (info?.data as { parentID?: unknown } | undefined)?.parentID
      if (typeof parentID === "string" && parentID && parentID !== current) {
        current = parentID
        continue
      }
    } catch {
      // unknown session: treat the current id as the root
    }
    break
  }
  rootSessionBySession.set(sessionID, current)
  if (rootSessionBySession.size > 256) {
    const oldest = rootSessionBySession.keys().next().value
    if (oldest !== undefined) rootSessionBySession.delete(oldest)
  }
  return current
}
...
export async function buildDeveAgentTurnTail(input: {
  ...
  // --- per-turn: self-improvement review nudge ---
  // The idle handler marks a session when enough real work has accumulated
  // without a skill write. The nudge is per-turn (never in the stable prefix),
  // and clears once delivered so it cannot nag every turn.
  if (autoSkillReviewPending.has(input.sessionID)) {
    autoSkillReviewPending.delete(input.sessionID)
    turnLines.push(
      "## Self-Improvement Review",
      "- This session completed substantial work without saving a reusable procedure.",
      "- If the work revealed a repeatable workflow (build steps, a debugging sequence, a project convention), call skill-save with a short kebab-case name, a one-line description (max 60 chars) and a body covering When to Use / Procedure / Pitfalls / Verification.",
      "- If nothing is genuinely reusable, do not save anything. Never record environment failures, one-off narratives, or unresolved problems.",
    )
  }
  ...
  if (loop.active) {
    stateLines.push(
      "## Active Loop",
      `- Status: ${loop.status}; completed runs: ${loop.runCount}/${loop.maxRuns}; ${loop.cron ? `schedule: ${loop.cron} (${loop.timezone || "UTC"})` : `interval: ${loop.intervalSeconds}s`}.`,
      `- Task: ${loop.task}`,
...
export function recordPrefixShape(
  ...
  boundSessionMap(prefixShapeBySession)
...
async function findCodeGraphCallNeighbors(directory: string, anchors: string[], limit: number) {
  ...
  // ponytail: transitive blast radius — BFS over reverse edges so a change to
  // an anchor file surfaces files that transitively import or call it (one hop
  // only reported direct neighbors, missing indirect impact). Bounded by `limit`.
  ...
  while (queue.length > 0 && related.size < limit) {
    ...
    for (const edge of (cached.edges ?? []).filter(isDeveAgentCodeGraphEdge)) {
      ...
      // imports-resolved: current depends on edge.to; a change to edge.to
      // affects current. Also follow the reverse (edge.from imports edge.to).
      if (edge.type === "imports-resolved") {
        const reverse = edge.to === current && known.has(edge.from) && !selected.has(edge.from)
        const forward = edge.from === current && known.has(edge.to) && !selected.has(edge.to)
        if (reverse && !related.has(edge.from)) {
          related.set(edge.from, "codegraph import")
          queue.push(edge.from)
        }
        if (forward && !related.has(edge.to)) {
          related.set(edge.to, "codegraph import")
          queue.push(edge.to)
        }
      }
      // calls: edge.from calls a symbol defined in edge.to (target file).
      // A change to the callee (edge.to) affects the caller (edge.from).
      if (edge.type === "calls") {
        const target = edge.to.split("#", 1)[0]
        const reverse = target === current && known.has(edge.from) && !selected.has(edge.from)
        const forward = edge.from === current && known.has(target) && !selected.has(target)
        if (reverse && !related.has(edge.from)) {
          related.set(edge.from, "codegraph call")
          queue.push(edge.from)
        }
        if (forward && !related.has(target)) {
          related.set(target, "codegraph call")
          queue.push(target)
        }
      }
    }
...
async function recordAutomaticTaskProgress(input: { directory: string; sessionID: string; tool: string; args?: unknown }) {
  ...
  if (loop.active && (loop.status === "running" || loop.status === "paused")) {
    writes.push(writeDeveAgentMemoryProgress({
      ...
      nextAction: loop.cron
        ? `Continue the next bounded loop pass at the next cron occurrence (${loop.cron} in ${loop.timezone || "UTC"}).`
        : `Continue the next bounded loop pass after the configured interval (${loop.intervalSeconds}s).`,
    }))
  }
  ...
  server: async (input: PluginInput, options?: PluginOptions): Promise<Hooks> => {
    ...
    return {
      ...
      "chat.message": async (input, output) => {
        // P0-4: freeze the composer mode onto THIS message at submit time
        // (the UI flushes its mode to the plugin right before sending).
        stampMessageMode(input.sessionID, input.messageID ?? (output.message as { id?: string } | undefined)?.id, runtimeState.mode)
        ...
        if (blocks.state) {
          if (!marker || marker.text !== blocks.state || marker.turns >= 6) {
            ...
            boundSessionMap(turnTailStateBySession)
      ...
      "permission.ask": async (input, output) => {
        ...
        const sessionID = typeof (input as any).sessionID === "string" ? (input as any).sessionID : ""
        // PermissionV1.Request carries no agent field, so read-only guards and
        // trace attribution always saw "". Resolve it once per session from
        // the session record and keep a small cache (session→agent is stable).
        let agent = typeof (input as any).agent === "string" ? (input as any).agent : ""
        if (!agent && sessionID) {
          const cached = permissionAgentBySession.get(sessionID)
          if (cached !== undefined) {
            agent = cached
          } else {
            try {
              const info = await (pluginClient as { session?: { get?: (args: unknown) => Promise<{ data?: { agent?: unknown } }> } }).session?.get?.({
                path: { id: sessionID },
                query: { directory: workspaceDirectory },
              })
              if (typeof info?.data?.agent === "string") agent = info.data.agent
            } catch {
              // attribution is best-effort; the guard falls back to empty
            }
            permissionAgentBySession.set(sessionID, agent)
            if (permissionAgentBySession.size > 256) {
              const oldest = permissionAgentBySession.keys().next().value
              if (oldest !== undefined) permissionAgentBySession.delete(oldest)
            }
          }
        }

        // DeveAgent declarative hooks (R156): user-written deny rules from the
        // workspace/user hooks files. Checked first and fail-soft — a hooks
        // error never changes the ask outcome. Deny-only: declarative hooks
        // tighten permissions, they can never loosen them.
        try {
          const hookRules = await loadHookRules(workspaceDirectory)
          const hookHit = matchHookRule(hookRules, action)
          if (hookHit) {
            output.status = "deny"
            return
          }
        } catch {
          // fail-soft: advisory hardening only
        }

        // Dangerous paths: denied unconditionally for attended sessions. In an
        // unattended goal/loop run (no user at the keyboard) an "ask" here
        // would block forever, so the request goes to the Guardian — an
        // independent LLM risk review that fails closed on any error and
        // aborts the run after repeated denies.
        if (isDangerousPermissionTarget(filePath)) {
          // Team/task children inherit the root session's automation state, so
          // an unattended child's dangerous write reaches the Guardian instead
          // of being hard-denied as "attended".
          const rootID = sessionID ? await resolveRootSessionID(sessionID, pluginClient, workspaceDirectory) : ""
          const unattended =
            Boolean(sessionID) &&
            (getGoal(sessionID).active ||
              getLoop(sessionID).active ||
              (rootID !== sessionID && (getGoal(rootID).active || getLoop(rootID).active)))
          // filePath patterns are workspace-relative for write/edit; resolving
          // dirname(pattern) against the process cwd scattered traces outside
          // the workspace. The trace always belongs in the workspace root.
          const traceDir = workspaceDirectory
          const trace = (entry: Record<string, unknown>) => appendGuardianTrace(traceDir, entry)
          if (!unattended) {
            trace({ kind: "hook-deny", why: "attended-dangerous-target", target: filePath })
            output.status = "deny"
            return
          }
          // ponytail: a previously Guardian-denied path stays denied for this
          // session - otherwise the model reaches the same write through
          // another tool (e.g. bash) and the deny is toothless.
          const deniedHit = guardianDeniedPathFor(sessionID, filePath)
          if (deniedHit) {
            trace({ kind: "hook-deny", why: "previously-denied-path", target: filePath, sessionID })
            output.status = "deny"
            return
          }
          // ponytail: only dangerous targets reach the Guardian; ordinary asks
          // keep waiting for the user exactly as before.
          const rawPattern = (input as any).pattern
          const transcript = await readDeveAgentRecentSessionMessages({ client: pluginClient, sessionID, directory: workspaceDirectory, limit: 12 })
          const review = await guardianReview({
            sessionID,
            permission: action,
            pattern: typeof rawPattern === "string" ? rawPattern : Array.isArray(rawPattern) ? rawPattern.join(", ") : "",
            target: filePath,
            recentTranscript: transcript,
            client: pluginClient,
            directory: workspaceDirectory,
          })
          const breaker = recordGuardianOutcome(sessionID, review.outcome)
          if (review.outcome === "deny") {
            recordGuardianDeniedPath(sessionID, [filePath])
          }
          if (breaker.triggered) {
            // Two consecutive denies abort the unattended run and surface the
            // rationale via the client's event surface (best-effort).
            if (getGoal(sessionID).active) clearGoal(sessionID)
            if (getLoop(sessionID).active) clearLoop(sessionID)
            void publishGuardianAbort({ client: pluginClient, directory: workspaceDirectory, sessionID, rationale: review.rationale })
          }
          // ponytail: a guardian allow may not override the read-only hard
          // denials. Limitation: DeveAgent keeps `mode` as a window-global
          // runtime state (no per-session mode exists — getSessionAuxiliary
          // only covers auxiliary models), so this recheck reads the global
          // mode rather than the requesting session's mode.
          if (review.outcome === "allow" && !(READONLY_MODES.includes(agent) || runtimeState.mode === "ask" || runtimeState.mode === "plan")) {
            trace({ kind: "hook-allow", target: filePath, sessionID, agent, mode: runtimeState.mode, modeSource: "window-global", permissionMode: runtimeState.permissionMode })
            output.status = "allow"
            return
          }
          trace({
            kind: "hook-deny",
            why: review.outcome === "allow" ? "mode-gate-blocks-allow" : "guardian-verdict-deny",
            modeSource: "window-global",
            target: filePath,
            sessionID,
      ...
      "tool.execute.after": async (input, output) => {
        metrics.lastSessionID = input.sessionID
        // Hermes-style self-improvement counter: count tool iterations since
        // the last skill write, so a burst of real work (not a timer) earns a
        // review. Skill/memory writes reset it.
        if (input.status !== "failed" && input.status !== "cancelled") {
          const previous = autoSkillItersBySession.get(input.sessionID) ?? 0
          autoSkillItersBySession.set(input.sessionID, previous + 1)
        }
      ...
      // --- event: track cache metrics from persisted step-finish parts ---
      // The old listener matched event types "usage"/"llm.usage" that nothing
      // in the product ever publishes, so the accumulator stayed at zero and
      // cache-metrics / /api/deveagent/metrics always reported 0%. The real
      // provider usage lands as `message.part.updated` parts of type
      // "step-finish" (session/processor.ts getUsage): tokens.input is the
      // NON-cached remainder, cache.read/write are separate, cost is computed
      // by OpenCode from model pricing — same source the app status bar uses.
      event: async (input) => {
        const evt = input.event as any
        // Self-improvement trigger: when a turn settles and the tool-iteration
        // counter has crossed its threshold, hand the session to the reviewer.
        ...
        if (evt?.type === "session.idle") {
          ...
          if (sessionID) {
            const iters = autoSkillItersBySession.get(sessionID) ?? 0
            if (
              shouldRunAutoSkillReview({
                toolIterationsSinceLastSkill: iters,
                hadFinalAnswer: true,
                interrupted: false,
                enabled: true,
              })
            ) {
              // The reviewer is the agent's own skill-save tool: it already
              // validates, dedups by provenance and reports honestly. Asking
              // here would duplicate the turn, so the counter is reset and the
              // review is surfaced as a suggestion in the next turn's state.
              autoSkillItersBySession.set(sessionID, 0)
              autoSkillReviewPending.add(sessionID)
            }
          }
          return
        }
        ...
        const hit = tokens.cache?.read ?? 0
        const miss = tokens.input ?? 0
        const prompt = miss + hit + (tokens.cache?.write ?? 0)
        ...
        metrics.totalCost += typeof part.cost === "number" ? part.cost : 0
        metrics.rounds.push({
          promptTokens: prompt, completionTokens: completion,
          cacheHitTokens: hit, cacheHitRate: prompt > 0 ? Math.round((hit / prompt) * 100) : 0,
          cost: typeof part.cost === "number" ? part.cost : 0,
        })
        if (metrics.rounds.length > 200) metrics.rounds.shift()
      ...
      tool: (() => {
        ...
        "team-dispatch": tool({
          ...
          execute: async (args, context) => {
            ...
            await completeTeamRun(run, {
              ...
              runMode: "sequential",
              startedAt,
              members: [{ id: member.id, name: member.name, attempts: outcome.attempts, childSessionID: (teamChildSessionID(outcome.result) ?? run.members.find((item) => item.id === member.id)?.childSessionID), error: outcome.error, ...usage }],
              tokens: usage.tokens,
              cost: usage.cost,
              budgetTokens: activeTeam.budgetTokens,
        ...
        "team-resume-interrupted": tool({
          ...
          execute: async (args, context) => {
            ...
            await completeTeamRun(resumed, {
              ...
              resumedFrom: prior.id,
              startedAt,
              members: results.map(({ member, result, attempts, error }, index) => ({ id: member.id, name: member.name, attempts, childSessionID: (teamChildSessionID(result) ?? resumed.members.find((item: { id: string; childSessionID?: string }) => item.id === member.id)?.childSessionID), error, ...usage[index] })),
              tokens,
              cost,
              budgetTokens: activeTeam.budgetTokens,
        ...
        "team-dispatch-all": tool({
          ...
          execute: async (args, context) => {
            ...
            updateTeamRunProgress(runRecord, {
              ...
              phase: "advisors",
              startedAt,
              members: results.map(({ member, result, attempts, error }, index) => ({ id: member.id, name: member.name, attempts, childSessionID: (teamChildSessionID(result) ?? runRecord.members.find((item: { id: string; childSessionID?: string }) => item.id === member.id)?.childSessionID), error, ...usage[index] })),
              tokens: totalTokens,
              cost: totalCost,
              budgetTokens: activeTeam.budgetTokens,
            ...
            if (synthesisMember && totalTokens < activeTeam.budgetTokens) {
              ...
              await completeTeamRun(synthesisRun, {
                ...
                runMode: "debate",
                startedAt: synthesisStartedAt,
                members: [{ id: synthesisMember.id, name: synthesisMember.name, attempts: outcome.attempts, childSessionID: (teamChildSessionID(outcome.result) ?? synthesisRun.members.find((item: { id: string; childSessionID?: string }) => item.id === synthesisMember.id)?.childSessionID), error: outcome.error, ...synthesisUsage }],
            ...
            if (executor && totalTokens + synthesisTokens < activeTeam.budgetTokens) {
              ...
              await completeTeamRun(executorRun, {
                ...
                runMode: dispatch.runMode,
                startedAt: executorStartedAt,
                members: [{ id: executor.member.id, name: executor.member.name, attempts: outcome.attempts, childSessionID: (teamChildSessionID(outcome.result) ?? executorRun.members.find((item: { id: string; childSessionID?: string }) => item.id === executor.member.id)?.childSessionID), error: outcome.error, ...executorUsage }],
        ...
        "goal-verify": tool({
          ...
          execute: async (args, context) => {
            ...
            // did the work is not the only judge of it. When a verifier model is
            // configured (workspace `.deveagent/verifier.json` | guardian.json |
            // recap.json) and the agent claims met=true, every criterion is
            // re-checked against the recent transcript before the goal may flip
            ...
            if (args.met) {
              ...
              if (goal.active && goal.status === "in_progress" && goal.criteria.length > 0) {
                ...
                try {
                  verification = await verifyGoalIndependently({
                    directory: workspaceDirectory,
                    description: goal.description,
                    criteria: goal.criteria,
                    transcript: await readDeveAgentRecentSessionMessages({
                      client: pluginClient,
                      sessionID: context.sessionID,
                      directory: workspaceDirectory,
                      limit: 12,
                  ...
                  verification = { available: false, failure: "error" }
                }
                if (verification?.available) {
                  recordGoalVerification(context.sessionID, {
                    at: Date.now(),
                    model: loadVerifierConfig(workspaceDirectory)?.model ?? "unknown",
                    verdict: verification.allMet ? "confirmed" : "rejected",
                    allMet: verification.allMet,
                    perCriterion: verification.perCriterion,
                    rationale: verification.rationale,
                  })
                }
                // A configured verifier that cannot answer no longer waves the
                // claim through: the goal stays in progress for a few bounded
                // retries, then fails honestly instead of trusting an
                // unverified completion claim.
                if (verifierConfigured && verification && !verification.available) {
                  const current = markGoalVerifyUnavailable(context.sessionID)
                  return JSON.stringify({
                    ...current,
                    independentVerifier: {
                      verdict: "unavailable",
                      note: current.status === "failed" ? current.stopReason : current.lastError,
                    },
                  })
                }
              }
            ...
            if (result.active) {
              await writeDeveAgentMemoryProgress({
                ...
                summary: [
                  `Goal ${result.status}: ${result.description}`,
            ...
            if (args.met && !rejected && result.active && result.status === "verified") {
              await rememberDeveAgentMemory({
                directory: workspaceDirectory,
                sessionID: context.sessionID,
                kind: "decision",
            ...
            return JSON.stringify(
              {
                ...result,
                independentVerifier: verification
                  ? verification.available
        ...
        "loop-set": tool({
          ...
          args: {
            ...
            cron: tool.schema.optional(tool.schema.string()).describe("Optional 5-field cron expression (minute hour day-of-month month day-of-week) that replaces intervalSeconds"),
            timezone: tool.schema.optional(tool.schema.string()).describe("Optional IANA timezone for the cron schedule (defaults to the unattended timezone)"),
        ...
        "skill-save": tool({
          ...
          args: {
            ...
            body: tool.schema.string().describe("Markdown body: When to Use, Procedure, Pitfalls, Verification"),
          },
          execute: async (args, context) => {
            const result = await writeAutoSkill({
              directory: input.directory,
              name: args.name,
        ...
        "computer-use-click": tool({
          description: "Click after permission approval. Default: content-area coordinates of the current DeveAgent window. With `handle`: coordinates relative to that OS window's top-left corner. With `elementName`: finds the matching element (from computer-use-elements) and clicks its center — preferred over guessing coordinates.",
          args: {
            x: tool.schema.optional(tool.schema.number()).describe("X coordinate (content-area or window-relative)"),
            y: tool.schema.optional(tool.schema.number()).describe("Y coordinate (content-area or window-relative)"),
            handle: tool.schema.optional(tool.schema.string()).describe("Window handle from computer-use-windows for OS-window clicks"),
            elementName: tool.schema.optional(tool.schema.string()).describe("Case-insensitive element name to click (requires handle)"),
          },
          execute: async (args, context) => {
            try {
              // Element-name path: resolve the element center to screen coordinates.
              // Permission is requested before the host-availability check (contract:
              // asks must happen even when no desktop host exists).
              if (args.elementName) {
                if (!args.handle || !/^\d{1,20}$/.test(args.handle.trim())) return "Element click failed: elementName requires a valid window handle."
                await context.ask({
                  permission: "computer-use",
                  patterns: ["desktop.elements", "desktop.click-screen", args.elementName].slice(0, 4),
                  always: ["*"],
                  metadata: { source: "deveagent.computer-use-click", elementName: args.elementName, handle: args.handle },
                })
                const hostRequest = (globalThis as any).__deveagent_host_request as ((action: string, payload?: unknown) => Promise<unknown>) | undefined
                if (!hostRequest) return "Desktop click unavailable outside the desktop host."
                const inventory = (await hostRequest("desktop.elements", { sessionID: context.sessionID, handle: args.handle })) as { ok?: boolean; elements?: { name: string; role: string; enabled: boolean; bounds: { x: number; y: number; width: number; height: number } }[] }
                const needle = args.elementName.trim().toLowerCase()
                const element = (inventory?.elements ?? []).find((item) => item.enabled && item.name.toLowerCase().includes(needle))
                if (!element) return JSON.stringify({ ok: false, reason: `no enabled element matching "${args.elementName.slice(0, 80)}" in that window` })
                const center = { x: Math.round(element.bounds.x + element.bounds.width / 2), y: Math.round(element.bounds.y + element.bounds.height / 2) }
                const clicked = (await hostRequest("desktop.click-screen", { sessionID: context.sessionID, x: center.x, y: center.y })) as { ok?: boolean }
                return JSON.stringify({ ok: clicked?.ok === true, element: { name: element.name, role: element.role }, clicked: center })
              }
              // Window-relative path.
              if (args.handle) {
                if (!/^\d{1,20}$/.test(args.handle.trim())) return "Window click failed: handle must be a decimal window id."
                if (!Number.isFinite(args.x) || !Number.isFinite(args.y)) return "Window click failed: x and y are required with handle."
                await context.ask({
                  permission: "computer-use",
                  patterns: ["desktop.window-rect", "desktop.click-screen", args.handle, `${args.x},${args.y}`].slice(0, 5),
                  always: ["*"],
                  metadata: { source: "deveagent.computer-use-click", handle: args.handle, x: args.x, y: args.y },
                })
                const hostRequest = (globalThis as any).__deveagent_host_request as ((action: string, payload?: unknown) => Promise<unknown>) | undefined
                if (!hostRequest) return "Desktop click unavailable outside the desktop host."
                const rectResult = (await hostRequest("desktop.window-rect", { sessionID: context.sessionID, handle: args.handle })) as { ok?: boolean; reason?: string; rect?: { x: number; y: number; width: number; height: number } }
                if (!rectResult?.ok || !rectResult.rect) return JSON.stringify({ ok: false, reason: rectResult?.reason ?? "window rect unavailable" })
                const screen = windowRelativeToScreen(rectResult.rect, args.x ?? 0, args.y ?? 0)
                const clicked = (await hostRequest("desktop.click-screen", { sessionID: context.sessionID, x: screen.x, y: screen.y })) as { ok?: boolean }
                return JSON.stringify({ ok: clicked?.ok === true, clicked: screen, relative: { x: args.x, y: args.y } })
              }
              // DeveAgent-own-window path (unchanged behavior).
              if (!Number.isFinite(args.x) || !Number.isFinite(args.y)) return "Click failed: provide x and y."
              await context.ask({
                permission: "computer-use",
        ...
        "computer-use-windows": tool({
          description: "List visible OS windows (title, process id, bounds, focused) after permission approval, so native apps can be targeted instead of guessed at. Optional case-insensitive title substring / exact pid filter. Windows only.",
          args: {
            title: tool.schema.optional(tool.schema.string()).describe("Case-insensitive substring the window title must contain"),
            pid: tool.schema.optional(tool.schema.number()).describe("Exact process id the window must belong to"),
          },
          execute: async (args, context) => {
            await context.ask({
              permission: "computer-use",
              patterns: ["desktop.windows", args.title ?? "", args.pid !== undefined ? String(args.pid) : ""].filter(Boolean).slice(0, 4),
              always: ["*"],
              metadata: { source: "deveagent.computer-use-windows", title: args.title, pid: args.pid },
            })
            const hostRequest = (globalThis as any).__deveagent_host_request as ((action: string, payload?: unknown) => Promise<unknown>) | undefined
            if (!hostRequest) return "Window inventory unavailable outside the desktop host."
            try {
              const result = (await hostRequest("desktop.windows", { sessionID: context.sessionID, title: args.title, pid: args.pid })) as {
                available?: boolean
                reason?: string
                windows?: unknown
              }
              if (!result?.available) return `Window inventory unavailable: ${result?.reason ?? "unknown reason"}`
              return JSON.stringify(result, null, 2)
            } catch (error) {
              return `Window inventory failed: ${error instanceof Error ? error.message.slice(0, 300) : String(error).slice(0, 300)}`
            }
          },
        }),

        "computer-use-focus": tool({
          description: "Bring an OS window to the foreground by its handle from computer-use-windows, after permission approval. Windows only.",
          args: { handle: tool.schema.string().describe("Window handle reported by computer-use-windows") },
          execute: async (args, context) => {
            if (!/^\d{1,20}$/.test(args.handle.trim())) return "Window focus failed: handle must be a decimal window id."
            await context.ask({
              permission: "computer-use",
              patterns: ["desktop.focus-window", args.handle],
              always: ["*"],
              metadata: { source: "deveagent.computer-use-focus", handle: args.handle },
            })
            const hostRequest = (globalThis as any).__deveagent_host_request as ((action: string, payload?: unknown) => Promise<unknown>) | undefined
            if (!hostRequest) return "Window focus unavailable outside the desktop host."
            try {
              return JSON.stringify(await hostRequest("desktop.focus-window", { sessionID: context.sessionID, handle: args.handle }))
            } catch (error) {
              return `Window focus failed: ${error instanceof Error ? error.message.slice(0, 300) : String(error).slice(0, 300)}`
            }
          },
        }),

        "computer-use-elements": tool({
          description: "Read the UI Automation element tree of an OS window (name, role, enabled, bounds for up to 200 on-screen interactive elements), after permission approval. Use it to find buttons and fields by name instead of guessing coordinates. Windows only.",
          args: { handle: tool.schema.string().describe("Window handle reported by computer-use-windows") },
          execute: async (args, context) => {
            if (!/^\d{1,20}$/.test(args.handle.trim())) return "Element inventory failed: handle must be a decimal window id."
            await context.ask({
              permission: "computer-use",
              patterns: ["desktop.elements", args.handle],
              always: ["*"],
              metadata: { source: "deveagent.computer-use-elements", handle: args.handle },
            })
            const hostRequest = (globalThis as any).__deveagent_host_request as ((action: string, payload?: unknown) => Promise<unknown>) | undefined
            if (!hostRequest) return "Element inventory unavailable outside the desktop host."
            try {
              const result = (await hostRequest("desktop.elements", { sessionID: context.sessionID, handle: args.handle })) as { ok?: boolean; reason?: string; windowBounds?: unknown; elements?: unknown }
              if (!result?.ok) return JSON.stringify({ ok: false, reason: result?.reason ?? "unknown reason" })
              // Compact JSON: pretty-printed trees blow past the tool output cap.
              return JSON.stringify(result)
            } catch (error) {
              return `Element inventory failed: ${error instanceof Error ? error.message.slice(0, 300) : String(error).slice(0, 300)}`
            }
          },
        }),

        "computer-use-type": tool({
          description: "Type text into whatever window currently has keyboard focus (Unicode, up to 500 characters), after permission approval. Focus the target window first with computer-use-focus when possible.",
          args: { text: tool.schema.string().describe("Text to type (1-500 characters)") },
          execute: async (args, context) => {
            if (typeof args.text !== "string" || args.text.trim().length === 0) return "Type failed: text is required."
            await context.ask({
              permission: "computer-use",
              patterns: ["desktop.type-text", args.text.slice(0, 40)],
              always: ["*"],
              metadata: { source: "deveagent.computer-use-type", characters: [...args.text].length },
            })
            const hostRequest = (globalThis as any).__deveagent_host_request as ((action: string, payload?: unknown) => Promise<unknown>) | undefined
            if (!hostRequest) return "Typing unavailable outside the desktop host."
            try {
              return JSON.stringify(await hostRequest("desktop.type-text", { sessionID: context.sessionID, text: args.text }))
            } catch (error) {
              return `Type failed: ${error instanceof Error ? error.message.slice(0, 300) : String(error).slice(0, 300)}`
            }
          },
        }),

        "computer-use-hotkey": tool({
          description: "Press a key combination (e.g. ctrl+s, shift+enter, f15) in whatever window currently has keyboard focus, after permission approval. Modifiers: ctrl/alt/shift/win plus one base key.",
          args: { keys: tool.schema.string().describe("Combo like ctrl+s, alt+f4 is NOT allowed, use e.g. ctrl+shift+p or f15") },
          execute: async (args, context) => {
            const combo = typeof args.keys === "string" ? args.keys : ""
            if (/alt\+f4/i.test(combo)) return "Hotkey failed: alt+f4 is blocked by policy."
            await context.ask({
              permission: "computer-use",
              patterns: ["desktop.hotkey", combo.slice(0, 40)],
              always: ["*"],
              metadata: { source: "deveagent.computer-use-hotkey", keys: combo },
            })
            const hostRequest = (globalThis as any).__deveagent_host_request as ((action: string, payload?: unknown) => Promise<unknown>) | undefined
            if (!hostRequest) return "Hotkey unavailable outside the desktop host."
            try {
              return JSON.stringify(await hostRequest("desktop.hotkey", { sessionID: context.sessionID, keys: combo }))
            } catch (error) {
              return `Hotkey failed: ${error instanceof Error ? error.message.slice(0, 300) : String(error).slice(0, 300)}`
            }
          },
        }),
        ...
        // DeveAgent CU audit (Plan.2026.7.29 §4): every computer-use tool action
        // lands in .deveagent/cu-audit.log — one line, honest outcome included.
        for (const [name, impl] of Object.entries(registered)) {
          if (!name.startsWith("computer-use-")) continue
          const originalExecute = (impl as { execute?: (...execArgs: unknown[]) => Promise<string> }).execute
          if (typeof originalExecute !== "function") continue
          ;(impl as { execute: unknown }).execute = async (execArgs: unknown, execContext: { sessionID?: string }) => {
            // A8: auto/full levels auto-approve the computer-use ask step (the
            // audit entry below still records the action).
            const wrappedContext =
              cuAskSkipped(workspaceDirectory) && execContext && typeof (execContext as { ask?: unknown }).ask === "function"
                ? ({
                    ...execContext,
                    ask: async (askInput: { permission?: string }) => {
                      if ((askInput as { permission?: string })?.permission === "computer-use") return undefined
                      return (execContext as { ask: (input: unknown) => Promise<unknown> }).ask(askInput)
                    },
                  } as typeof execContext)
                : execContext
            const output = await originalExecute(execArgs, wrappedContext)
            try {
              const rawArgs = (execArgs ?? {}) as Record<string, unknown>
              const target = [rawArgs.handle, rawArgs.elementName, rawArgs.command, rawArgs.keys, rawArgs.text, rawArgs.target, rawArgs.title].find(
                (value) => typeof value === "string" && value,
              ) ?? (typeof rawArgs.x === "number" && typeof rawArgs.y === "number" ? `${rawArgs.x},${rawArgs.y}` : undefined)
              let ok = true
              let detail: string | undefined
              try {
                const parsed = JSON.parse(output) as { ok?: unknown; error?: unknown; reason?: unknown }
                ok = parsed.ok !== false
                detail = typeof parsed.error === "string" ? parsed.error : typeof parsed.reason === "string" ? parsed.reason : undefined
              } catch {
                // Non-JSON outputs are prose; the structured {ok:false} shape
                // is preferred upstream, but the heuristic must at least catch
                // the refusal vocabulary the tools actually emit ("denied by
                // the foreground lock", "refused", ...), not just "failed".
                ok = !/(fail|denied|refused|unavailable|not allowlisted|is not|error|unable|cannot)/i.test(output.slice(0, 160))
                detail = output.slice(0, 160)
              }
              void appendCuAudit(workspaceDirectory, {
                sessionID: execContext?.sessionID,
                tool: name,
                action: name.replace(/^computer-use-/, ""),
...
export function getDeveAgentMetrics() {
  ...
  return {
    ...
    // ponytail: long-run memory watch — sampled at poll time (pull model,
    // zero timers), so a slow leak shows up in the Overview panel.
    memory: (() => {
      try {
...
function createTeamRun(input: TeamRunInput, status: TeamRunRecord["status"]): TeamRunRecord {
  ...
  teamMemberTraceDir = record.directory
  // DeveAgentRun mirror (Plan.2026.7.29): team/moa runs share the unified log.
  void recordDeveAgentRun(record.directory, {
    id: record.id,
    kind: record.runMode === "debate" ? "moa" : "team",
    sessionID: record.sessionID,
    directory: record.directory,
    description: record.task ?? "team run",
    status: record.status === "running" ? "running" : record.status,
...
async function completeTeamRun(record: TeamRunRecord, input: TeamRunInput) {
  ...
  // S2: the ledger row carries the parent-child mapping (which child sessions
  // the members ran in) and the real usage totals. Usage sums only over the
  // numbers members actually reported; when nobody reported, the fields stay
  // undefined — the ledger never rewrites unknown usage to 0.
  const childSessionIDs = [...new Set(members.map((member) => member.childSessionID).filter((id): id is string => typeof id === "string" && id.length > 0))].slice(0, 20)
  const sumUsage = (pick: (member: (typeof members)[number]) => number | undefined) => {
  ...
  void recordDeveAgentRun(record.directory, {
    ...
    ...(sumUsage((member) => member.tokens) !== undefined ? { tokens: sumUsage((member) => member.tokens) } : {}),
    ...(sumUsage((member) => member.cost) !== undefined ? { cost: sumUsage((member) => member.cost) } : {}),
  }).catch(() => undefined)
  await saveTeamRunsToDisk().catch(() => undefined)
  return record
}

// S3: retry the failed members of the latest failed team/moa run. Each member
// owns a child session that still holds its transcript, so the retry re-drives
// THAT session with a continuation prompt instead of starting a stranger
// session from scratch. A new run record carries this attempt; it is driven to
// completion by the promise below (allSettled), so the board and the ledger
// see an honest terminal state — and if the process dies mid-retry, the
// existing interrupted-run recovery owns the leftover row.
export async function retryFailedTeamMembers(
  input: { sessionID: string },
  runner?: { runTask: (input: unknown) => Promise<unknown>; waitTask?: (input: { jobID: string; timeout_ms?: number }) => Promise<unknown> },
): Promise<
  | { ok: false; reason: string }
  | { ok: true; runID: string; sourceRunID: string; retried: { memberID: string }[] }
> {
  if (!runner?.runTask) return { ok: false, reason: "task bridge unavailable" }
  const source = teamRunRecords.find((run) => run.sessionID === input.sessionID && run.status === "failed")
  if (!source) return { ok: false, reason: "no-failed-run" }
  const retryable = source.members.filter(
    (member) => member.status === "failed" || member.status === "unknown" || !!member.error,
  )
  if (retryable.length === 0) return { ok: false, reason: "no-retryable-members" }

  // The retry re-executes the member through the SAME native TaskTool bridge
  // the dispatch route uses, in a fresh child session. Prompting a subagent
  // session directly is not supported by the host, so continuity comes from
  // the re-sent task text, not from the old transcript.
  const team = getDeveAgentTeam(input.sessionID)
  const fullMember = (id: string) => team.members.find((member) => member.id === id)
  const record = createTeamRun(
    {
      ...
      startedAt: Date.now(),
      members: retryable.map((member) => ({ id: member.id, name: member.name, attempts: 0, status: "pending" as const })),
      tokens: 0,
      cost: 0,
  ...
  void (async () => {
    const settled = await Promise.allSettled(
      retryable.map((member) => {
        const full = fullMember(member.id)
        if (!full) throw new Error(`Team member ${member.id} is no longer configured.`)
        const outcome = runTeamMember(
          { runTask: runner.runTask, waitTask: runner.waitTask },
          full,
          full.systemPrompt || EXPERT_PROMPTS[full.role] || "You are a read-only team advisor.",
          source.task,
          full.role !== "executor",
          team.childTimeoutMs,
          (childSessionID, jobID, attempt) => recordTeamChildStart(record, member.id, childSessionID, jobID, attempt),
          undefined,
          teamChildOutputBudget(team, 0),
          0,
        )
        return outcome
      }),
    ...
    const finished = retryable.map((member, index) => {
      ...
      if (outcome.status === "fulfilled") {
        return {
          ...
          ...taskUsage(outcome.value.result),
        }
      }
      return {
        id: member.id,
        ...
        error: (outcome.reason instanceof Error ? outcome.reason.message : String(outcome.reason)).slice(0, 200),
      }
    })
    const usage = finished.map((member) => ({ tokens: "tokens" in member ? member.tokens : undefined, cost: "cost" in member ? member.cost : undefined }))
    await completeTeamRun(record, {
      sessionID: source.sessionID,
      directory: source.directory,
      task: source.task,
      runMode: source.runMode,
      startedAt: record.startedAt,
      members: finished,
      tokens: usage.reduce((total, item) => total + (item.tokens ?? 0), 0),
      cost: usage.reduce((total, item) => total + (item.cost ?? 0), 0),
      budgetTokens: source.budgetTokens,
      budgetExceeded: false,
      resumedFrom: source.id,
    } as TeamRunInput)
  ...
  return {
    ...
    retried: retryable.map((member) => ({ memberID: member.id })),
  }
}
...
type GoalState = {
  ...
  stopReason?: string
  /** store revision: bumped on every save (see loop rev) */
  rev?: number
  retryCount: number
  /** P0-2: consecutive scans where the configured budget usage was unreadable */
  budgetUnreadableCount: number
  /** P0-2: consecutive goal-verify calls where a configured verifier could not answer */
  verifyUnavailableCount: number
  ...
  // ponytail: self-iteration (Codex auto-continue parity) — after a verified
  // goal, an aux model may propose the next goal and the loop re-arms itself,
  // bounded by maxIterations. Off unless explicitly requested.
  autoIterate?: boolean
  iterations?: number
  maxIterations?: number
  criteriaDone?: boolean[]
  // ponytail: independent verifier (Plan.2026.7.29 §3) — per-criterion verdicts
...
export type GoalVerificationRecord = {
  ...
  allMet: boolean
  perCriterion: { index: number; met: boolean; reason: string }[]
  rationale: string
}
...
const EMPTY_GOAL: GoalState = {
  ...
  maxReentries: 8,
  retryCount: 0,
  budgetUnreadableCount: 0,
  verifyUnavailableCount: 0,
  attempts: [],
}
const goalStateBySession = new Map<string, GoalState>()
// Self-improvement: tool iterations since the last skill write, per session.
const autoSkillItersBySession = new Map<string, number>()
// Sessions whose work crossed the review threshold and are waiting for the
// next turn to be told (the prompt side reads and clears this).
const autoSkillReviewPending = new Set<string>()
/** Test seam: lets a regression in the idle trigger be caught by a unit test. */
export function autoSkillReviewPendingForTest(): string[] {
  return [...autoSkillReviewPending]
}
export function markAutoSkillReviewPendingForTest(sessionID: string): void {
  autoSkillReviewPending.add(sessionID)
}
...
// permission.ask hook: resolved session→agent attribution (PermissionV1.Request
// has no agent field). Bounded FIFO, sessions are stable for their lifetime.
const permissionAgentBySession = new Map<string, string>()
...
function validPersistedGoal(value: unknown): GoalState | undefined {
  ...
  const criteria = raw.criteria.filter((item): item is string => typeof item === "string").slice(0, 10).map((item) => item.slice(0, 200))
  return {
    active: true,
    ...
    budgetTokens: typeof raw.budgetTokens === "number" && Number.isFinite(raw.budgetTokens) ? Math.max(1, Math.floor(raw.budgetTokens)) : undefined,
    budgetCostUsd: typeof raw.budgetCostUsd === "number" && Number.isFinite(raw.budgetCostUsd) ? raw.budgetCostUsd : undefined,
    retryCount: typeof raw.retryCount === "number" ? Math.max(0, Math.floor(raw.retryCount)) : 0,
    budgetUnreadableCount: typeof raw.budgetUnreadableCount === "number" && Number.isFinite(raw.budgetUnreadableCount) ? Math.max(0, Math.floor(raw.budgetUnreadableCount)) : 0,
    verifyUnavailableCount: typeof raw.verifyUnavailableCount === "number" && Number.isFinite(raw.verifyUnavailableCount) ? Math.max(0, Math.floor(raw.verifyUnavailableCount)) : 0,
...
function validPersistedVerification(value: unknown, criteriaCount: number): GoalVerificationRecord | undefined {
  if (!value || typeof value !== "object") return
  const raw = value as Record<string, unknown>
  if (raw.verdict !== "confirmed" && raw.verdict !== "rejected") return
  const perCriterion = (Array.isArray(raw.perCriterion) ? raw.perCriterion : []).flatMap((item) => {
    if (!item || typeof item !== "object") return []
    const entry = item as Record<string, unknown>
    const index = typeof entry.index === "number" && Number.isFinite(entry.index) ? Math.floor(entry.index) : 0
    if (index < 1 || index > criteriaCount) return []
    return [{ index, met: entry.met === true, reason: typeof entry.reason === "string" ? entry.reason.slice(0, 300) : "" }]
  })
  if (perCriterion.length === 0) return
  return {
    at: typeof raw.at === "number" && Number.isFinite(raw.at) ? raw.at : Date.now(),
    model: typeof raw.model === "string" ? raw.model.slice(0, 120) : "unknown",
    verdict: raw.verdict,
  ...
  try {
    ...
    if (Array.isArray(data?.goals)) {
      ...
      for (const [sessionID, value] of data.goals) {
        ...
        // Recovery-only (see loadLoopsFromDisk): a key already in memory is
        // newer — its writer persists the change and a stale disk snapshot must
        // never revert or resurrect it.
        const goal = validPersistedGoal(value)
        if (!goal) continue
        const live = goalStateBySession.get(sessionID)
        const liveRev = live?.rev ?? 0
        if (live && (goal.rev ?? 0) <= liveRev) continue
        goal.rev = liveRev + 1
        if (goal) {
...
// P0-2 fail-closed bounds: a configured budget whose usage cannot be read, and
// a configured verifier that cannot answer, both stop the goal after a few
// consecutive failures instead of running (or trusting claims) forever.
const GOAL_BUDGET_UNREADABLE_MAX = 3
const GOAL_VERIFY_UNAVAILABLE_MAX = 3
...
// ponytail: paths the Guardian has denied for a session. A guardian deny only
// rejects the tool call that asked; without this record the model can achieve
// the same write through another tool (e.g. bash). Later asks whose target
// blob mentions a denied path are denied outright (heuristic: may over-block
// commands that merely mention the path - accepted for the safety posture).
const guardianDeniedPaths = new Map<string, Set<string>>()

export function recordGuardianDeniedPath(sessionID: string, blobs: string[]) {
  if (!sessionID) return
  let set = guardianDeniedPaths.get(sessionID)
  if (!set) {
    set = new Set()
    guardianDeniedPaths.set(sessionID, set)
    boundSessionMap(guardianDeniedPaths)
  }
...
export function guardianDeniedPathFor(sessionID: string, blob: string): string | undefined {
  const set = guardianDeniedPaths.get(sessionID)
  if (!set) return undefined
  for (const denied of set) {
    if (blob.includes(denied)) return denied
  }
  return undefined
}

export function clearGuardianDeniedPaths(sessionID: string | undefined) {
  if (!sessionID) return
  guardianDeniedPaths.delete(sessionID)
}
...
export async function scanGoalQueueOnce(client: unknown, directory: string) {
  ...
  if (!prompt) return
  // Race guard: a just-cancelled goal is deleted from memory immediately but
  // persisted asynchronously. Flush the write chain first so the reload below
  // cannot read the pre-cancel snapshot and resurrect the goal.
  ...
  for (const item of getGoalQueue(workspaceDirectory)) {
    ...
    // Budget gate: the goal machine has no paused state, so a breach ends the
    // goal as failed with the real numbers (same shape as retry exhaustion).
    {
      const budget = await automationBudgetBreach(client, item.sessionID, workspaceDirectory, goal)
      if (budget.breach) {
        goal.status = "failed"
        goal.active = false
        goal.stopReason = budget.breach
        goal.nextAttemptAt = undefined
        finishGoalAttempt(item.sessionID, "failed", budget.breach)
        recordGoalRunEnd(goal, item.sessionID, "failed", budget.breach)
        void saveGoalsToDisk().catch(() => undefined)
        continue
      }
      // P0-2: missing usage must not mean an uncapped run. A few consecutive
      // unreadable readings stop the goal honestly; one readable reading resets.
      if (!budget.readable) {
        // While the meter is blind no new turn may fire: an unaccounted turn
        // is exactly the uncapped spend the budget gate exists to prevent.
        goal.budgetUnreadableCount += 1
        if (goal.budgetUnreadableCount >= GOAL_BUDGET_UNREADABLE_MAX) {
          const reason = `预算用量连续 ${goal.budgetUnreadableCount} 次不可读；按预算保护停止`
          goal.status = "failed"
          goal.active = false
          goal.stopReason = reason
          goal.lastError = reason
          goal.nextAttemptAt = undefined
          finishGoalAttempt(item.sessionID, "failed", reason)
          recordGoalRunEnd(goal, item.sessionID, "failed", reason)
        }
        void saveGoalsToDisk().catch(() => undefined)
        continue
      } else if (goal.budgetUnreadableCount !== 0) {
        goal.budgetUnreadableCount = 0
        void saveGoalsToDisk().catch(() => undefined)
      }
    }
    ...
    try {
      ...
      if (leaseOwned && current === goal && current.active && current.status === "in_progress") {
        ...
        if (deadlineReached) {
          ...
          recordGoalRunEnd(current, item.sessionID, "failed", current.stopReason)
        } else {
...
export function setGoal(input: { description: string; criteria: string[]; sessionID?: string; directory?: string; maxReentries?: number; maxDurationMinutes?: number; budgetTokens?: number; budgetCostUsd?: number; autoIterate?: boolean; maxIterations?: number; iterations?: number }) {
  ...
  const budgetTokens = typeof input.budgetTokens === "number" && Number.isFinite(input.budgetTokens)
    ? Math.max(1, Math.floor(input.budgetTokens))
    : undefined
  const budgetCostUsd = typeof input.budgetCostUsd === "number" && Number.isFinite(input.budgetCostUsd)
    ? Math.max(0.0001, Math.round(input.budgetCostUsd * 10000) / 10000)
    : undefined
  const goal: GoalState = {
    ...
    verifyUnavailableCount: 0,
    attempts: [],
    ...(input.autoIterate ? { autoIterate: true, iterations: Math.max(0, Math.floor(input.iterations ?? 0)), maxIterations: Math.max(1, Math.min(input.maxIterations ?? 3, 10)) } : {}),
    criteriaDone: criteria.map(() => false),
    ...(budgetTokens !== undefined ? { budgetTokens } : {}),
    ...(budgetCostUsd !== undefined ? { budgetCostUsd } : {}),
    // ponytail: 30s worker grace window — the creating turn drives the goal
    // first (autocontinue handles in-turn continuation); the worker poll stays
    // a recovery net instead of instant double-prompting.
  ...
  // DeveAgentRun: one unified record per goal execution (Plan.2026.7.29).
  void recordDeveAgentRun(goal.directory ?? input.directory, {
    id: `goal-${input.sessionID ?? "anon"}-${goal.startedAt}`,
    kind: "goal",
    sessionID: input.sessionID,
    directory: goal.directory ?? input.directory,
    description: goal.description,
    status: "running",
    startedAt: goal.startedAt,
  }).catch(() => undefined)
  resetGuardianCircuit(input.sessionID)
  clearGuardianDeniedPaths(input.sessionID)
  void saveGoalsToDisk().catch(() => undefined)
  return { ...goal }
}

/**
 * ponytail: test hook — setGoal seeds a 30s worker grace window
 * (nextAttemptAt in the future). Recovery/scheduling tests need the goal
 * ready immediately, so they fast-forward this timestamp instead of waiting.
 */
export function goalNextAttemptForTest(sessionID: string, nextAttemptAt: number) {
  const goal = goalStateBySession.get(goalKey(sessionID))
  if (goal) goal.nextAttemptAt = nextAttemptAt
...
/**
 * DeveAgentRun: terminal goal transitions must always close their run record,
 * otherwise the Overview history shows goals stuck on "running" forever.
 */
function recordGoalRunEnd(goal: { directory?: string; description: string; startedAt?: number }, sessionID: string | undefined, status: "failed" | "stopped", stopReason: string) {
  void recordDeveAgentRun(goal.directory, {
    id: `goal-${sessionID ?? "anon"}-${goal.startedAt ?? 0}`,
    kind: "goal",
    sessionID,
...
export function clearGoal(sessionID?: string) {
  ...
  const goal = goalStateBySession.get(key)
  if (goal?.active && goal.status === "in_progress") {
    // DeveAgentRun: a cancelled in-progress goal ends its run record.
    recordGoalRunEnd(goal, sessionID, "stopped", "cancelled by user")
  }
...
/** ponytail: toggle self-iteration on a live goal without resetting it. */
export function setGoalAutoIterate(sessionID: string | undefined, enabled: boolean, maxIterations = 3) {
  const goal = goalStateBySession.get(goalKey(sessionID))
  if (!goal?.active) return { ...EMPTY_GOAL }
  goal.autoIterate = enabled
  if (enabled) {
    goal.maxIterations = Math.max(1, Math.min(Math.floor(maxIterations) || 3, 10))
    goal.iterations = Math.max(0, Math.floor(goal.iterations ?? 0))
  }
  void saveGoalsToDisk().catch(() => undefined)
  return { ...goal }
}

/** ponytail: toggle one criterion's done flag; all done => goal verified. */
export function toggleGoalCriterion(sessionID: string | undefined, index: number, done: boolean) {
  const goal = goalStateBySession.get(goalKey(sessionID))
  if (!goal?.active) return { ...EMPTY_GOAL }
  if (index < 0 || index >= goal.criteria.length) return { ...goal }
  if (!Array.isArray(goal.criteriaDone) || goal.criteriaDone.length !== goal.criteria.length) {
    goal.criteriaDone = goal.criteria.map(() => false)
  }
...
/** ponytail: record the independent verifier's per-criterion verdicts on the goal. */
export function recordGoalVerification(sessionID: string | undefined, record: GoalVerificationRecord) {
  const goal = goalStateBySession.get(goalKey(sessionID))
  if (!goal?.active) return
  goal.verifier = record
  void saveGoalsToDisk().catch(() => undefined)
}

/** ponytail: the active goal anchored to a directory (worktree panel). */
export function getActiveGoalByDirectory(directory: string) {  const resolved = automationDirectory(directory)
  for (const [sessionID, goal] of goalStateBySession) {
    if (goal.active && goal.directory === resolved) {
      return {
        sessionID,
        description: goal.description,
        status: goal.status,
        reentries: goal.reentries,
        maxReentries: goal.maxReentries,
        iterations: goal.iterations,
        criteriaDone: goal.criteriaDone,
      }
    }
  }
  return undefined
...
// P0-2: bounded fail-closed handling for a configured verifier that cannot
// answer. Keeps the goal in progress while the counter is within
// GOAL_VERIFY_UNAVAILABLE_MAX, then ends the goal as failed with the real
// numbers instead of letting an unverified claim stand forever.
export function markGoalVerifyUnavailable(sessionID: string) {
  const goal = goalStateBySession.get(sessionID)
  if (!goal || !goal.active || goal.status !== "in_progress") return getGoal(sessionID)
  goal.verifyUnavailableCount += 1
  if (goal.verifyUnavailableCount > GOAL_VERIFY_UNAVAILABLE_MAX) {
    goal.status = "failed"
    goal.active = false
    goal.stopReason = `独立验收连续 ${goal.verifyUnavailableCount} 次不可用；按验收保护停止，不采纳未经验证的完成声明`
    goal.lastError = goal.stopReason
    goal.nextAttemptAt = undefined
    finishGoalAttempt(sessionID, "failed", goal.stopReason)
    recordGoalRunEnd(goal, sessionID, "failed", goal.stopReason)
  } else {
    goal.lastError = `独立验收不可用（第 ${goal.verifyUnavailableCount}/${GOAL_VERIFY_UNAVAILABLE_MAX} 次）；保持 in_progress，稍后重试 goal-verify`
  }
  void saveGoalsToDisk().catch(() => undefined)
...
export function verifyGoal(input: { met: boolean; reason?: string; sessionID?: string }) {
  ...
  if (input.met) {
    ...
    // ponytail: self-iteration — an auto-iterate goal re-arms itself with an
    // aux-model-proposed next goal, bounded by maxIterations (default 3).
    // Fire-and-forget and fail-soft: any failure leaves the goal verified.
    if (goal.autoIterate) {
      const iterations = (goal.iterations ?? 0) + 1
      const maxIterations = goal.maxIterations ?? 3
      const previous = { description: goal.description, criteria: goal.criteria, reentries: goal.reentries }
      const directory = goal.directory
      const reentries = goal.reentries
      const maxReentries = goal.maxReentries
      void (async () => {
        try {
          if (iterations > maxIterations) {
            // Budget spent: keep the verified goal as-is, no re-arm.
            return
          }
          const proposal = await proposeNextGoal({ directory, previous, iterationsLeft: maxIterations - iterations })
          // Clear first: the verified goal stays active=true until cleared,
          // and setGoal refuses an occupied slot. If the proposal fails we
          // simply leave the goal cleared - it was verified either way.
          ...
          fresh.autoIterate = true
          fresh.maxIterations = maxIterations
          goalStateBySession.set(goalKey(input.sessionID), fresh)
          void saveGoalsToDisk().catch(() => undefined)
        } catch {}
      })()
    }
...
type LoopState = {
  ...
  budgetCostUsd?: number
  retryCount: number
  /** store revision: bumped on every save, lets loads drop stale snapshots */
  rev?: number
  maxRetries: number
  startedAt?: number
  completedAt?: number
...
function validPersistedLoop(value: unknown): LoopState | undefined {
  ...
  // R197: an explicitly provided rev must be a non-negative safe integer.
  // Invalid revisions reject the entire entry (corrupt store guard).
  if (raw.rev !== undefined) {
    if (typeof raw.rev !== "number" || !Number.isInteger(raw.rev) || raw.rev < 0 || raw.rev > Number.MAX_SAFE_INTEGER) return undefined
  }
  return {
    active: true,
    ...
    cron: typeof raw.cron === "string" && parseCron(raw.cron) ? raw.cron.trim() : undefined,
    budgetTokens: typeof raw.budgetTokens === "number" && Number.isFinite(raw.budgetTokens) ? Math.max(1, Math.floor(raw.budgetTokens)) : undefined,
    budgetCostUsd: typeof raw.budgetCostUsd === "number" && Number.isFinite(raw.budgetCostUsd) ? raw.budgetCostUsd : undefined,
    timezone: typeof raw.timezone === "string" && raw.timezone.trim() ? asTimezone(raw.timezone, "UTC") : undefined,
    retryCount: typeof raw.retryCount === "number" ? Math.max(0, Math.floor(raw.retryCount)) : 0,
...
async function loadLoopsFromDisk() {
  try {
    ...
    for (const [sessionID, value] of data.loops) {
      if (typeof sessionID !== "string" || sessionID.length > 160) continue
      // A worker owns this session's state in memory right now; the disk copy
      // is stale by definition and must not overwrite it (same guard as the
      // goal store — a pause/run-now landing just before a tick was reverted).
      if (loopWorkerInflight.has(sessionID)) continue
      const loop = validPersistedLoop(value)
      if (!loop) continue
      // The revision lives in loopRevBySession, NOT on the loop object:
      // setLoop replaces the object wholesale, so an object-carried rev would
      // restart at 0 and let a snapshot of the replaced object win.
      // Cold-start: if the session is not in loopStateBySession yet, accept
      // any disk revision (the revision gate only prevents overwriting
      // existing in-memory state with stale snapshots).
      const hasLive = loopStateBySession.has(sessionID)
      const liveRev = loopRevBySession.get(sessionID) ?? 0
      if (hasLive && (loop.rev ?? 0) <= liveRev) continue
      loopRevBySession.set(sessionID, loop.rev ?? 0)
      loopStateBySession.set(sessionID, loop)
    }
  } catch {
...
/**
 * Monotonic per-session loop revision, deliberately kept OUTSIDE the loop
 * object. setLoop() replaces that object wholesale, so an object-carried rev
 * restarts at 0 and a disk snapshot of the replaced object — which carries a
 * higher rev — wins the load comparison and resurrects its pre-mutation
 * limits. That is exactly how loop-set(maxRuns=1, interval=3600s) reverted to
 * the prompt-time default (maxRuns=8, interval=60s): Run now then executed
 * "run 1 of 8" and correctly never reached a terminal state.
 */
const loopRevBySession = new Map<string, number>()
async function saveLoopsToDisk() {
  ...
  for (const [sessionID, loop] of loops) {
    const next = (loopRevBySession.get(sessionID) ?? 0) + 1
    loopRevBySession.set(sessionID, next)
    loop.rev = next
  }
  // Sessions the map no longer tracks must not keep a counter forever; a
  // cleared-then-recreated session id is fine because the counter only ever
  // needs to dominate snapshots of a loop that still exists.
  for (const sessionID of [...loopRevBySession.keys()]) {
    if (!loopStateBySession.has(sessionID)) loopRevBySession.delete(sessionID)
  }
...
/** ponytail: unified loop run record (DeveAgentRun, Plan.2026.7.29). */
function recordLoopRun(loop: LoopState, sessionID: string | undefined, status: "running" | "completed" | "failed" | "stopped", extra?: { stopReason?: string; finishedAt?: number }) {
  if (!loop.startedAt) return Promise.resolve()
  return recordDeveAgentRun(loop.directory, {
    id: `loop-${sessionID ?? "anon"}-${loop.startedAt}`,
    kind: "loop",
    sessionID,
...
/**
 * Next scheduled run for a loop. Cron expressions win over the plain
 * interval; if a persisted cron stops matching (should not happen — it is
 * validated at set time), the interval keeps the loop alive.
 */
function nextLoopRunAt(loop: Pick<LoopState, "cron" | "timezone" | "intervalSeconds">, fromMs: number) {
  if (loop.cron) {
    const next = nextCronRun(loop.cron, fromMs, loop.timezone)
    if (next !== undefined) return next
  }
  return fromMs + loop.intervalSeconds * 1_000
}
...
export function setLoop(input: {
  ...
  // Normalize cron FIRST: a whitespace-only value must count as "no cron" for
  // both the deadline default and the validation below.
  const cron = typeof input.cron === "string" && input.cron.trim() ? input.cron.trim() : undefined
  // With a cron schedule the wall-clock deadline defaults to off — cron loops
  // are automations that may legitimately run for months, bounded by maxRuns.
  const maxDurationMinutes =
    input.maxDurationMinutes === undefined ? (cron ? undefined : 60) : boundedLoopNumber(input.maxDurationMinutes, 60, 1, 7 * 24 * 60)
  const timezone = typeof input.timezone === "string" && input.timezone.trim() ? input.timezone.trim() : undefined
  if (timezone && !isValidTimeZone(timezone)) throw new Error(`Unknown IANA timezone: ${timezone}`)
  const loopTimezone = timezone ?? asTimezone(runtimeState.unattendedTimezone, "UTC")
  // Validate in the SAME timezone the schedule is stored with, so a cron that
  // never matches in the workspace tz is rejected here instead of silently
  // falling back at schedule time.
  if (cron && !parseCron(cron)) throw new Error(`Invalid 5-field cron expression: ${cron}`)
  if (cron && nextCronRun(cron, Date.now(), loopTimezone) === undefined) throw new Error(`Cron expression never matches: ${cron}`)
  const budgetTokens = typeof input.budgetTokens === "number" && Number.isFinite(input.budgetTokens)
    ? Math.max(1, Math.floor(input.budgetTokens))
    : undefined
  const budgetCostUsd = typeof input.budgetCostUsd === "number" && Number.isFinite(input.budgetCostUsd)
  ...
  const loop: LoopState = {
    ...
    startedAt: now,
    ...(cron ? { cron, timezone: loopTimezone } : {}),
    ...(budgetTokens !== undefined ? { budgetTokens } : {}),
    ...(budgetCostUsd !== undefined ? { budgetCostUsd } : {}),
    deadlineAt: maxDurationMinutes === undefined ? undefined : now + maxDurationMinutes * 60_000,
    nextRunAt: cron ? (nextCronRun(cron, now, loopTimezone) ?? now + intervalSeconds * 1_000) : now + intervalSeconds * 1_000,
  }
  loopStateBySession.set(loopKey(input.sessionID), loop)
  resetGuardianCircuit(input.sessionID)
      clearGuardianDeniedPaths(input.sessionID)
  void saveLoopsToDisk().catch(() => undefined)
  void recordLoopRun(loop, input.sessionID, "running")
  return { ...loop }
}
...
export function clearLoop(sessionID?: string) {
  ...
  const loop = loopStateBySession.get(key)
  if (loop?.active && loop.status !== "completed" && loop.status !== "failed") {
    void recordLoopRun(loop, sessionID, "stopped", { stopReason: "cancelled by user", finishedAt: Date.now() })
  }
...
/**
 * Pull the next scheduled run to now ("run now" in the Automations UI). The
 * scanner performs the actual run — this only reschedules, it never fakes a
 * completed run.
 */
export function runLoopNow(sessionID?: string) {
  const loop = loopStateBySession.get(loopKey(sessionID))
  if (!loop?.active || loop.status !== "running") return { ok: false as const, reason: "no running loop" }
...
async function automationBudgetBreach(
  ...
  budget: { budgetTokens?: number; budgetCostUsd?: number },
): Promise<{ breach?: string; readable: boolean }> {
  // P0-2: `readable: false` means the gate could not see a number it is
  // supposed to enforce; the callers bound consecutive unreadable readings
  // instead of silently running uncapped.
  if (budget.budgetTokens === undefined && budget.budgetCostUsd === undefined) return { readable: true }
  try {
    const info = await (client as { session?: { get?: (args: unknown) => Promise<{ data?: { tokens?: unknown; cost?: unknown } }> } }).session?.get?.({
      path: { id: sessionID },
      query: { directory },
    })
    const tokens = info?.data?.tokens as { input?: unknown; output?: unknown; reasoning?: unknown; cache?: { read?: unknown; write?: unknown } } | undefined
    const cost = info?.data?.cost
    if (budget.budgetTokens !== undefined && !(tokens && typeof tokens === "object")) {
    ...
    if (budget.budgetCostUsd !== undefined) {
      ...
      if (cost >= budget.budgetCostUsd) {
        return { breach: `成本预算已用尽（${cost.toFixed(4)}/${budget.budgetCostUsd}）`, readable: true }
      }
    }
    return { readable: true }
  } catch {
    // Usage unreadable → never estimate, but do report it so the caller can
    // bound how long the goal runs with an unenforceable budget.
    return { readable: false }
  }
}
export async function scanLoopQueueOnce(client: unknown, directory: string) {
  ...
  const prompt = automationPrompt(client)
  if (!prompt) return
  // Flush pending writes first: pause/run-now mutate memory then persist
  // fire-and-forget, so reloading before the chain drains can resurrect the
  // pre-click state.
  await waitForLoopStoreFlush()
  await loadLoopsFromDisk()
  await loadMessageModesFromDisk()
  const workspaceDirectory = automationDirectory(directory) ?? path.resolve(directory)
  const loopQueue = getLoopQueue(workspaceDirectory)
  for (const item of loopQueue) {
    ...
    if (loop.runCount >= loop.maxRuns) {
      ...
      await recordLoopRun(loop, item.sessionID, "completed", { finishedAt: loop.completedAt })
      await saveLoopsToDisk().catch(() => undefined)
      continue
    }
    ...
    let deadlineReached = false
    try {
      const remainingMs = loop.deadlineAt ? Math.max(0, loop.deadlineAt - Date.now()) : undefined
      if (remainingMs !== undefined && remainingMs <= 0) {
        deadlineReached = true
        throw new Error("Loop wall-clock budget exhausted.")
      }
      const promptRun = prompt({
        path: { id: item.sessionID },
        query: { directory: workspaceDirectory },
      ...
      const deadline = remainingMs === undefined
        ...
        : new Promise<never>((_, reject) => {
            deadlineTimer = setTimeout(() => {
              deadlineReached = true
              // Fail the wait before abort can synchronously resolve the prompt.
              reject(new Error("Loop wall-clock budget exhausted."))
              void cancel?.().catch(() => undefined)
            }, remainingMs)
          })
      try {
      ...
      leaseOwned = await lease.owns()
      if (leaseOwned && loopStateBySession.get(item.sessionID) === loop && loop.active) {
        if (!deadlineReached) loop.retryCount += 1
        loop.lastError = (error instanceof Error ? error.message : String(error)).slice(0, 300)
        if (deadlineReached || loop.retryCount > loop.maxRetries) {
          loop.active = false
          loop.status = "failed"
          loop.stopReason = deadlineReached ? "Loop wall-clock budget exhausted." : "Loop retry budget exhausted."
          loop.nextRunAt = undefined
...
// Trace sink for the member-path investigation; runTeamMember has no direct
// directory binding, so the active team run directory is captured by callers.
let teamMemberTraceDir: string | undefined
function teamMemberTraceDirectory() {
  return teamMemberTraceDir
}
...
async function runTeamMember(
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    try {
      ...
      // S3 trace: which execution path a member actually takes (background
      // waitTask vs foreground fallback) while hunting the swallowed-failure
      // defect; remove once the member outcome reflects child errors.
      appendGuardianTrace(teamMemberTraceDirectory(), { kind: "team-member-path", member: member.id, hasWaitTask: Boolean(taskRunner.waitTask), attempt })
...
export async function mapTeamWithConcurrency<T, R>(items: readonly T[], limit: number, map: (item: T) => Promise<R>): Promise<R[]> {
  ...
  // One member's crash must not abandon its siblings mid-flight: every worker
  // runs to completion here, and only then does the first rejection surface.
...
// Install provenance: which URL each remote skill came from and the hash of
// the content at install time. Powers update/check — without it a re-install
// is the only refresh path and the user cannot tell "installed" from
// "installed but stale".
type RemoteSkillManifestEntry = { url: string; hash: string; installedAt: string; updatedAt?: string }
type RemoteSkillManifest = Record<string, RemoteSkillManifestEntry>

function remoteSkillManifestPath(directory?: string) {
  const dir = remoteSkillsDir(directory)
  return dir ? path.join(dir, ".manifest.json") : undefined
}

async function readRemoteSkillManifest(directory?: string): Promise<RemoteSkillManifest> {
  const file = remoteSkillManifestPath(directory)
  if (!file) return {}
  try {
    const parsed = JSON.parse(await readFile(file, "utf8"))
    return parsed && typeof parsed === "object" ? (parsed as RemoteSkillManifest) : {}
  } catch {
    return {}
  }
}

async function writeRemoteSkillManifest(directory: string | undefined, manifest: RemoteSkillManifest) {
  const file = remoteSkillManifestPath(directory)
  if (!file) return
  await writeFile(file, JSON.stringify(manifest, null, 2), { encoding: "utf8" }).catch(() => undefined)
}

export async function checkRemoteSkillUpdates(input: { directory?: string }): Promise<
  { id: string; name?: string; url: string; installedAt: string; updatedAt?: string; currentHash: string; remoteHash: string; upToDate: boolean; error?: string }[]
> {
  const manifest = await readRemoteSkillManifest(input.directory)
  const results = await Promise.all(
    Object.entries(manifest).map(async ([id, entry]) => {
      const base = { id, url: entry.url, installedAt: entry.installedAt, updatedAt: entry.updatedAt, currentHash: entry.hash }
      try {
        const normalized = normalizeRemoteSkillUrl(entry.url)
        if (!normalized.url) return { ...base, remoteHash: "", upToDate: false, error: normalized.error }
        const res = await fetch(normalized.url, { signal: AbortSignal.timeout(15_000), redirect: "error" })
        if (!res.ok) return { ...base, remoteHash: "", upToDate: false, error: `HTTP ${res.status}` }
        const content = await res.text()
        if (Buffer.byteLength(content, "utf8") > REMOTE_SKILL_MAX_BYTES) return { ...base, remoteHash: "", upToDate: false, error: "Remote Skill exceeds 256 KiB limit." }
        const remoteHash = createHash("sha256").update(content, "utf8").digest("hex")
        return { ...base, remoteHash, upToDate: remoteHash === entry.hash }
      } catch (error) {
        return { ...base, remoteHash: "", upToDate: false, error: error instanceof Error ? error.message.slice(0, 200) : String(error) }
      }
    }),
  )
  return results
...
export async function installRemoteSkill(input: { url: string; id?: string; directory?: string }): Promise<{ id: string; savedPath: string; hash?: string; changed?: boolean; error?: string }> {
  try {
    ...
    const changed = previous === undefined || createHash("sha256").update(previous, "utf8").digest("hex") !== hash
    ...
    const manifest = await readRemoteSkillManifest(input.directory)
    const now = new Date().toISOString()
    manifest[id] = { url: url.href, hash, installedAt: manifest[id]?.installedAt ?? now, updatedAt: changed && manifest[id] ? now : manifest[id]?.updatedAt }
    await writeRemoteSkillManifest(input.directory, manifest)
    return { id, savedPath: filePath, hash, changed }
  } catch (err: any) {
...
export async function removeRemoteSkill(id: string, directory?: string): Promise<{ removed: boolean }> {
  try {
    ...
    if (safeID) {
      const manifest = await readRemoteSkillManifest(directory)
      if (manifest[safeID]) {
        delete manifest[safeID]
        await writeRemoteSkillManifest(directory, manifest)
      }
    }
    return { removed: true }
  } catch {
...
export async function saveLocalSkill(input: { id?: string; name: string; description?: string; content: string }): Promise<{ id: string; savedPath: string; error?: string }> {
  try {
    ...
    // Overwrite-protection: keep up to 3 timestamped backups when replacing an
    // existing draft, so a bad promote can be rolled back by hand.
    if (existsSync(filePath)) {
      try {
        const backupDir = path.join(localSkillsDir(), "backups")
        mkdir(backupDir, { recursive: true })
        await copyFile(filePath, path.join(backupDir, `${path.basename(filePath)}.bak-${Date.now().toString(36)}`))
      } catch {}
    }
...
export function setDeveAgentTeam(input: Partial<DeveAgentTeamSnapshot> & { sessionID?: string }): DeveAgentTeamSnapshot {
  ...
  if (input.sessionID) {
    teamStateBySession.set(input.sessionID, next)
    boundSessionMap(teamStateBySession)
  } else teamState = next
