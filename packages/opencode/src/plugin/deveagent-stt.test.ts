...
import { downloadUrlCandidates } from "./deveagent-local-stt"
...
describe("deveagent stt config", () => {
  ...
  test("validate requires baseUrl/apiKey/model except for built-in engines", () => {
    ...
    expect(validateSttConfig({ provider: "local-whisper", baseUrl: "", apiKey: "", model: "ggml-base-q5_1" })).toBeNull()
  })
})
...
describe("deveagent stt presets", () => {
  ...
  test("browser built-in marker is present and needs no API fields", () => {
    const browser = STT_PROVIDER_PRESETS.find((preset) => preset.id === "browser")
...
describe("deveagent local stt download failover", () => {
  test("huggingface URLs get the hf-mirror.com candidate (CN reachability)", () => {
    expect(downloadUrlCandidates("https://huggingface.co/ggerganov/whisper.cpp/resolve/main/model.bin?download=true")).toEqual([
      "https://huggingface.co/ggerganov/whisper.cpp/resolve/main/model.bin?download=true",
      "https://hf-mirror.com/ggerganov/whisper.cpp/resolve/main/model.bin?download=true",
    ])
  })

  test("non-huggingface URLs keep a single candidate", () => {
    expect(downloadUrlCandidates("https://github.com/ggml-org/whisper.cpp/releases/download/v1.9.2/whisper-bin-x64.zip")).toEqual([
