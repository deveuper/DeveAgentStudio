...
export async function readCheckpoints(
  ...
  try {
    ...
    // Tolerant replay: a torn line from a crash is skipped, not treated as the
    // end of the file (one bad byte used to hide every later checkpoint).
    const { records } = readJsonlTolerant({
      text: data,
      parse: (value) => normalizeCheckpoint(value as Record<string, unknown>),
      max: bounded,
    })
