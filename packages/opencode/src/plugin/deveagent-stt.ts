 ...
 * /audio/transcriptions endpoint are listed. `local-whisper` uses the bundled
 * installer/runtime, while `browser` is a last-resort Chromium Web Speech
 * marker (no API fields). `custom` is an empty-slot preset the user fills in.
 */
export const STT_PROVIDER_PRESETS: Array<{
  ...
  { id: "local-whisper", name: "本地 Whisper（推荐，离线）", baseUrl: "", model: "ggml-base-q5_1", builtin: true },
  ...
  { id: "browser", name: "Chromium Web Speech（兜底）", baseUrl: "", model: "", builtin: true },
]
...
export function loadSttConfig(workspace?: string): DeveAgentSttConfig | null {
  ...
  for (const path of candidates) {
    try {
      ...
      if (raw && (raw.baseUrl || raw.provider === "browser" || raw.provider === "local-whisper")) {
        return {
...
export function validateSttConfig(config: DeveAgentSttConfig): string | null {
  ...
  if (config.provider !== "browser" && config.provider !== "local-whisper") {
...
export async function testSttConnection(
  ...
  if (config.provider === "browser") {
    return { ...base, ok: true, detail: "Chromium Web Speech 可作为兜底，但是否联网由运行环境决定。" }
  }
  if (config.provider === "local-whisper") {
    const { localSttStatus } = await import("./deveagent-local-stt")
    const status = localSttStatus()
    return {
      ...base,
      ok: status.ready,
      detail: status.ready ? `本地 Whisper ${status.version} 已就绪。` : "本地 Whisper 尚未安装完成。",
    }
  }
