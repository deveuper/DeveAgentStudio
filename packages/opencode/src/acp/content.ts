...
function partAudience(part: Extract<ReplayPart, { type: "text" }>) {
  // DeveAgent turn-tail runtime-state parts are app configuration riding the
  // user turn, not conversation content: ACP replays treat them as user
  // audience (dropped), never as assistant speech.
  if (part.text.startsWith("<deveagent-runtime-state")) return { annotations: { audience: ["user"] as Role[] } }
