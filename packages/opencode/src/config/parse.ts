...
export function schema<S extends EffectSchema.Decoder<unknown, never>>(
  ...
  // Upstream fix (38e10eb14): unknown top-level config keys no longer hard-fail
  // the whole config load. The relaxation comes from removing the strict
  // topLevelExtraKeys check; onExcessProperty defaults to "ignore" and the
  // option here lets write-back paths preserve unknown keys instead.
