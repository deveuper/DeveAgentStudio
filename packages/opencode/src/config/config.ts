...
type Info = ConfigV1.Info & {
  ...
  // mcp_origins mirrors plugin_origins for MCP entries: the DeveAgent trust gate withholds
  // project-declared servers (they spawn child processes) until the workspace is trusted.
  mcp_origins?: Record<string, { source: string; scope: ConfigPlugin.Scope }>
}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const ensureGitignore = Effect.fn("Config.ensureGitignore")(function* (dir: string) {
      // Best-effort: a read-only mount must not crash startup; the gitignore
      // write below has its own failure handling.
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const recordMcpOrigins = Effect.fnUntraced(function* (source: string, next: Info, kind?: ConfigPlugin.Scope) {
          const keys = Object.keys(next.mcp ?? {})
          if (!keys.length) return
          const hit = kind ?? (yield* pluginScopeForSource(source))
          const origins = { ...(result.mcp_origins ?? {}) }
          for (const key of keys) origins[key] = { source, scope: hit }
          result.mcp_origins = origins
        })

        const merge = (source: string, next: Info, kind?: ConfigPlugin.Scope) => {
          result = mergeConfigConcatArrays(result, next)
          return Effect.gen(function* () {
            yield* recordMcpOrigins(source, next, kind)
    ...
    const updateGlobal = Effect.fn("Config.updateGlobal")(function* (config: Info) {
      ...
      if (!file.endsWith(".jsonc")) {
        // Preserve unknown top-level keys on the write-back parse so a rewrite
        // never silently erases forward-compatible or third-party keys.
