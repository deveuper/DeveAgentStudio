...
export const sessionHandlers = HttpApiBuilder.group(InstanceHttpApi, "session", (handlers) =>
  Effect.gen(function* () {
    ...
    // Rewind checkpoints (R135): capture a git-backed snapshot BEFORE the
    // agent acts on a user turn, so a later restore can put both files and
    // conversation position back. Fail-soft: checkpointing must never break
    // prompting.
    const recordTurnCheckpoint = Effect.fn("SessionHttpApi.recordTurnCheckpoint")(function* (sessionID: SessionID) {
      const tracked = yield* Effect.option(snap.track())
      const hash = Option.getOrUndefined(tracked)
      if (!hash) return
      const dir = yield* InstanceState.directory
      const snapshotHash: string = hash
      // All failure modes are swallowed inside the promise so the helper's
      // error channel stays empty and the prompt handlers keep their contract.
      yield* Effect.promise(() =>
        import("@/plugin/deveagent-checkpoints")
          .then((m) => m.recordCheckpoint(dir, { sessionID, snapshotHash }))
          .catch(() => undefined),
      )
    })
    ...
    const prompt = Effect.fn("SessionHttpApi.prompt")(function* (ctx: {
      ...
      yield* requireSession(ctx.params.sessionID)
      yield* recordTurnCheckpoint(ctx.params.sessionID)
