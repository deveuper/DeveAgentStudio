...
import { PtyTool, PtyWriteTool } from "./pty"
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const pty = yield* PtyTool
    const ptyWrite = yield* PtyWriteTool
    const globtool = yield* GlobTool
    const writetool = yield* WriteTool
    const edit = yield* EditTool
    const greptool = yield* GrepTool
    const codemodetool = yield* CodemodeTool
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
              ...
              Effect.gen(function* () {
                ...
                const pluginCtx: PluginToolContext = {
                  ...
                  runTask: (input) =>
                    bridge.promise(
                      Effect.gen(function* () {
                        const nativeTask = yield* Tool.init(task)
                        return yield* nativeTask.execute(input as any, toolCtx)
                      }),
                    ),
                  waitTask: (input) =>
                    bridge.promise(
                      Effect.gen(function* () {
                        ...
                        // A child session whose provider turn errored does not
                        // necessarily fail the JOB (the turn ends with an
                        // errored assistant message, not a thrown task). A
                        // waiter must see the failure, not a completed pass
                        // with empty output.
                        if (childID) {
        ...
        const tool = yield* Effect.all({
          ...
          pty: Tool.init(pty),
          pty_write: Tool.init(ptyWrite),
          read: Tool.init(read),
          glob: Tool.init(globtool),
          grep: Tool.init(greptool),
          codemode: Tool.init(codemodetool),
          edit: Tool.init(edit),
        ...
        return {
          ...
          builtin: [
            ...
            tool.glob,
            tool.grep,
            // ponytail: registered like any builtin; read-only v1 wires no extra
            // permission asks inside execute, so it inherits the session's
            // default tool permission for the "codemode" id.
            tool.codemode,
            tool.edit,
            tool.write,
            tool.task,
