...
// One card rhythm for the whole overview panel: same surface, same radius, same
// padding everywhere. Cards previously mixed p-2/p-3/p-4, so no two sections
// started on the same vertical line and the eye had no stable column to read.
const CARD = "rounded-lg border border-v2-border-border-base bg-v2-background-bg-layer-02 p-3"
// The three metrics the user actually watches (usage / cache / cost) carry a
// stronger border: the emphasis is structural, not a new colour.
const CARD_CORE = "rounded-lg border border-v2-border-border-strong bg-v2-background-bg-layer-02 p-3"
// Section title vs value: the title stays a quiet 11px label, the value is the
// only thing that gets size. One title treatment instead of the three variants
// (10px faint / 11px muted / 11px medium) that were in use.
const SECTION_TITLE = "text-[11px] font-[520] uppercase tracking-wide text-v2-text-text-muted"
  ...
  return (
    <div class="flex min-h-full w-full min-w-0 max-w-full flex-col gap-3 p-3 pb-8 text-[13px] text-v2-text-text-base">
      ...
      <Switch>
           ...
           automations) fold into a collapsed section below them. */}
             ...
             <div class={`flex flex-col gap-2.5 ${CARD_CORE}`}>
               ...
               <Show
                 ...
                 fallback={<div class="text-[12px] text-v2-text-text-faint">{language.t("deveagent.dashboard.awaitingModelUsage")}</div>}
               >
                 ...
                 <div class="flex flex-col gap-1 text-[11px] text-v2-text-text-muted">
                   <span class="flex items-center gap-1"><span class="size-2 rounded-full" style={{ background: "var(--v2-state-fg-info)" }} /> {language.t("deveagent.dashboard.mainModel")} <span class="tabular-nums text-v2-text-text-base">{Math.round((sessionMetrics.sessionTotalTokens() / (sessionMetrics.sessionTotalTokens() + sessionMetrics.teamUsage().tokens)) * 100)}%</span></span>
                   <span class="flex items-center gap-1"><span class="size-2 rounded-full" style={{ background: "var(--v2-state-fg-warning)" }} /> {language.t("deveagent.statusbar.subagents")} <span class="tabular-nums text-v2-text-text-base">{Math.round((sessionMetrics.teamUsage().tokens / (sessionMetrics.sessionTotalTokens() + sessionMetrics.teamUsage().tokens)) * 100)}%</span></span>
                 </div>
                 <div class="flex flex-col gap-1.5 border-t border-v2-border-border-muted pt-2">
                   <div class="flex items-center justify-between text-[12px]">
                     <span class="flex items-center gap-1.5"><span class="size-2 rounded-full" style={{ background: "var(--v2-state-fg-info)" }} /> <span class="text-v2-text-text-muted">{language.t("deveagent.dashboard.mainModel")}</span></span>
                     <span class="tabular-nums font-medium text-v2-text-text-base">{number().format(sessionMetrics.sessionTotalTokens())}</span>
                   </div>
                   <div class="flex items-center justify-between text-[12px]">
                     <span class="flex items-center gap-1.5"><span class="size-2 rounded-full" style={{ background: "var(--v2-state-fg-warning)" }} /> <span class="text-v2-text-text-muted">{language.t("deveagent.statusbar.subagents")}</span></span>
                     <span class="tabular-nums font-medium text-v2-text-text-base">{number().format(sessionMetrics.teamUsage().tokens)}</span>
                   </div>
                 </div>
               </Show>
             </div>
             ...
             <div class={CARD_CORE}>
               ...
               <div
                 class={VALUE_HERO}
                 classList={{ "text-v2-text-text-faint": !sessionMetrics.hasUsage() }}
                 style={{
               ...
               <div class="mt-2 text-[11px] text-v2-text-text-muted">
                 <Show
                   when={sessionMetrics.hasUsage()}
                   fallback={<span class="text-v2-text-text-faint">{language.t("deveagent.dashboard.awaitingModelUsage")}</span>}
                 >
                   {language.t("deveagent.dashboard.sessionReadWrite")}{" "}
                   <span class="tabular-nums font-medium text-v2-text-text-base">{number().format(sessionMetrics.sessionCacheReadTokens())}</span>
                   {" / "}
                   <span class="tabular-nums font-medium text-v2-text-text-base">{number().format(sessionMetrics.sessionCacheWriteTokens())}</span>
                 </Show>
               </div>
               <Show when={cacheShape()?.lastReason !== undefined && cacheShape()!.lastReason !== "none"}>
                 <div class="mt-1.5 text-[10px] leading-4 text-v2-text-text-muted">
                   {language.t("deveagent.dashboard.prefixShapeChanged")}: {shapeReasonLabel(cacheShape()!.lastReason)} · {language.t("deveagent.dashboard.count")} <span class="tabular-nums">{cacheShape()!.changes}</span>{cacheShape()!.lastChangedAt ? ` · ${formatTimestamp(cacheShape()!.lastChangedAt ?? undefined)}` : ""}
                 </div>
                 <div class="mt-0.5 text-[9px] leading-4 text-v2-text-text-faint">
             ...
             <Show when={grilling()?.started}>
               <div class={`flex items-center justify-between gap-3 ${CARD}`}>
                 <div>
                   <div class={SECTION_TITLE}>Grilling Me</div>
                   ...
                   <Show when={grilling()?.completedAt}>
                     <div class="mt-0.5 text-[10px] leading-4 text-v2-text-text-muted">{language.t("deveagent.dashboard.completedAt")} {formatTimestamp(grilling()?.completedAt)}</div>
                   </Show>
                   <Show when={grilling()?.startedAt}>
                     <div class="mt-0.5 text-[10px] leading-4 text-v2-text-text-muted">{language.t("deveagent.dashboard.startedAt")} {formatTimestamp(grilling()?.startedAt)}</div>
                   </Show>
                 ...
                 <div class="text-right">
                   <div class="text-[22px] font-semibold leading-none tabular-nums text-v2-text-text-base">{grilling()?.decisionCount ?? 0}</div>
                   <div class="mt-1 text-[10px] text-v2-text-text-muted">{language.t("deveagent.dashboard.confirmedDecisions")}</div>
       ...
       <div class={`flex flex-col gap-2.5 ${CARD}`}>
        <div class="flex w-full items-center justify-between gap-2">
          <span class="flex items-center gap-2">
            ...
            <span class={SECTION_TITLE}>{hasTaskAggregate() ? language.t("deveagent.dashboard.taskTotal") : language.t("deveagent.dashboard.contextWindow")}</span>
          </span>
          <span class="text-[13px] font-medium tabular-nums text-v2-text-text-base">
        ...
        <div class="flex items-center justify-end text-[11px] text-v2-text-text-muted">
          <span>{language.t("deveagent.dashboard.untilCompaction")} <span class="tabular-nums">{sessionMetrics.hasContext() && sessionMetrics.contextLimit() ? compactTokens(sessionMetrics.contextLimit()! - sessionMetrics.totalTokens()) : "--"}</span></span>
        </div>
        ...
        <div class="flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-v2-text-text-muted">
          <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-v2-state-fg-success inline-block" /> {language.t("deveagent.dashboard.usageInput")} <span class="tabular-nums font-medium text-v2-text-text-base">{number().format(hasTaskAggregate() ? sessionMetrics.sessionInputTokens() : sessionMetrics.inputTokens())}</span></span>
          <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-v2-text-text-accent inline-block" /> {language.t("deveagent.dashboard.usageOutput")} <span class="tabular-nums font-medium text-v2-text-text-base">{number().format(hasTaskAggregate() ? sessionMetrics.sessionOutputTokens() : sessionMetrics.outputTokens())}</span></span>
          <Show when={sessionMetrics.teamUsage().tokens > 0}>
            <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-v2-state-fg-warning inline-block" /> {language.t("deveagent.statusbar.subagents")} <span class="tabular-nums font-medium text-v2-text-text-base">{number().format(sessionMetrics.teamUsage().tokens)}</span></span>
          </Show>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-3">
        ...
        <Show when={sessionMetrics.hasTaskTiming()}>
          <div class={CARD}>
            ...
            <div class={`mt-1 ${VALUE_STAT}`}>{formatElapsed(sessionMetrics.taskTiming().elapsedMs ?? 0)}</div>
            ...
            <Show when={sessionMetrics.taskTiming().completedAt !== undefined}>
              <div class="mt-1 text-[10px] leading-4 text-v2-text-text-muted">{language.t("deveagent.dashboard.completedAt")} {formatTimestamp(sessionMetrics.taskTiming().completedAt)}</div>
            </Show>
          </div>
        </Show>
        <Show when={sessionMetrics.taskAgents().length > 0}>
          <div class={`col-span-2 ${CARD}`}>
            <div class="mb-2 flex items-center justify-between gap-2">
              <div class={SECTION_TITLE}>{language.t("deveagent.statusbar.subagents")}</div>
              <div class="text-[11px] font-medium tabular-nums text-v2-text-text-base">{language.t("deveagent.dashboard.subagentCount", { count: sessionMetrics.taskAgents().length })}</div>
            </div>
            <div class="flex flex-col gap-1.5">
              <For each={sessionMetrics.taskAgents()}>
                {(agent) => (
                  <div class="flex items-center gap-2 rounded-md border border-v2-border-border-muted bg-v2-background-bg-layer-01 px-2 py-1.5">
                    ...
                    <span class="shrink-0 text-[10px] tabular-nums text-v2-text-text-muted">{number().format(agent.tokens)} tokens</span>
                    <Show when={agent.updatedAt}>
                      {(updatedAt) => <span class="shrink-0 text-[10px] tabular-nums text-v2-text-text-muted">{formatTimestamp(updatedAt())}</span>}
                    </Show>
                  </div>
                )}
              </For>
      ...
      <div class={CARD}>
        <div class="flex items-center justify-between gap-2">
          ...
          <span class="text-[10px] text-v2-text-text-faint">{language.t("deveagent.dashboard.globalDefault")}</span>
        </div>
        <Show
          when={fallbackChain().length > 0}
          fallback={<div class="mt-1 text-[12px] text-v2-text-text-faint">{language.t("deveagent.dashboard.noFallbackChain")}</div>}
        >
      ...
      <Show when={params.id}>
        <div class={CARD}>
          <div class="flex items-center justify-between gap-2">
            <div class={SECTION_TITLE}>Session Vision Chain</div>
            <Show
              when={sessionAuxiliary()?.overridden}
              fallback={<span class="text-[10px] text-v2-text-text-faint">{language.t("deveagent.dashboard.globalDefault")}</span>}
            >
              <button
                type="button"
          ...
          <Show
            ...
            fallback={<div class="mt-1 text-[12px] text-v2-text-text-faint">{language.t("deveagent.dashboard.usingGlobalVision")}</div>}
          >
            <div class="mt-1 break-words text-[12px] font-medium text-v2-text-text-base">
       ...
       <DeveagentAutomationsPanel />
        ...
        <Match when={activeTab() === "files"}>
          <div class="flex flex-col gap-2">
            <Show
              when={files().length > 0}
              fallback={
                // Empty states use the same card rhythm as populated ones; only
                // the text tone drops to faint so it reads as "nothing yet"
                // rather than as a card that failed to render.
                <div class={`${CARD} text-[12px] text-v2-text-text-faint`}>
            ...
            >
              <For each={changes()}>
                {(item) => (
                  <button
                    type="button"
                    class={`${CARD} text-left hover:border-v2-border-border-focus`}
                    onClick={() => item.file && openChange(item.file)}
                  >
                    <div class="text-[12px] text-v2-text-text-base truncate">{item.file}</div>
                    <div class="mt-1 text-[11px] tabular-nums text-v2-text-text-muted">
        ...
        <Match when={activeTab() === "codegraph"}>
          <div class="flex flex-col gap-3">
            <div class={CARD}>
              <div class="flex items-center gap-2">
                <div class="text-[13px] font-semibold text-v2-text-text-base">{language.t("deveagent.codegraph.subtitle")}</div>
                <div class="flex-1" />
                <button
                  type="button"
                  class="rounded-md border border-v2-border-border-base bg-v2-background-bg-layer-01 px-2 py-1 text-[11px] text-v2-text-text-base hover:border-v2-border-border-focus disabled:opacity-50"
              ...
              <Show when={graphIndex()}>
                {(index) => (
                  <div class="mt-2 rounded-md border border-v2-border-border-muted bg-v2-background-bg-layer-01 p-2 text-[11px] tabular-nums text-v2-text-text-muted">
              ...
              <Show when={graphIndexStatusLoading()}>
                <div class="mt-2 text-[11px] text-v2-text-text-faint">{language.t("deveagent.codegraph.readingStatus")}</div>
              </Show>
              ...
              <Show when={!sdk().directory}>
                <div class="mt-2 rounded-md border border-dashed border-v2-border-border-base bg-v2-background-bg-layer-01 p-2 text-[11px] leading-4 text-v2-text-text-faint">
              ...
              <div class="mt-3 grid grid-cols-2 gap-3">
                <div class="rounded-md border border-v2-border-border-muted bg-v2-background-bg-layer-01 p-2.5">
                  <div class={SECTION_TITLE}>{language.t("deveagent.codegraph.estimatedTokens")}</div>
                  <div
                    class="mt-1 text-[18px] font-semibold tabular-nums text-v2-text-text-base"
                    classList={{ "text-v2-text-text-faint": !contextPack() }}
                  >
                    {contextPack() ? number().format(contextPack()!.totalEstimatedTokens) : contextPackLoading() ? language.t("deveagent.codegraph.reading") : "--"}
                  </div>
                </div>
                <div class="rounded-md border border-v2-border-border-muted bg-v2-background-bg-layer-01 p-2.5">
                  <div class={SECTION_TITLE}>{language.t("deveagent.codegraph.packFiles")}</div>
                  <div
                    class="mt-1 text-[18px] font-semibold tabular-nums text-v2-text-text-base"
                    classList={{ "text-v2-text-text-faint": !contextPack() }}
                  >
              ...
              <Show when={contextPack() && (contextPack()!.tokensSaved ?? 0) > 0}>
                <div class="mt-2 rounded-md border border-v2-state-border-success bg-v2-state-bg-success p-2 text-[11px] font-medium text-v2-state-fg-success">
                  {language.t("deveagent.codegraph.tokenSaverSaved", { count: number().format(contextPack()!.tokensSaved ?? 0) })}
                  <span class="text-[10px] font-normal text-v2-text-text-muted"> · {language.t("deveagent.codegraph.original")} <span class="tabular-nums">{number().format(contextPack()!.totalOriginalTokens ?? 0)}</span> → {language.t("deveagent.codegraph.afterCompaction")} <span class="tabular-nums">{number().format(contextPack()!.totalEstimatedTokens)}</span></span>
                </div>
              </Show>
              <Show when={contextPack() && contextPack()!.tokenSaverEnabled === false}>
                <div class="mt-2 rounded-md border border-v2-border-border-base bg-v2-background-bg-layer-01 p-2 text-[11px] leading-4 text-v2-text-text-muted">
              ...
              <Show when={contextPackLoading()}>
                <div class="mt-2 text-[11px] text-v2-text-text-faint">{language.t("deveagent.codegraph.readingContextPack")}</div>
              </Show>
              <Show when={!sdk().directory}>
                <div class="mt-2 rounded-md border border-dashed border-v2-border-border-base bg-v2-background-bg-layer-01 p-2 text-[11px] leading-4 text-v2-text-text-faint">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div class={CARD}>
                <div class={SECTION_TITLE}>{language.t("deveagent.codegraph.contextFiles")}</div>
                <div class={`mt-1 ${VALUE_STAT}`}>{contextFiles().length}</div>
                <div class="mt-1 text-[10px] leading-4 text-v2-text-text-muted">{language.t("deveagent.codegraph.contextFilesSource")}</div>
              </div>
              <div class={CARD}>
                <div class={SECTION_TITLE}>{language.t("deveagent.codegraph.sessionChanges")}</div>
                <div class={`mt-1 ${VALUE_STAT}`}>{changes().length}</div>
                <div class="mt-1 text-[10px] leading-4 text-v2-text-text-muted">{language.t("deveagent.codegraph.sessionChangesSource")}</div>
              </div>
            </div>
            <Show
              when={contextPack()?.files.length}
              fallback={
                <div class="rounded-lg border border-dashed border-v2-border-border-base bg-v2-background-bg-layer-01 p-3 text-[12px] leading-5 text-v2-text-text-faint">
