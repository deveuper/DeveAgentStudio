    ...
    <Show when={open()}>
      ...
      >
        ...
        <section
          role="dialog"
          aria-modal="true"
          aria-label={language.t("deveagent.expertsDrawer.title")}
        ...
        >
          <header class="flex shrink-0 items-center justify-between border-b border-border-weak-base px-4 py-3">
            <div>
              <div class="text-[14px] font-medium text-text-strong">{language.t("deveagent.expertsDrawer.subtitle")}</div>
              <div class="mt-0.5 text-[10px] text-text-weak">{language.t("deveagent.expertsDrawer.hint")}</div>
            </div>
            <button class="size-7 rounded-md text-text-weak hover:bg-surface-base hover:text-text-base" type="button" aria-label={language.t("deveagent.expertsDrawer.close")} title={language.t("deveagent.expertsDrawer.close")} onClick={() => setOpen(false)}>
              ×
            </button>
          </header>
