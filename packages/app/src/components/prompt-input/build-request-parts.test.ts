...
describe("buildRequestParts", () => {
  test("omits empty text parts when submitting attachments", () => {
    const result = buildRequestParts({
      prompt: [{ type: "file", path: "README.md", content: "@README.md", start: 0, end: 10 }],
      context: [],
      images: [],
      text: "   ",
      messageID: "msg_empty_text",
      sessionID: "ses_empty_text",
      sessionDirectory: "/repo",
    })

    expect(result.requestParts.some((part) => part.type === "text" && !part.synthetic)).toBe(false)
    expect(result.requestParts.some((part) => part.type === "file")).toBe(true)
  })
