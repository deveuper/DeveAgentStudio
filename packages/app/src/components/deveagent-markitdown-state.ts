...
export function formatDeveAgentMarkItDownBytes(bytes: number | undefined, unknownLabel: string) {
  if (bytes === undefined || !Number.isFinite(bytes) || bytes < 0) return unknownLabel
  if (bytes < 1024) return `${bytes} B`
  if (bytes < 1024 * 1024) return `${(bytes / 1024).toFixed(1)} KB`
  return `${(bytes / (1024 * 1024)).toFixed(1)} MB`
}

export function formatDeveAgentMarkItDownTime(value: number | undefined, unknownLabel: string) {
  if (value === undefined || !Number.isFinite(value)) return unknownLabel
  const date = new Date(value)
  return Number.isNaN(date.getTime()) ? unknownLabel : date.toISOString()
}
