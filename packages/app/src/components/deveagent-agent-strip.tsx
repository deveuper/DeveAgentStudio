      ...
      >
        ...
        >
          ...
          <Show when={activeGoal()}>
            <span class="shrink-0 font-medium text-v2-text-text-base">{language.t("deveagent.agentStrip.goal")}</span>
            <Show when={criteriaTotal() > 0}>
              <span class="shrink-0 font-medium tabular-nums text-v2-text-text-base">
              ...
              <span class="h-1 w-12 shrink-0 overflow-hidden rounded-full bg-v2-border-border-muted">
                <span
                  ...
                  style={{
                    ...
                    // --v2-state-fg-accent is not a defined token; the fill was
                    // resolving to nothing and the bar rendered invisible.
                    // --v2-background-bg-accent is the real accent token.
        ...
        <Show when={open()}>
          <div class="flex flex-col gap-1.5 border-t border-v2-border-border-muted px-2.5 py-2">
            <For each={jobs() ?? []}>
              {(job) => (
