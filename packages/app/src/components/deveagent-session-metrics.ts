...
export function createDeveAgentSessionMetrics() {
  ...
  // Child tasks complete independently of the parent message stream, so their
  // usage is refreshed on a cadence. Two speeds instead of one:
  //   - in flight: the numbers are live and the user is watching them (3s).
  //   - settled: a finished tree can still gain usage from a background team
  //     job that outlived the parent turn, so keep a slow settle poll (15s)
  //     rather than stopping outright. `session_status` has no entry for an
  //     idle session (the server deletes it), so only busy/retry count as work.
  // Both stretch 4x under low power.
  const workInFlight = createMemo(() =>
    taskSessionIDs().some((sessionID) => {
      const type = sync().data.session_status[sessionID]?.type
