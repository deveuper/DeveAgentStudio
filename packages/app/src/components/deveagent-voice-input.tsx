...
import { recordingToWav } from "./deveagent-audio"
...
export function DeveagentVoiceInput(props: {
  ...
  const [engine, setEngine] = createSignal<"browser" | "api" | "local">("browser")
  ...
  const startBrowser = () => {
    ...
    if (!Ctor) {
      showToast({
        variant: "error",
        title: language.t("deveagent.voice.notSupported"),
        description: language.t("deveagent.voice.notSupportedHint"),
      })
      return
    }
    setEngine("browser")
    ...
    recognition.onerror = (event) => {
      showToast({
        variant: "error",
        title: language.t("deveagent.voice.error"),
    ...
    try {
      ...
      showToast({
        variant: "error",
        title: language.t("deveagent.voice.cannotStart"),
  ...
  const resolveSpeechEngine = async (): Promise<"browser" | "api" | "local"> => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (state.auxiliary?.speech?.providerID && state.auxiliary.speech.modelID) return "api"
      }
    } catch {
    ...
    try {
      ...
      if (config?.provider === "local-whisper") return data.local?.ready ? "local" : "browser"
  ...
  const transcribeRecording = async (blob: Blob) => {
    ...
    try {
      ...
      const response = await serverSDK().fetch(`${serverSDK().url.replace(/\/+$/, "")}/api/deveagent/voice/transcribe`, {
        ...
        body: JSON.stringify({
          audioBase64: await blobBase64(wav),
          mimeType: "audio/wav",
      ...
      showToast({
        variant: "error",
        title: `${engine() === "local" ? language.t("deveagent.voice.local") : language.t("deveagent.voice.online")} ${language.t("deveagent.voice.transcriptionFallback")}`,
        description: error instanceof Error ? error.message : "unknown",
      })
      releaseStream()
      startBrowser()
  ...
  const startRecorder = async (nextEngine: "api" | "local") => {
    try {
      const mediaDevices = (navigator as Navigator & { mediaDevices?: MediaDevices }).mediaDevices
      if (!mediaDevices) throw new Error(language.t("deveagent.voice.micUnsupported"))
      ...
      recorder.onerror = () => {
        releaseStream()
        setRecording(false)
        showToast({ variant: "error", title: language.t("deveagent.voice.recordingFailed"), description: language.t("deveagent.voice.recordingFailedHint") })
      }
      recorder.onstop = () => {
      ...
      showToast({
        variant: "error",
        title: language.t("deveagent.voice.cannotStartRecording"),
        description: error instanceof Error ? error.message : "unknown",
      })
    }
  }
  ...
  const toggle = async () => {
    ...
    const nextEngine = await resolveSpeechEngine()
    if (nextEngine !== "browser" && typeof MediaRecorder !== "undefined") {
      await startRecorder(nextEngine)
      return
    }
    startBrowser()
  }
  ...
  return (
    <button
      type="button"
      aria-label={processing() ? language.t("deveagent.voice.transcribing") : recording() ? language.t("deveagent.voice.stopRecording") : language.t("deveagent.voice.startVoiceInput")}
      title={processing() ? language.t("deveagent.voice.transcribing") : recording() ? language.t("deveagent.voice.stopRecording") : language.t("deveagent.voice.startVoiceInput")}
      class={`relative inline-flex size-8 shrink-0 items-center justify-center rounded-full border border-transparent bg-transparent transition-colors ${
        recording()
          ? "border-red-500/40 bg-red-500/10 text-red-600 hover:bg-red-500/20 dark:text-red-300"
          : "text-v2-icon-icon-base hover:bg-v2-background-bg-layer-03"
      }`}
      disabled={processing()}
      onClick={() => void toggle()}
      ...
      data-engine={engine()}
    >
      <IconV2 name="microphone" size="small" />
      <Show when={recording()}>
        <span aria-hidden class="absolute right-0.5 top-0.5 size-1.5 rounded-full bg-red-500" />
      </Show>
    </button>
  )
}
