    ...
    <div class="flex flex-col gap-3 p-3">
      <div class="rounded-lg border border-v2-border-border-base bg-v2-background-bg-layer-02 p-3">
        <div class="flex items-center gap-2">
          <div class="text-[13px] font-semibold text-v2-text-text-base">{language.t("deveagent.codegraph.title")}</div>
          <div class="flex-1" />
          <button
            type="button"
      ...
      <div class="rounded-lg border border-v2-border-border-base bg-v2-background-bg-layer-02 p-3">
        <div class="text-[13px] font-semibold text-v2-text-text-base">Context Pack</div>
        <div class="mt-2 grid grid-cols-2 gap-3">
          <div class="rounded-md border border-v2-border-border-muted bg-v2-background-bg-layer-01 p-2.5">
            <div class="text-[11px] font-[520] uppercase tracking-wide text-v2-text-text-muted">{language.t("deveagent.codegraph.estimatedTokens")}</div>
            <div class="mt-1 text-[18px] font-semibold tabular-nums text-v2-text-text-base" classList={{ "text-v2-text-text-faint": !pack() }}>
              {pack() ? pack()!.totalEstimatedTokens.toLocaleString() : packLoading() ? language.t("deveagent.codegraph.reading") : "--"}
            </div>
          </div>
          <div class="rounded-md border border-v2-border-border-muted bg-v2-background-bg-layer-01 p-2.5">
            <div class="text-[11px] font-[520] uppercase tracking-wide text-v2-text-text-muted">{language.t("deveagent.codegraph.packFiles")}</div>
            <div class="mt-1 text-[18px] font-semibold tabular-nums text-v2-text-text-base" classList={{ "text-v2-text-text-faint": !pack() }}>
              {pack() ? pack()!.files.length : packLoading() ? language.t("deveagent.codegraph.reading") : "--"}
            </div>
          </div>
        </div>
        <Show when={pack() && (pack()!.tokensSaved ?? 0) > 0}>
          <div class="mt-2 rounded-md border border-v2-state-border-success bg-v2-state-bg-success p-2 text-[11px] font-medium text-v2-state-fg-success">
            ...
            <span class="text-[10px] font-normal text-v2-text-text-muted">
              {" "}
              · {language.t("deveagent.codegraph.original")} <span class="tabular-nums">{pack()!.totalOriginalTokens.toLocaleString()}</span> → {language.t("deveagent.codegraph.compacted")} <span class="tabular-nums">{pack()!.totalEstimatedTokens.toLocaleString()}</span>
            </span>
          </div>
        </Show>
        <Show when={packLoading()}>
          <div class="mt-2 text-[11px] text-v2-text-text-faint">{language.t("deveagent.codegraph.readingPack")}</div>
        </Show>
