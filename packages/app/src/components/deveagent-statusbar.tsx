...
export function DeveagentStatusBar(props: { showTerminalToggle?: boolean } = {}) {
  ...
  return (
    <div
      ...
      class="relative z-10 flex min-h-7 min-w-0 max-w-full shrink-0 items-center gap-x-3 overflow-x-auto border-t border-v2-border-border-muted bg-surface-raised-base px-3 py-1 text-[12px] whitespace-nowrap select-none [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
      style={{ contain: "inline-size" }}
    >
      <span
        class="size-1.5 shrink-0 rounded-full"
        title={sessionMetrics.hasUsage() ? language.t("deveagent.statusbar.usage") : language.t("deveagent.statusbar.ready")}
        aria-label={sessionMetrics.hasUsage() ? language.t("deveagent.statusbar.usage") : language.t("deveagent.statusbar.ready")}
        data-component="deveagent-statusbar-state"
        style={{ background: sessionMetrics.hasUsage() ? "var(--v2-state-fg-success)" : "var(--v2-state-fg-warning)" }}
      />
      <StatusMetric
        glyph="◔" label={language.t("deveagent.statusbar.context")}
      ...
      <Show when={composer.snapshot().permissionMode !== "default"}>
        <span class="flex shrink-0 items-center gap-1" title={`${language.t("deveagent.statusbar.permission")}: ${permissionLabel()}`}>
          <span class="text-v2-text-text-faint select-none" aria-hidden="true">⛨</span>
          <span class="text-v2-text-text-muted">{permissionLabel()}</span>
        </span>
      </Show>
      <Show when={goal()?.active && goal()?.startedAt}>
        <span class="flex items-center gap-1 shrink-0 rounded bg-v2-background-bg-accent/10 px-1.5 py-0.5 text-v2-text-text-accent">
          ...
          <Show when={goal()?.status === "in_progress"}>
            <span class="text-[9px] tabular-nums opacity-80">
              {(() => {
      ...
      <Show when={loop()?.active}>
        <span class="flex items-center gap-1 shrink-0 rounded bg-v2-background-bg-accent/10 px-1.5 py-0.5 tabular-nums text-v2-text-text-accent">
          ...
          <Show when={loop()?.status === "running" && loop()?.nextRunAt}>
            <span data-component="deveagent-statusbar-loop-countdown" class="shrink-0 tabular-nums opacity-90">
      ...
      <KbPopover open={moreOpen()} onOpenChange={setMoreOpen} placement="top-end">
        <KbPopover.Trigger
          ...
          class="shrink-0 rounded px-1 text-v2-text-text-faint hover:text-v2-text-text-base"
          title={language.t("deveagent.statusbar.moreStatus")}
          aria-label={language.t("deveagent.statusbar.moreStatus")}
        >
          ⋯
        </KbPopover.Trigger>
        <KbPopover.Portal>
          ...
          >
            <div class="flex flex-col gap-2 text-[11px]">
              <div class="flex items-center justify-between"><span class="text-v2-text-text-muted">{language.t("deveagent.statusbar.tools")}</span><span class="text-v2-text-text-base">{composer.snapshot().toolExecution === "parallel" ? language.t("deveagent.statusbar.parallel") : language.t("deveagent.statusbar.serial")}</span></div>
              <Show when={sessionMetrics.teamUsage().tokens > 0}>
                <div class="flex items-center justify-between"><span class="text-v2-text-text-muted">{language.t("deveagent.statusbar.subagents")}</span><span class="tabular-nums text-v2-text-text-base">{sessionMetrics.taskAgents().length} · {number().format(sessionMetrics.teamUsage().tokens)}</span></div>
              </Show>
              <Show when={sessionMetrics.hasTaskAggregate() && sessionMetrics.taskTiming().elapsedMs !== undefined}>
                <div class="flex items-center justify-between"><span class="text-v2-text-text-muted">{language.t("deveagent.statusbar.taskSpan")}</span><span class="tabular-nums text-v2-text-text-base">{formatElapsed(sessionMetrics.taskTiming().elapsedMs ?? 0)}</span></div>
              </Show>
              ...
              <div class="flex items-center justify-between border-t border-v2-border-border-muted pt-2">
                <span class="text-v2-text-text-muted">{language.t("deveagent.statusbar.provider")}</span><span class="text-v2-text-text-base">{providerLabel()}</span>
              </div>
...
// Reasonix-style stat: a glyph carries the meaning, the number carries the
// value, and the full label lives in the tooltip. This keeps the bar to a
...
function StatusMetric(props: { glyph: string; label: string; value: string; tone: "success" | "warning" | "danger" | "base"; dataAction?: string }) {
  ...
  const empty = () => props.value === "--" || props.value === ""
  return (
    <span
    ...
    >
      <span class="text-v2-text-text-faint select-none" aria-hidden="true">{props.glyph}</span>
