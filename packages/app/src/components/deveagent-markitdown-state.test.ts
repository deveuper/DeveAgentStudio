...
describe("DeveAgent MarkItDown state", () => {
  ...
  test("ignores unrelated text metadata and does not invent byte values", () => {
    ...
    expect(formatDeveAgentMarkItDownBytes(undefined, "size unknown")).toBe("size unknown")
    expect(formatDeveAgentMarkItDownBytes(2048, "size unknown")).toBe("2.0 KB")
    expect(formatDeveAgentMarkItDownTime(undefined, "time unknown")).toBe("time unknown")
    expect(formatDeveAgentMarkItDownTime(1_700_000_000_000, "time unknown")).toBe("2023-11-14T22:13:20.000Z")
  })
})
