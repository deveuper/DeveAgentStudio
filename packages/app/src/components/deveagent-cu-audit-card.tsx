    ...
    <Show when={(log()?.entries.length ?? 0) > 0}>
      ...
      >
        <div class="flex w-full items-center justify-between gap-2">
          <span class="text-[11px] font-[520] uppercase tracking-wide text-v2-text-text-muted">{language.t("deveagent.cuAudit.title")}</span>
          <span class="text-[11px] text-v2-text-text-faint">{language.t("deveagent.cuAudit.recentActions")}</span>
        </div>
        <div class="flex flex-col gap-1">
          <For each={(log()?.entries ?? []).slice(0, 10)}>
            {(entry) => (
              <div class="flex items-center gap-2 text-[11px] leading-5" title={entry.detail || entry.target || undefined}>
        ...
        <Show when={(log()?.runs.length ?? 0) > 0}>
          <div class="mt-1 border-t border-v2-border-border-muted pt-2">
            <div class="text-[11px] font-[520] uppercase tracking-wide text-v2-text-text-muted">{language.t("deveagent.cuAudit.runHistory")}</div>
