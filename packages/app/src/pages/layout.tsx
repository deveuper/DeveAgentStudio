...
export default function Layout(props: ParentProps) {
  ...
  const openCurrentDirectory = (app?: string) => {
    ...
    if (!directory) {
      showToast({ variant: "error", title: language.t("deveagent.layout.noWorkspaceToOpen"), description: language.t("deveagent.layout.selectProjectFirst") })
      return
    }
    if (!platform.openPath) {
      showToast({ variant: "error", title: language.t("deveagent.layout.cannotOpenExternalApp"), description: directory })
      return
    }
    platform.openPath(directory, app).catch((error: unknown) => {
      showToast({
        variant: "error",
        title: language.t("deveagent.layout.openWorkspaceFailed"),
  ...
  makeEventListener(window, "deveagent:new-session", () => {
    ...
    if (!directory) {
      showToast({ variant: "error", title: language.t("deveagent.layout.cannotCreateSession"), description: language.t("deveagent.layout.noWorkingDirectory") })
      return
    }
  ...
  const SidebarPanel = (panelProps: {
    ...
    const loadArchivedSessions = async () => {
      ...
      try {
        ...
        if (request !== archivedRequest || archivedState.directory !== directoryKey) return
        setArchivedState({ loading: false, items: [], error: errorMessage(error, language.t("deveagent.layout.archivedSessionsLoadFailed")) })
      }
    }
    ...
    const deleteArchivedSession = async (session: Session) => {
      if (!window.confirm(language.t("deveagent.layout.deleteSessionConfirm", { title: session.title || language.t("deveagent.session.unnamed") }))) return
      try {
        ...
        if (!result.data) throw new Error(language.t("deveagent.layout.sessionDeleteNotConfirmed"))
        setArchivedState("items", (items) => items.filter((item) => item.id !== session.id))
        showToast({ title: language.t("deveagent.layout.sessionDeleted"), description: language.t("deveagent.layout.sessionDeletedDescription") })
      } catch (error) {
        setArchivedState("error", errorMessage(error, language.t("deveagent.layout.archivedSessionDeleteFailed")))
        showToast({ variant: "error", title: language.t("deveagent.layout.cannotDeleteArchivedSession"), description: errorMessage(error, language.t("common.requestFailed")) })
      }
    }
                      ...
                      <DropdownMenu.Content class="mt-1">
                        ...
                        >
                          <DropdownMenu.ItemLabel>
                            {isPinned(project) ? language.t("deveagent.layout.unpinProject") : language.t("deveagent.layout.pinProject")}
                          </DropdownMenu.ItemLabel>
                        </DropdownMenu.Item>
                        <DropdownMenu.Item
                        ...
                        >
                          <DropdownMenu.ItemLabel>{language.t("deveagent.layout.createDeliveryWorktree")}</DropdownMenu.ItemLabel>
                        </DropdownMenu.Item>
                        <DropdownMenu.Item
                          onSelect={() => {
                        ...
                        <DropdownMenu.Item
                          ...
                          onSelect={() => {
                            ...
                            if (!dir) return
                            if (!platform.openPath) {
                              showToast({ variant: "error", title: language.t("deveagent.layout.cannotOpenExternalApp"), description: dir })
                              return
                            }
                            ...
                            platform.openPath(dir).catch((error: unknown) => {
                              showToast({
                                variant: "error",
                                title: language.t("deveagent.layout.cannotRevealInFileManager"),
                                description: error instanceof Error ? error.message : String(error),
                              })
                            })
                          }}
                        >
                          <DropdownMenu.ItemLabel>{language.t("deveagent.layout.showInFileExplorer")}</DropdownMenu.ItemLabel>
                        </DropdownMenu.Item>
                        <DropdownMenu.Item
                        ...
                        >
                          <DropdownMenu.ItemLabel>{language.t("deveagent.shell.archiveProject")}</DropdownMenu.ItemLabel>
                        </DropdownMenu.Item>
                        <DropdownMenu.Item
                          data-action="project-delete-entry"
                          onSelect={() => {
                            if (!window.confirm(language.t("deveagent.shell.removeProjectConfirm"))) return
                            forgetProjectEntry(project.worktree)
                          }}
                        >
                          <DropdownMenu.ItemLabel>{language.t("deveagent.shell.removeProjectEntry")}</DropdownMenu.ItemLabel>
                        </DropdownMenu.Item>
                        <DropdownMenu.Separator />
                        <DropdownMenu.Item
              ...
              <div class="flex-1 min-h-0 flex flex-col">
                ...
                <div class="shrink-0 border-t border-border-weaker-base px-2 py-1" data-component="archived-sessions">
                  ...
                  >
                    ...
                    <span class="min-w-0 flex-1 truncate">{language.t("deveagent.layout.archivedSessions")}</span>
                  ...
                  <Show when={archivedState.open}>
                    <div class="max-h-44 overflow-y-auto py-1">
                      <Show
                        ...
                        fallback={<div class="px-2 py-2 text-11-regular text-text-weak">{language.t("deveagent.layout.archivedSessionsLoadFailed")}</div>}
                      >
                        <Show
                          when={archivedState.items.length > 0}
                          fallback={<div class="px-2 py-2 text-11-regular text-text-weak">{language.t("deveagent.layout.noArchivedSessions")}</div>}
                        >
                            ...
                            {(session) => (
                              <div class="group/archived-session flex min-w-0 items-center gap-1 rounded-md px-1 hover:bg-surface-raised-base">
                                <button
                                  ...
                                  title={session.title}
                                >
                                  {session.title || language.t("deveagent.session.unnamed")}
                                </button>
                                <button
                                  type="button"
                                  ...
                                  title={language.t("deveagent.layout.restoreToSessionList")}
                                >
                                  {language.t("deveagent.layout.restore")}
                                </button>
                                <button
                                  type="button"
                                  ...
                                  title={language.t("deveagent.layout.deleteSessionRecordHint")}
                                >
  ...
  const createIsolatedWorktree = async (project: LocalProject) => {
    ...
    try {
      ...
      if (!created?.directory) throw new Error(language.t("deveagent.layout.noWorkspaceDirectoryReturned"))
      ...
      showToast({ title: language.t("deveagent.layout.worktreeCreated"), description: created.directory })
    } catch (error) {
      showToast({
        variant: "error",
        title: language.t("deveagent.layout.worktreeCreateFailed"),
        description: error instanceof Error ? error.message : String(error),
      })
    }
  }
          ...
          <div class="flex-1 min-h-0 min-w-0 flex">
              ...
              fallback={
                ...
                >
                  ...
                  <IconButton
                    ...
                    variant="ghost"
                    size="small"
                    title={language.t("deveagent.shell.showSidebar")}
                    aria-label={language.t("deveagent.shell.showSidebar")}
            ...
            >
              <aside class="flex min-h-0 w-[clamp(224px,20vw,296px)] shrink-0 flex-col border-r border-border-weaker-base bg-surface-base">
                {/* R195-G4: brand block compressed - the tagline was pure
                    decoration; the name alone identifies the app. */}
                <div class="flex items-center gap-2 border-b border-border-weaker-base bg-background-base px-3 py-2">
                  <div class="flex size-6 shrink-0 items-center justify-center rounded-md bg-v2-background-bg-accent text-[10px] font-semibold text-white">
                    DA
                  </div>
                  <div class="min-w-0 flex-1 text-12-medium text-text-strong">DeveAgent Studio</div>
                  <IconButton
                    icon="arrow-left"
                    variant="ghost"
                    size="small"
                    class="!rounded-md"
                    title={language.t("deveagent.shell.hideSidebar")}
                    aria-label={language.t("deveagent.shell.hideSidebar")}
                    onClick={() => setDeveagentSidebarOpen(false)}
                  />
                </div>
                <div
                  class="min-h-0 flex-1 overflow-x-hidden overflow-y-auto bg-background-base [scrollbar-color:var(--v2-border-border-muted)_transparent] [scrollbar-gutter:stable] [scrollbar-width:thin] [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-v2-border-border-muted hover:[&::-webkit-scrollbar-thumb]:bg-v2-border-border-focus"
                        ...
                        <Show when={projects().length > 0}>
                          <div class="px-2 py-1" data-component="deveagent-sidebar-projects">
                            <button
                              ...
                              class="flex h-7 w-full items-center gap-2 rounded-md px-1.5 text-left text-[11px] font-medium uppercase tracking-wide text-text-faint hover:bg-surface-raised-base hover:text-text-weak"
                            ...
                            >
                              ...
                              />
                              <span class="flex-1">{language.t("deveagent.shell.projects")}</span>
                                    ...
                                    >
                                      ...
                                      >
                                        <Show when={working()} fallback={<Icon name="folder" size="small" class="shrink-0 text-icon-weak" />}>
                                          <span
                                            ...
                                            title={language.t("deveagent.shell.projectWorking")}
                                          />
                                        </Show>
                                      ...
                                      <div class="relative shrink-0">
                                        <IconButton
                                          ...
                                          aria-label={language.t("deveagent.layout.projectActions")}
                                        ...
                                        <Show when={deveagentProjectMenu() === pathKey(projectRoot(project.worktree))}>
                                          ...
                                          >
                                            ...
                                            >
                                              {language.t("deveagent.shell.archiveProject")}
                                            </button>
                                            <button
                                              type="button"
                                              role="menuitem"
                                            ...
                                            >
                                              {language.t("deveagent.shell.removeProjectEntry")}
                                            </button>
                                          </div>
                                        </Show>
                                      </div>
                        ...
                        <Show when={archivedProjects().length > 0}>
                          ...
                          >
                            ...
                            >
                              ...
                              />
                              <span class="flex-1">{language.t("deveagent.shell.archivedProjects")}</span>
                                ...
                                {(archived) => (
                                  <div class="group/archived flex h-8 items-center gap-1 rounded-md px-1 text-12-medium text-text-weak hover:bg-surface-raised-base">
                                    ...
                                    <button
                                      ...
                                      title={language.t("deveagent.shell.removeArchivedProject")}
                ...
                <div class="flex h-10 shrink-0 items-center gap-1 border-t border-border-weaker-base bg-background-base px-2" data-component="deveagent-sidebar-footer">
                  ...
                  >
                    ...
                    <span>{language.t("deveagent.shell.settings")}</span>
                  </button>
                </div>
              </aside>
            </Show>
            <main class="flex min-h-0 min-w-0 basis-0 flex-1 flex-col overflow-x-hidden contain-strict">
              ...
              >
                <div class="min-w-0 flex-1">
                  ...
                  <div class="truncate text-[11px] text-text-faint" data-component="deveagent-current-directory">
                    {currentDir() || language.t("deveagent.shell.openProjectHint")}
                  </div>
                </div>
                <DropdownMenu modal={true}>
                  <DropdownMenu.Trigger
                    ...
                    size="small"
                    class="!rounded-md"
                    title={language.t("deveagent.shell.openWorkspace")}
                    aria-label={language.t("deveagent.shell.openWorkspace")}
                  ...
                  <DropdownMenu.Portal>
                    <DropdownMenu.Content class="mt-1" data-component="deveagent-open-workspace-menu">
                      <DropdownMenu.Item onSelect={() => openCurrentDirectory()}>
                        <DropdownMenu.ItemLabel>{language.t("deveagent.shell.fileManager")}</DropdownMenu.ItemLabel>
                      </DropdownMenu.Item>
                      ...
                      <DropdownMenu.Item onSelect={() => {
                        ...
                        void navigator.clipboard.writeText(directory).then(
                          () => showToast({ variant: "success", title: language.t("deveagent.shell.pathCopied"), description: directory }),
                          (error: unknown) => showToast({ variant: "error", title: language.t("deveagent.shell.copyPathFailed"), description: String(error) }),
                        )
                      }}>
                        <DropdownMenu.ItemLabel>{language.t("deveagent.shell.copyPath")}</DropdownMenu.ItemLabel>
                      </DropdownMenu.Item>
                    </DropdownMenu.Content>
                  </DropdownMenu.Portal>
                </DropdownMenu>
                ...
                >
                  {language.t("deveagent.sidebar.review")}
                </button>
                <IconButton
                  icon="plus"
                  variant="ghost"
                  size="small"
                  class="!rounded-md"
                  title={language.t("deveagent.sidebar.newSession")}
                  aria-label={language.t("deveagent.sidebar.newSession")}
