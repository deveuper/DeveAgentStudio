...
import { DeveagentCodeGraphHomePanel } from "@/components/deveagent-codegraph-home"
...
function HomeDesign() {
  ...
  const [state, setState] = createStore({
    ...
    sessionNoteKind: "" as "" | "review" | "metrics",
  })

  onMount(() => {
    const handleOpenPanel = (event: Event) => {
      const detail = (event as CustomEvent<string>).detail
      if (detail === "memory") setState("memoryOpen", true)
      if (detail === "team") setState("teamOpen", true)
      if (detail === "codegraph") setState("codegraphOpen", true)
      if (detail === "review" || detail === "metrics") {
        setState("sessionNoteKind", detail)
        setState("sessionNoteOpen", true)
      }
      if (detail === "close") {
        setState("memoryOpen", false)
        setState("teamOpen", false)
        setState("codegraphOpen", false)
        setState("sessionNoteOpen", false)
        setState("sessionNoteKind", "")
      }
    }
    window.addEventListener("deveagent:open-panel", handleOpenPanel)
    onCleanup(() => window.removeEventListener("deveagent:open-panel", handleOpenPanel))
  })
  ...
  return (
    <div class="rounded-[10px] shadow-[var(--v2-elevation-raised)] m-2 min-h-0 lg:overflow-hidden bg-v2-background-bg-base self-stretch flex-1">
      ...
      <Show when={state.memoryOpen}>
        <div class="fixed inset-x-0 bottom-0 top-10 z-[70] flex" onClick={() => setState("memoryOpen", false)}>
          <div class="min-w-0 flex-1" />
          <section
            role="dialog"
            aria-modal="true"
            aria-label="Memory"
            class="flex h-full w-[460px] max-w-[92vw] flex-col overflow-hidden border-l border-v2-border-border-muted bg-v2-background-bg-base shadow-[var(--v2-elevation-raised)]"
            onClick={(event) => event.stopPropagation()}
          >
            <header class="flex shrink-0 items-center justify-between border-b border-v2-border-border-muted px-4 py-3">
              <div>
                <div class="text-[14px] font-medium text-v2-text-text-base">Memory</div>
                <div class="mt-0.5 text-[10px] text-v2-text-text-muted">{language.t("deveagent.home.memoryPanelSubtitle")}</div>
              </div>
              <button class="size-7 rounded-md text-v2-text-text-muted hover:bg-v2-background-bg-layer-02 hover:text-v2-text-text-base" type="button" aria-label={language.t("deveagent.home.closeMemory")} title={language.t("deveagent.home.closeMemory")} onClick={() => setState("memoryOpen", false)}>
                ×
              </button>
            </header>
            <div class="min-h-0 flex-1 overflow-hidden bg-v2-background-bg-base">
              <Show
                when={state.selection.directory}
                fallback={
                  <div class="flex h-full items-center justify-center px-8 text-center text-[12px] text-v2-text-text-muted">
                    {language.t("deveagent.home.selectProjectForMemory")}
                  </div>
                }
              >
                <SDKProvider directory={state.selection.directory ?? ""}>
      ...
      <Show when={state.teamOpen}>
        <div class="fixed inset-x-0 bottom-0 top-10 z-[70] flex" onClick={() => setState("teamOpen", false)}>
          <div class="min-w-0 flex-1" />
          <section
            role="dialog"
            aria-modal="true"
            aria-label={language.t("deveagent.sidebar.team")}
            class="flex h-full w-[560px] max-w-[92vw] flex-col overflow-hidden border-l border-v2-border-border-muted bg-v2-background-bg-base shadow-[var(--v2-elevation-raised)]"
            onClick={(event) => event.stopPropagation()}
          >
            <header class="flex shrink-0 items-center justify-between border-b border-v2-border-border-muted px-4 py-3">
              <div>
                <div class="text-[14px] font-medium text-v2-text-text-base">{language.t("deveagent.sidebar.team")}</div>
                <div class="mt-0.5 text-[10px] text-v2-text-text-muted">{language.t("deveagent.home.teamPanelSubtitle")}</div>
              </div>
              <button class="size-7 rounded-md text-v2-text-text-muted hover:bg-v2-background-bg-layer-02 hover:text-v2-text-text-base" type="button" aria-label={language.t("deveagent.newSession.closeTeamPanel")} title={language.t("deveagent.newSession.closeTeamPanel")} onClick={() => setState("teamOpen", false)}>
                ×
              </button>
            </header>
            <div class="min-h-0 flex-1 overflow-hidden bg-v2-background-bg-base">
      ...
      <Show when={state.codegraphOpen}>
        <div class="fixed inset-x-0 bottom-0 top-10 z-[70] flex" onClick={() => setState("codegraphOpen", false)}>
          <div class="min-w-0 flex-1" />
          <section
            role="dialog"
            aria-modal="true"
          ...
          >
            <header class="flex shrink-0 items-center justify-between border-b border-v2-border-border-muted px-4 py-3">
              <div>
                ...
                <div class="mt-0.5 text-[10px] text-v2-text-text-muted">{language.t("deveagent.home.codeGraphPanelSubtitle", { action: language.t("deveagent.home.openSession") })}</div>
              </div>
              <button class="size-7 rounded-md text-v2-text-text-muted hover:bg-v2-background-bg-layer-02 hover:text-v2-text-text-base" type="button" aria-label={language.t("deveagent.home.closeCodeGraph")} title={language.t("deveagent.home.closeCodeGraph")} onClick={() => setState("codegraphOpen", false)}>
                ×
              </button>
            </header>
            <div class="min-h-0 flex-1 overflow-y-auto bg-v2-background-bg-base">
              <Show
                ...
                fallback={
                  <div class="flex h-full items-center justify-center px-8 text-center text-[12px] text-v2-text-text-muted">
                    {language.t("deveagent.home.selectProjectForCodeGraph")}
                  </div>
                }
              >
                {(directory) => <DeveagentCodeGraphHomePanel directory={directory} />}
              </Show>
            </div>
          </section>
        </div>
      </Show>
      <Show when={state.sessionNoteOpen}>
        <div class="fixed inset-x-0 bottom-0 top-10 z-[70] flex" onClick={() => setState("sessionNoteOpen", false)}>
          <div class="min-w-0 flex-1" />
          <section
            role="dialog"
            aria-modal="true"
            aria-label={state.sessionNoteKind === "review" ? language.t("deveagent.sidebar.review") : language.t("deveagent.sidebar.overview")}
            class="flex h-full w-[420px] max-w-[92vw] flex-col overflow-hidden border-l border-v2-border-border-muted bg-v2-background-bg-base shadow-[var(--v2-elevation-raised)]"
            onClick={(event) => event.stopPropagation()}
          >
            <header class="flex shrink-0 items-center justify-between border-b border-v2-border-border-muted px-4 py-3">
              <div>
                <div class="text-[14px] font-medium text-v2-text-text-base">{state.sessionNoteKind === "review" ? language.t("deveagent.sidebar.review") : language.t("deveagent.sidebar.overview")}</div>
                <div class="mt-0.5 text-[10px] text-v2-text-text-muted">{language.t("deveagent.home.sessionNoteNeedsSession")}</div>
              </div>
              <button class="size-7 rounded-md text-v2-text-text-muted hover:bg-v2-background-bg-layer-02 hover:text-v2-text-text-base" type="button" aria-label={state.sessionNoteKind === "review" ? language.t("deveagent.session.closeReviewPanel") : language.t("deveagent.session.closeOverviewPanel")} title={language.t("common.close")} onClick={() => setState("sessionNoteOpen", false)}>
                ×
              </button>
            </header>
            <div class="flex flex-1 flex-col items-center justify-center gap-3 px-10 text-center">
              <div class="text-[12px] leading-5 text-v2-text-text-muted">
                {state.sessionNoteKind === "review"
                  ? language.t("deveagent.home.reviewEmptyHint")
                  : language.t("deveagent.home.overviewEmptyHint")}
              </div>
              <button
                type="button"
                data-action="home-open-session"
                class="rounded-md border border-v2-border-border-focus/40 bg-v2-background-bg-accent/10 px-3 py-1.5 text-[12px] font-medium text-v2-text-text-accent hover:bg-v2-background-bg-accent/15 disabled:opacity-40"
              ...
              >
                {language.t("deveagent.home.openSession")}
              </button>
              <Show when={!newSessionProject()}>
                <div class="text-[11px] text-v2-text-text-faint">{language.t("deveagent.home.noProjects")}</div>
              </Show>
            </div>
          </section>
        </div>
...
function LegacyHome() {
  ...
  return (
    <div class="mx-auto mt-55 w-full md:w-auto px-4">
      <div class="md:w-xl opacity-12 text-4xl font-bold tracking-widest text-text-base select-none">DeveAgent</div>
      <Button
        size="large"
        variant="ghost"
