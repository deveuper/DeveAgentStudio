...
export function SessionComposerRegion(props: {
  ...
  const selectProject = async (worktree: string) => {
    layout.projects.open(worktree)
    const loaded = await serverSync().project.loadSessions(worktree)
  ...
  const controls = createMemo(() => ({
    ...
    model: {
      ...
      // Pricing metadata and auth state are not reliable for free OpenCode
      // models. The native full catalog is the useful path whenever models
      // are actually available; the unpaid fallback is only for empty state.
      paid: local.model.list().length > 0,
      // A provider query that never settles (offline, hung sidecar) used to
      // keep this true forever, which hid the model chip entirely - the user
      // could not open the picker OR reach the "connect a provider" dialog.
      // Loading only counts while there is nothing to show; once any model
      // list exists (or the queries settled) the chip must render.
      loading:
        local.model.list().length === 0 &&
        (agentsQuery.isLoading || providersQuery.isLoading || globalProvidersQuery.isLoading),
    },
    projects: {
