...
export function createTimelineModel(input: {
  ...
  const recoverMissing = (sessionID: string, error: unknown) => {
    if (!isServerNotFoundError(error)) throw error
    input.onMissing?.(sessionID)
  }
    ...
    ([directory, id]) => {
      ...
      refreshFrame = requestAnimationFrame(() => {
        ...
        refreshTimer = window.setTimeout(() => {
          ...
          untrack(() => {
            if (stale) void sync().session.sync(id, { force: true }).catch((error) => recoverMissing(id, error))
          })
        }, 0)
      })
