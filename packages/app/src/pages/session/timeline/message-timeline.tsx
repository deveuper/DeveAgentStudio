...
import { loadMessageModes, messageMode } from "@/components/deveagent-message-modes"
...
export function MessageTimeline(props: {
  ...
  onMissingSession?: (sessionID: string, error: unknown) => void
}) {
  ...
  const language = useLanguage()
  // R201: plan-mode output carries an explicit badge so planning content is
  // distinguishable from execution output in the transcript. P0-4: the badge
  // reads the mode stamped onto the turn's user message at submit time, so
  // switching the composer later cannot re-label historical turns.
  const planBadgeVisible = (userMessageID: string | undefined) => messageMode(sessionID(), userMessageID) === "plan"
  ...
  // P0-4: keep the per-turn mode snapshots fresh; message count participates so
  // a turn submitted in this view picks up its own stamp (deduped in-flight).
  createEffect(() => {
    const id = sessionID()
    const count = sessionMessages().length
    if (id) void loadMessageModes(id)
  })
    ...
    on(
      ...
      ([id, description]) => {
        ...
        void sync().session.sync(id).catch((error) => {
          if (isServerNotFoundError(error)) props.onMissingSession?.(id, error)
          else throw error
        })
      },
      { defer: true },
  ...
  const deleteSession = async (sessionID: string) => {
    ...
    // Cancel any goal/loop on this session first so the workers never retry a
    // dead session (best-effort; the deletion proceeds regardless).
    await cancelSessionAutomation(serverSDK(), sessionID)
      ...
      case "AssistantPart": {
        ...
        return (
          <TimelineRowFrame row={assistantPartRow}>
            <div data-slot="session-turn-message-container" class="w-full px-4 md:px-5">
              {/* R191-G2: the assistant side of a turn had no attribution at
                  all - bare text after a right-aligned user bubble. A small
                  role marker on the FIRST assistant row of the turn makes the
                  transcript scannable without bubbling the reply. */}
              <Show when={!assistantPartRow().previousAssistantPart}>
                <div
                  data-component="session-assistant-role"
                  class="mb-1 flex items-center gap-1.5 select-none"
                  aria-hidden="true"
                >
                  <span class="flex size-4 items-center justify-center rounded-[4px] bg-v2-background-bg-accent/15 text-[8px] font-semibold tracking-wide text-v2-text-text-accent">
                    DA
                  </span>
                  <span class="text-[11px] text-v2-text-text-muted">DeveAgent</span>
                  <Show when={planBadgeVisible(assistantPartRow().userMessageID)}>
                    <span class="rounded bg-v2-background-bg-accent/15 px-1 text-[10px] font-medium text-v2-text-text-accent">{language.t("deveagent.session.planBadge")}</span>
                  </Show>
                </div>
              </Show>
              <div
