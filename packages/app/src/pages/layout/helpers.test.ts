...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("deveagent://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("deveagent://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("deveagent://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("deveagent://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("deveagent://open-project")).toBeUndefined()
    expect(parseDeepLink("deveagent://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "deveagent://open-project?directory=/a",
      "deveagent://other?directory=/b",
      "deveagent://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("deveagent://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("deveagent://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("deveagent://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("deveagent://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "deveagent://new-session?directory=/a",
      "deveagent://open-project?directory=/b",
      "deveagent://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __DEVEAGENT__: {
        deepLinks: ["deveagent://open-project?directory=/a"],
      },
    } as unknown as Window & { __DEVEAGENT__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["deveagent://open-project?directory=/a"])
...
describe("session time grouping (R205-G10)", () => {
  const NOW = new Date("2026-09-13T15:00:00").getTime()
  const session = (updated: number) =>
    ({ id: "s" + updated, directory: "/w", time: { created: updated, updated } }) as unknown as Session

  test("today uses the calendar day, not a rolling 24h", () => {
    expect(sessionTimeGroup(session(new Date("2026-09-13T00:01:00").getTime()), NOW)).toBe("today")
    expect(sessionTimeGroup(session(NOW), NOW)).toBe("today")
  })

  test("yesterday and this week land in the week bucket", () => {
    expect(sessionTimeGroup(session(new Date("2026-09-12T23:59:00").getTime()), NOW)).toBe("week")
    expect(sessionTimeGroup(session(NOW - 6 * 24 * 60 * 60 * 1000), NOW)).toBe("week")
  })

  test("older than 7 days is earlier", () => {
    expect(sessionTimeGroup(session(NOW - 8 * 24 * 60 * 60 * 1000), NOW)).toBe("earlier")
    expect(sessionTimeGroup(session(0), NOW)).toBe("earlier")
  })

  test("groups come in fixed order and empty buckets are omitted", () => {
    const grouped = groupSessionsByTime(
      [session(NOW - 30 * 24 * 60 * 60 * 1000), session(NOW - 1000), session(NOW - 2 * 24 * 60 * 60 * 1000)],
      NOW,
    )
    expect(grouped.map((g) => g.group)).toEqual(["today", "week", "earlier"])
    expect(grouped[0].sessions).toHaveLength(1)
    expect(grouped[1].sessions).toHaveLength(1)
    expect(grouped[2].sessions).toHaveLength(1)
    const onlyToday = groupSessionsByTime([session(NOW)], NOW)
