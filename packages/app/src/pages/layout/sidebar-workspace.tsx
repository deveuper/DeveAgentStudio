...
import { groupSessionsByTime, sortedRootSessions, type SessionTimeGroup } from "./helpers"
...
const WorkspaceSessionList = (props: {
  ...
  <nav class="flex flex-col gap-1">
    ...
    <For each={groupSessionsByTime(props.sessions(), Date.now())}>
      {(bucket) => (
        <div class="flex flex-col gap-1">
          <div
            data-component="sidebar-session-group"
            data-group={bucket.group}
            class="px-2 pt-2 pb-0.5 text-[10px] font-medium uppercase tracking-wide text-v2-text-text-faint select-none"
          >
            {bucket.group === "today" ? props.language.t("deveagent.sidebar.groupToday") : bucket.group === "week" ? props.language.t("deveagent.sidebar.groupLast7Days") : props.language.t("deveagent.sidebar.groupEarlier")}
          </div>
      ...
      <Collapsible variant="ghost" open={open()} class="shrink-0" onOpenChange={openWrapper}>
        ...
        <Collapsible.Content>
          <WorkspaceSessionList
            ...
            mobile={props.mobile}
            ctx={props.ctx}
            showNew={() => false} // R195-G4: dedupe - the workspace group button is the single entry
            loading={loading}
            sessions={sessions}
            hasMore={hasMore}
            loadMore={loadMore}
