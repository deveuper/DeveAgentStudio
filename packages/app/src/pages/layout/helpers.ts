...
// R205-G10: sidebar sessions bucket into Today / Last 7 days / Earlier so
// the list is scannable without per-row timestamps. Today is calendar-day
// based; the week bucket rolls 7*24h. Empty buckets are omitted and the
// order is fixed (today, week, earlier).
export type SessionTimeGroup = "today" | "week" | "earlier"

export function sessionTimeGroup(session: Session, now: number): SessionTimeGroup {
  const updated = session.time?.updated ?? session.time?.created ?? 0
  const startOfToday = new Date(now)
  startOfToday.setHours(0, 0, 0, 0)
  if (updated >= startOfToday.getTime()) return "today"
  if (updated >= now - 7 * 24 * 60 * 60 * 1000) return "week"
  return "earlier"
}

export const SESSION_GROUP_ORDER: SessionTimeGroup[] = ["today", "week", "earlier"]

export function groupSessionsByTime(
  sessions: Session[],
  now: number,
): { group: SessionTimeGroup; sessions: Session[] }[] {
  const buckets: Record<SessionTimeGroup, Session[]> = { today: [], week: [], earlier: [] }
  for (const session of sessions) buckets[sessionTimeGroup(session, now)].push(session)
  return SESSION_GROUP_ORDER.filter((group) => buckets[group].length > 0).map((group) => ({ group, sessions: buckets[group] }))
}
...
export function getProjectAvatarSource(id?: string, icon?: { color?: string; url?: string; override?: string }) {
  // The built-in project used to render the OpenCode "O" favicon; DeveAgent
  // shows no branded avatar for it (same fallback as icon-less projects).
