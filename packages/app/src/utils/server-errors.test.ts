...
import { formatServerError, isServerNotFoundError, parseReadableConfigInvalidError } from "./server-errors"
...
describe("isServerNotFoundError", () => {
  test("recognizes wrapped and direct SDK session-not-found shapes only", () => {
    expect(isServerNotFoundError(new Error("missing", { cause: { status: 404 } }))).toBe(true)
    expect(isServerNotFoundError({ status: 404 })).toBe(true)
    expect(isServerNotFoundError({ response: { status: 404 } })).toBe(true)
    expect(isServerNotFoundError({ cause: { response: { status: 404 } } })).toBe(true)
    expect(
      isServerNotFoundError({
        body: {
          name: "NotFoundError",
          data: { message: "Session not found: ses_0503bc51affeyYF4LowqOSxT3D" },
        },
        status: 404,
      }),
    ).toBe(true)
    ...
    expect(isServerNotFoundError(new Error("failed", { cause: { status: 500 } }))).toBe(false)
    expect(isServerNotFoundError({ body: { name: "NotFoundError", data: { message: "Project not found: p1" } } })).toBe(false)
    expect(isServerNotFoundError({ status: 500 })).toBe(false)
    expect(isServerNotFoundError(new Error("missing"))).toBe(false)
  })
})
