...
export const isServerNotFoundError = (error: unknown) => {
  return hasSessionNotFoundShape(error, new Set())
}

function hasSessionNotFoundShape(value: unknown, seen: Set<object>): boolean {
  ...
  // The generated SDK can expose a 404 either as a transport response or as
  // { body: { name: "NotFoundError", data: { message } } }. Treat only the
  // session-specific named error as recoverable, not every arbitrary 404.
  if (
    record.name === "NotFoundError" &&
    typeof record.data === "object" &&
    record.data !== null &&
    typeof (record.data as { message?: unknown }).message === "string" &&
    (record.data as { message: string }).message.startsWith("Session not found:")
  ) {
    return true
  }

  return hasSessionNotFoundShape(record.response, seen) || hasSessionNotFoundShape(record.cause, seen) || hasSessionNotFoundShape(record.body, seen)
}
