...
describe("session prefetch", () => {
  ...
  test("removes a rejected inflight prefetch so a later session sync cannot reuse it", async () => {
    clearSessionPrefetch(scope, "/tmp/rejected", ["ses_missing"])

    const pending = runSessionPrefetch({
      directory: "/tmp/rejected",
      scope,
      sessionID: "ses_missing",
      task: async () => Promise.reject(new Error("Session not found: ses_missing")),
    })

    await expect(pending).rejects.toThrow("Session not found: ses_missing")
    expect(getSessionPrefetchPromise(scope, "/tmp/rejected", "ses_missing")).toBeUndefined()
  })
