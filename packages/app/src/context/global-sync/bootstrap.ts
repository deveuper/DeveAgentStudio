...
function warmSessions(input: {
  ...
  if (ids.length === 0) return Promise.resolve(known)
  return Promise.all(
    ids.map((sessionID) =>
      retry(() => input.sdk.session.get({ sessionID }))
        ...
        .catch((error) => {
          // A stale permission/question can outlive its session after a server
          // restart. It must not fail the whole bootstrap Promise.all.
          if (isServerNotFoundError(error)) return undefined
          throw error
        }),
    ),
  ).then((warmed) => new Set([...known, ...warmed.filter((id): id is string => !!id)]))
}
...
export async function bootstrapDirectory(input: {
  ...
  setStore: SetStoreFunction<State>
  vcsCache: VcsCache
  loadSessions: (directory: string) => Promise<boolean> | void
  translate: (key: string, vars?: Record<string, string | number>) => string
  global: {
    config: Config
    path: Path
  ...
  ;(async () => {
      ...
      () =>
        retry(() =>
          input.sdk.permission.list().then((x) => {
            ...
            return warmSessions({ ids, store: input.store, setStore: input.setStore, sdk: input.sdk }).then((available) =>
              batch(() => {
    ...
    // ponytail: MCP status connects every configured server (30s timeout each,
    // unbounded) — awaiting it here made one dead server stall the whole
    // workbench for 30s on every project open. Warm it in the background
    // instead; consumers read the query reactively and get it when ready.
    if (input.mcp) {
      void input.queryClient.fetchQuery(loadMcpQuery(input.scope, input.directory, input.sdk)).catch(() => {})
    }
