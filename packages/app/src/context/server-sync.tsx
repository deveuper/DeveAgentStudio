...
export function normalizeSessionListResponse<T>(response: { data?: T[] } | undefined): T[] {
...
export function createServerSyncContextInner(serverSDK: ServerSDK) {
  ...
  async function loadSessions(directory: string, options?: { limit?: number; notifyOnError?: boolean }): Promise<boolean> {
    const key = directoryKey(directory)
    if (!key) return true
      ...
      .fetchQuery({
        ...
        queryFn: () =>
          loadRootSessionsWithFallback({
            ...
            const nonArchived = normalizeSessionListResponse(x)
              .filter((s) => !!s?.id)
