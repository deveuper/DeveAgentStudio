...
export const createDirSyncContext = (
  ...
  const recoverMissingSession = (directory: string, setStore: Setter, sessionID: string, error: unknown) => {
    if (!isServerNotFoundError(error)) return false

    // A server restart or deletion can race any of the session endpoints. Clear
    // every local cache and let the shared tab owner leave the dead route.
    seenFor(directory).delete(sessionID)
    evict(directory, setStore, [sessionID])
    notifySessionTabsRemoved({ directory, sessionIDs: [sessionID] })
    return true
  }
  ...
  const loadMessages = async (input: {
    ...
    await fetchMessages(input)
      ...
      .catch((error) => {
        if (recoverMissingSession(input.directory, input.setStore, input.sessionID, error)) return
        throw error
      })
      .finally(() => {
        setMeta(
        ...
        return runInflight(inflight, key, async () => {
          ...
          if (pending) {
            try {
              ...
              // A prefetch can outlive a deleted session. Route that 404 through
              // the same cache/tab cleanup as every foreground session request.
              if (recoverMissingSession(directory, setStore, sessionID, error)) return
              throw error
            }
            const seeded = getSessionPrefetch(serverSDK.scope, directory, sessionID)
            if (seeded && store.message[sessionID] !== undefined && meta.limit[key] === undefined) {
